...
class memberObj  extends DaftarObj2{
	var $Prefix = 'member';
	...
	var $PageTitle = 'DATA MEMBER';
	var $PageIcon = 'images/masterdata_ico.gif';
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='member.xls';
	var $namaModulCetak='MASTER DATA';
	var $Cetak_Judul = 'member';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'memberForm';
	var $noModul=9;
			...
			$TotalColSpan2 = $this->FieldSum_Cp2[$Mode-1];//$Mode ==1 ? 5 : 4;
				...
				"<tr>
					...
					<td class='GarisDaftar' align='right'><b>".number_format($this->totalSaldo,0,',','.')." </div></td>
						<td class='$ColStyle' colspan='2' align='center'><b></td>
				</tr>" ;
	...
	$aqry = "UPDATE member set k='$dk',l='$dl',m='$dm',n='$dn',o='$do',nm_member='$nama' where concat (k,' ',l,' ',m,' ',n,' ',o)='".$idplh."'";$cek .= $aqry;
	...
	//get data -----------------
	 ...
	 $idplh = $_REQUEST[$this->Prefix.'_idplh'];
		...
		if($fmST == 0){
				if(mysql_num_rows(mysql_query("select * from member where email = '$email'")) != 0){
				...
				if($err == ''){
					...
					mysql_query(VulnWalkerInsert('member',$data));
					$cek = VulnWalkerInsert('member',$data);
				}
			}else{
				...
				mysql_query(VulnWalkerUpdate('member',$data,"email='$hubla'"));
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'getdata':{
				...
				$n = substr($Id, 6,2);
				$o = substr($Id, 9,2);
				$get = mysql_fetch_array( mysql_query("select *, concat(k,'.',l,'.',m,'.',n,'.',o) as kodemember  from member where k='$k' AND l='$l' AND m='$m' AND n='$n' AND o='$o'"));


				$content = array('kode_member' => $get['kodemember'], 'nm_member' => $get['nm_member']);
		...
		case 'modul':{
				...
				$idPenggunaAplikasi = $member_cb[0];
	...
	function setPage_OtherScript(){
		...
		return
			 ...
			 <script type='text/javascript' src='js/member/member.js' language='JavaScript' ></script>
	...
	function setFormEditdata($dt){
	 ...
	 $this->form_height = 150;
	  if ($this->form_fmST==1) {
		$this->form_caption = 'FORM EDIT KODE member';
	  }
		...
		$queryKAedit=mysql_fetch_array(mysql_query("SELECT k, nm_member FROM member WHERE k='$k' and l = '0' and m='0' and n='00' and o='00'")) ;
		$cek.=$queryKAedit;
		$queryKBedit=mysql_fetch_array(mysql_query("SELECT l, nm_member FROM member WHERE k='$k' and l='$l' and m= '0' and n='00' and o='00'")) ;
		$queryKCedit=mysql_fetch_array(mysql_query("SELECT m, nm_member FROM member WHERE k='$k' and l='$l' and m='$m' and n='00' and o='00'")) ;
		$queryKDedit=mysql_fetch_array(mysql_query("SELECT n, nm_member FROM member WHERE k='$k' and l='$l' and m='$m' and n='$n' and o='00'")) ;
		$queryKEedit=mysql_fetch_array(mysql_query("SELECT o, nm_member FROM member WHERE k='$k' and l='$l' and m='$m' and n='$n' and o='$o'")) ;
	//	$cek.="SELECT ke, nm_account FROM ref_jurnal WHERE ka='$data_ka' and kb='$data_kb' and kc='$data_kc' and kd='$data_kd' and ke='$data_ke' and kf='0'";
		...
		$datka=$queryKAedit['k'].".  ".$queryKAedit['nm_member'];
		$datkb=$queryKBedit['l'].". ".$queryKBedit['nm_member'];
		$datkc=$queryKCedit['m']." .  ".$queryKCedit['nm_member'];
		$datkd=$queryKDedit['n'].". ".$queryKDedit['nm_member'];
		$datke=$queryKEedit['o'];
       ...
       //items ----------------------
		  ...
		  'kode_Akun' => array(
			...
			'Kode_Rincian_Objek' => array(
						...
						<input type='text' name='nm_member' id='nm_member' value='".$dt['nm_member']."' size='36px'>
						</div>",
						 ),
	...
	//	$nip	 = '';
		...
		$readonly='readonly';
		$get = mysql_fetch_array(mysql_query("select * from member where email ='$dt'"));
	...
	<A href=\"pages.php?Pg=member\" title='member' style='color:blue'  >member</a> |
	...
	function setKolomHeader($Mode=1, $Checkbox=''){
		...
		$headerTable =
			"<thead>
				...
				<th class='th01' width=150>PASSWORD</th>
				<th class='th01' >NAMA LENGKAP</th>
				<th class='th01' >NO TELEPON </th>
				<th class='th01' >SALDO </th>
				<th class='th01' >MERK HP </th>
				<th class='th01' >IMEI </th>
				<th class='th01' >STATUS </th>
				<th class='th01'  width=50 >VERIFIKASI </th>
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
		...
		$Koloms[] = array('', $isi['device_name']);
		$Koloms[] = array('', $isi['imei']);
		$Koloms[] = array('', $isi['status']);
	...
	$TampilOpt =
			...
			genFilterBar(
			 ...
			 $arrayStatus = array(
				 			array('registered','AKTIF'),
				 			array('suspend','SUSPEND'),
			 );

			 $arrayVerifikasi = array(
				 			array('ok','TERVERIFIKASI'),
				 			array('belum','BELUM'),
			 );
			 $comboStatus = cmbArray('filterStatus',$filterStatus,$arrayStatus,'--STATUS--',"onchange=$this->Prefix.refreshList(true)");
			 $comboVerifikasi = cmbArray('filterVerifikasi',$filterVerifikasi,$arrayVerifikasi,'--VERIFIKASI--',"onchange=$this->Prefix.refreshList(true)");
			 $TampilOpt =
					 ...
					 <td>VERIFIKASI </td>
					 <td>: </td>
					 <td style='width:90%;'>$comboVerifikasi </td>
					 </tr>
					 <tr>
						<td>STATUS </td>
						<td>: </td>
					 ...
					 <td>NAMA LENGKAP </td>
					 <td>: </td>
					 <td style='width:90%;'><input type='text' name='filterNama' id='filterNama' value='$filterNama' size='100' > </td>
					 </tr>
					 <tr>
					<td>NOMOR TELEPON </td>
					<td>: </td>
					<td style='width:90%;'><input type='text' name='filterNomorTelepon' id='filterNomorTelepon' value='$filterNomorTelepon'  size='100'> </td>
					</tr>

					<tr>
				 <td>MERK HP</td>
				 <td>: </td>
				 <td style='width:90%;'><input type='text' name='filterMerk' id='filterMerk' value='$filterMerk'  size='100'> </td>
				 </tr>

				 <tr>
				<td>IMEI</td>
				<td>: </td>
				<td style='width:90%;'><input type='text' name='filterImei' id='filterImei' value='$filterImei'  size='100'> </td>
				</tr>
	...
	function getDaftarOpsi($Mode=1){
		...
		if(!empty($filterNomorTelepon))$arrKondisi[] = "no_telepon like '%$filterNomorTelepon%'";
		if(!empty($filterNama))$arrKondisi[] = "nama_lengkap like '%$filterNama%'";
		if(!empty($filterMerk))$arrKondisi[] = "device_name like '%$filterMerk%'";
		if($filterImei != '')$arrKondisi[] = "imei like '%$filterImei%'";
		if(!empty($filterStatus))$arrKondisi[] = "status = '$filterStatus'";
		if(!empty($filterVerifikasi))$arrKondisi[] = "verifikasi = '$filterVerifikasi'";
