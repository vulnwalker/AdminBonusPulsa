...
class cariprogramObj  extends DaftarObj2{	
	  ...
	  switch($tipe){	
		...
		case 'getp':{
				...
				$pnya = $_REQUEST['pnya'];
					...
					$qry = "SELECT * FROM ref_programkegiatan WHERE p='$pnya' AND q='00' ";
					$aqry = mysql_query($qry);
					$daqry = mysql_fetch_array($aqry);
					
					$kgt = "SELECT q,concat (q,'. ',nama_program_kegiatan) as nama FROM ref_programkegiatan WHERE p='$pnya' AND q!='00'";$cek.=$kgt;
					
					$content['kegiatan'] = cmbQuery('kegiatan','',$kgt,"style='width:500px;' ",'--- PILIH KEGIATAN ---');
					
					$content['program'] = $daqry['p'].". ".$daqry['nama_program_kegiatan'];
					$content['p'] = $daqry['p'];
   ...
   function setPage_OtherScript(){
	 ...
	 $headerTable =
	  "<thead>
	   <tr>
	  	   ...
	  	   /*$Checkbox*/"		
		   <th class='th01' rowspan='2'>Program</th>
	   </thead>";
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $Koloms[] = array('align="left"',"<a href='javascript:".$this->Prefix.".pilProg(`".$isi['p']."`)' >".$isi['p'].". ".$isi['nama_program_kegiatan']."</a>");
	 return $Koloms;
	}
