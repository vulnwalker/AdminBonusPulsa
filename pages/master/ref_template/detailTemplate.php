	  ...
	  switch($tipe){	
		...
		case 'setTempTemplate': {
			...
			if ($status == "644" ){
				...
				$data = array('c1' => $cmbUrusanForm, 'c' => $cmbBidangForm , 'd' => $cmbSKPDForm,'e' =>$cmbUnitForm ,  'username' => $username );
				$curl = curl_init('http://123.231.253.228/atisisbada_v2/curl/insertTempTemplate.php') ;
				curl_setopt($curl, CURLOPT_POST, true);
				curl_setopt($curl, CURLOPT_POSTFIELDS, $data);
				curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
				curl_setopt($curl, CURLOPT_REFERER,'http://123.231.253.228/atisisbada_v2/curl/insertTempTemplate.php');
				curl_exec($curl);
		...
		case 'setCookiesUnit': {
			...
			setcookie('VulnWalkerUrusan',$cmbUrusanForm);
			setcookie('VulnWalkerBidang',$cmbBidangForm);
			setcookie('VulnWalkerSkpd',$cmbSKPDForm);	
			setcookie('VulnWalkerUnit',$cmbUnitForm);
			break;
		}
...
$baris = $_REQUEST['baris'];
	...
	$selectedBidang = $c;
	$selectedSkpd = $d;
			 $selectedUrusan = $_COOKIE['VulnWalkerUrusan'];
			 $selectedBidang = $_COOKIE['VulnWalkerBidang'];
			 $selectedskpd = $_COOKIE['VulnWalkerSkpd'];
			 $selectedUnit = $_COOKIE['VulnWalkerUnit'];
...
/*			<div class='FilterBar' style='margin-top:5px;'>".
	...
	function getDaftarOpsi($Mode=1){
		...
		$c1 =  $_COOKIE['VulnWalkerUrusan'];
		$c = $_COOKIE['VulnWalkerBidang'];
		$d = $_COOKIE['VulnWalkerSkpd'];
		$e = $_COOKIE['VulnWalkerUnit'];
		$username = $_COOKIE['coID'];
