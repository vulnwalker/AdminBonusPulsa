...
class refstandarhargaObj  extends DaftarObj2{	
		...
		if($err==''){
			...
			if($fmST == 0){
				...
				$data = array(	  'spesifikasi'=>$spesifikasi,
					 ...
					 mysql_query(VulnWalkerUpdate("ref_std_harga",$data,"concat(f1,'.',f2,'.',f,'.',g,'.',h,'.',i,'.',j) = '$kodeBarang'"));
					 $cek .=VulnWalkerInsert("ref_std_harga",$data);
	...
	function setPage_OtherScript(){
		...
		return 
			 ...
			 <script type='text/javascript' src='js/master/refstandarharga/".strtolower($this->Prefix).".js' language='JavaScript' ></script><script type='text/javascript' src='js/master/refstandarharga/refbarang.js' language='JavaScript' ></script>
			 ".
	...
	$cek ='';
		...
		$this->form_fmST = 1;				
		//get data 
		$aqry = "SELECT * FROM  ref_std_harga WHERE f1=$f1 and f2=$f2 and f='$f' and g='$g' and h='$h' and i='$i' and j='$j' "; $cek.=$aqry;
	...
	function setForm($dt){	
	 ...
	 $this->form_height = 250;
	  if ($this->form_fmST==0) {
		...
		$kodeBarang =$dt['f1'].".".$dt['f2'].".".$dt['f'].".".$dt['g'].".".$dt['h'].".".$dt['i'].".".$dt['j'];
	 ...
	 $headerTable =
	  "<thead>
	   <tr>
  	   ...
  	   $Checkbox		
	   <th class='th01' width='150' colspan='7'>KODE BARANG</th>
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $arrayGetNamabarang = mysql_fetch_array(mysql_query("select * from ref_barang where concat(f1,'-',f2,'-',f,'-',g,'-',h,'-',i,'-',j) = '$kodeBarang' "));
	...
	$TampilOpt = 
			...
			<td>".
			cmbQuery1("cmbAkun",$cmbAkun,"select f1 as valueCmbAkun , nm_barang from ref_barang where f1 != '' and f2 = '0' and f = '0' and g ='00' and h ='00' and i='00' and j = '000'  ","onChange=\"$this->Prefix.refreshList(true)\"",'Pilih','').
			"</td>
			</tr>
			<tr>
			<td style='width:170px;' >KELOMPOK</td><td style='width:10px'>:</td>
			<td>".
			cmbQuery1("cmbKelompok",$cmbKelompok,"select f2 as valueCmbKelompok , nm_barang from ref_barang where f1 = '$cmbAkun' and f2 != '0' and f = '0' and g ='00' and h ='00' and i='00' and j = '000' ","onChange=\"$this->Prefix.refreshList(true)\"",'Pilih','').
			"</td>
			</tr><tr>
			<td style='width:170px;' >JENIS</td><td>:</td>
			<td>".
			cmbQuery1("cmbJenis",$cmbJenis,"select f as valueCmbJenis, nm_barang from ref_barang where f1 = '$cmbAkun' and f2 = '$cmbKelompok' and f != '0'  and g ='00' and h ='00' and i='00' and j = '000'","onChange=\"$this->Prefix.refreshList(true)\"",'Pilih','').
			"</td>
			</tr><tr>
