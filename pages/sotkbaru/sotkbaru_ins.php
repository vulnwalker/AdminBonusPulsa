...
class SOTKBaru_insObj extends DaftarObj2{
	var $Prefix = 'SOTKBaru_ins';
	var $SHOW_CEK = TRUE;	
	...
	//get data -----------------
	 ...
	 $idplh = explode(" ",$idubah);
	 
	 $no_ba= $_REQUEST['no_bast'];
	 $tgl_ba= $_REQUEST['tgl_bast'];

	 if( $err=='' && $c1 =='' ) $err= 'URUSAN Belum Di Isi !!';
	 if( $err=='' && $c2 =='' ) $err= 'BIDANG Belum Di Isi !!';
	 if( $err=='' && $d2 =='' ) $err= 'SKPD Belum Di Isi !!';
	 if( $err=='' && $e2 =='' ) $err= 'UNIT Belum Di Isi !!';
	 if( $err=='' && $e12 =='' ) $err= 'SUB UNIT Belum Di Isi !!';
	 if( $err=='' && $no_ba =='' ) $err= 'Nomor BAST Belum Di Isi !!';
	 //if( $err=='' && $tgl_ba =='' ) $err= 'Tanggal BAST Belum Di Isi !!';
	...
	$qry_bast = "SELECT no_ba,no_ba FROM ref_bast where c='$c' and d='$d' and e='$e' and e1='$e1'";	
		...
		$TampilOpt =
			...
			genFilterBar(
				array(
					"<span id='inputpenerimaanbarang' style='color:black;font-size:14px;font-weight:bold;'/>PILIH SOTK BARU</span>",
			...
			genFilterBar(
					...
					$this->isiform(
						array(
							array(
								...
								'name'=>'fmSKPDUrusan',
								'label-width'=>'200px;',
								'value'=>$this->cmbQueryUrusan('fmSKPDUrusan',$fmSKPDUrusan,'','onchange=SOTKBaru_ins.UrusanAfter() '.$disabled1,'--- Pilih URUSAN ---','00'),
							...
							array(
								...
								'name'=>'fmSKPDBidang2',
								'label-width'=>'200px;',
								'value'=>$this->cmbQueryBidang('fmSKPDBidang2',$fmSKPDBidang2,'','onchange=SOTKBaru_ins.BidangAfter2() '.$disabled1,'--- Pilih BIDANG ---','00'),
							...
							array(
								...
								'name'=>'fmSKPDskpd2',
								'label-width'=>'200px;',
								'value'=>$this->cmbQuerySKPD('fmSKPDskpd2',$fmSKPDskpd2,'','onchange=SOTKBaru_ins.SKPDAfter2() '.$disabled1,'--- Pilih SKPD ---','00'),
							...
							array(
								...
								'name'=>'fmSKPDUnit2',
								'label-width'=>'200px;',
								'value'=>$this->cmbQueryUnit('fmSKPDUnit2',$fmSKPDUnit2,'','onchange=SOTKBaru_ins.UnitAfter2() '.$disabled1,'--- Pilih UNIT ---','00'),
						...
						array(
							array(
								...
								'value'=>"<span id='nomber'>"
										.cmbQuery('no_bast',$no_bast,$qry_bast," style='width:200px;' onchange='SOTKBaru_ins.ambilTglBA()'","--- NOMOR BAST ---")."</span>".
								"<input type='hidden' name='tgl_bast' id='tgl_bast' value='$tgl_bast' /> ".
								"<input type='button' name='BaruBAST' id='BaruBAST' value='BARU' onclick='SOTKBaru_ins.BaruBAST()' />
								<input type='button' name='EditBAST' id='EditBAST' value='EDIT' onclick='SOTKBaru_ins.EditBAST()' />"
				...
				genFilterBar(
					...
					<table>
						<tr>
							<td>".$this->buttonnya('SOTKBaru_ins.Simpan()','save_f2.png','SIMPAN','SIMPAN','SIMPAN')."</td>
		...
		"<html>".
			...
			"<table id='KerangkaHal' class='menubar' cellspacing='0' cellpadding='0' border='0' width='100%' height='100%' >".
				...
				"<tr height='*' valign='top'> <td >".
					...
					"<div class='main' >".
					$form1.
					"<input type='hidden' name='skpd_lama' id='skpd_lama' value='".$_REQUEST['SOTKBaruSkpdfmSKPD']."' />".
					"<input type='hidden' name='unit_lama' id='unit_lama' value='".$_REQUEST['SOTKBaruSkpdfmUNIT']."' />".
					"<input type='hidden' name='subunit_lama' id='subunit_lama' value='".$_REQUEST['SOTKBaruSkpdfmSUBUNIT']."' />".
					"<input type='hidden' name='seksi_lama' id='seksi_lama' value='".$_REQUEST['SOTKBaruSkpdfmSEKSI']."' />".
...
//							$ISI7 = "{$isiKIB_E['jenis']}";
	...
	function set_selector_other($tipe){
		...
		switch($tipe){
			case 'UrusanAfter':{
				$content['c2']= $this->cmbQueryBidang('fmSKPDBidang2',$fmSKPDBidang2,'','onchange=SOTKBaru_ins.refreshList(true)','--- Pilih BIDANG ---','00');
				$content['d2']= $this->cmbQuerySKPD('fmSKPDskpd2',$fmSKPDskpd2,'','onchange=SOTKBaru_ins.BidangAfter2()','--- Pilih SKPD ---','00');
				$content['e2']= $this->cmbQueryUnit('fmSKPDUnit2',$fmSKPDUnit2,'','onchange=SOTKBaru_ins.UnitAfter2() ','--- Pilih UNIT ---','00');
				$content['e12']= $this->cmbQuerySubUnit('fmSKPDSubUnit2',$fmSKPDSubUnit2,'','onchange=SOTKBaru_ins.refreshList(true)','--- Pilih SUB UNIT ---','000');
			break;
			}
			case 'BidangAfter2':{
				$content= $this->cmbQuerySKPD('fmSKPDskpd2',$fmSKPDskpd2,'','onchange=SOTKBaru_ins.BidangAfter2()','--- Pilih SKPD ---','00');
			break;
			}
			...
			case 'SKPDAfter2':{
				...
				$content=$this->cmbQueryUnit('fmSKPDUnit2',$fmSKPDUnit2,'','onchange=SOTKBaru_ins.UnitAfter2() ','--- Pilih UNIT ---','00');
			break;
		    }
...
$SOTKBaru_ins = new SOTKBaru_insObj();
