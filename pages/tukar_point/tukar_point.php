...
class tukar_pointObj  extends DaftarObj2{
	var $Prefix = 'tukar_point';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'tukar_point'; //bonus
	var $TblName_Hapus = 'tukar_point';
	var $MaxFlush = 10;
	...
	var $PageTitle = 'DATA TUKAR POINT';
	var $PageIcon = 'images/masterdata_ico.gif';
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='tukar_point.xls';
	var $namaModulCetak='MASTER DATA';
	var $Cetak_Judul = 'tukar_point';
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'tukar_pointForm';
	var $noModul=9;
	var $TampilFilterColapse = 0; //0
	var $username = "";

	function setTitle(){
		return 'DATA TUKAR POINT';
	}
	...
	$aqry = "UPDATE tukar_point set k='$dk',l='$dl',m='$dm',n='$dn',o='$do',nm_tukar_point='$nama' where concat (k,' ',l,' ',m,' ',n,' ',o)='".$idplh."'";$cek .= $aqry;
	...
	//get data -----------------
	 ...
	 $idplh = $_REQUEST[$this->Prefix.'_idplh'];
		...
		if(empty($namaTukar))$err = "Isi Nama Tukar";
		if(empty($jumlahPoint))$err = "Isi Jumlah Point";
		if(empty($jumlahDapat))$err = "Isi Jumlah Dapat";
		...
		if($fmST == 0){
				...
				if($err == ''){
					...
					$data = array(
								  'nama_tukar' => $namaTukar,
								  'jumlah_dapat' => $jumlahDapat,
								  'jumlah_point' => $jumlahPoint,
								);
					mysql_query(VulnWalkerInsert('tukar_point',$data));
					$cek = VulnWalkerInsert('tukar_point',$data);
				}
			}else{
				$data = array(
								'nama_tukar' => $namaTukar,
				...
				mysql_query(VulnWalkerUpdate('tukar_point',$data,"id='$hubla'"));
	 ...
	 $cek = ''; $err=''; $content=''; $json=TRUE;
	  ...
	  switch($tipe){
		...
		case 'getdata':{
				...
				$n = substr($Id, 6,2);
				$o = substr($Id, 9,2);
				$get = mysql_fetch_array( mysql_query("select *, concat(k,'.',l,'.',m,'.',n,'.',o) as kodetukar_point  from tukar_point where k='$k' AND l='$l' AND m='$m' AND n='$n' AND o='$o'"));


				$content = array('kode_tukar_point' => $get['kodetukar_point'], 'nm_tukar_point' => $get['nm_tukar_point']);
		...
		case 'modul':{
				...
				$idPenggunaAplikasi = $tukar_point_cb[0];
		...
		case 'saveSuspend':{
				...
				if(empty($status)){
					...
					if($status == "suspend"){
						...
						$getTokenTarget = mysql_fetch_array(mysql_query("select * from tukar_point where email ='$email'"));
					...
					mysql_query(VulnWalkerUpdate("tukar_point",$data,"email = '$email'"));
				}
	...
	function setPage_OtherScript(){
		...
		return
			 ...
			 <script type='text/javascript' src='js/tukar_point/tukar_point.js' language='JavaScript' ></script>
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$fm = $this->setForm($tukar_point_cb[0]);
	...
	function setFormEditdata($dt){
	 ...
	 $this->form_height = 150;
	  if ($this->form_fmST==1) {
		$this->form_caption = 'FORM EDIT KODE tukar_point';
	  }
		...
		$queryKAedit=mysql_fetch_array(mysql_query("SELECT k, nm_tukar_point FROM tukar_point WHERE k='$k' and l = '0' and m='0' and n='00' and o='00'")) ;
		$cek.=$queryKAedit;
		$queryKBedit=mysql_fetch_array(mysql_query("SELECT l, nm_tukar_point FROM tukar_point WHERE k='$k' and l='$l' and m= '0' and n='00' and o='00'")) ;
		$queryKCedit=mysql_fetch_array(mysql_query("SELECT m, nm_tukar_point FROM tukar_point WHERE k='$k' and l='$l' and m='$m' and n='00' and o='00'")) ;
		$queryKDedit=mysql_fetch_array(mysql_query("SELECT n, nm_tukar_point FROM tukar_point WHERE k='$k' and l='$l' and m='$m' and n='$n' and o='00'")) ;
		$queryKEedit=mysql_fetch_array(mysql_query("SELECT o, nm_tukar_point FROM tukar_point WHERE k='$k' and l='$l' and m='$m' and n='$n' and o='$o'")) ;
	//	$cek.="SELECT ke, nm_account FROM ref_jurnal WHERE ka='$data_ka' and kb='$data_kb' and kc='$data_kc' and kd='$data_kd' and ke='$data_ke' and kf='0'";
		...
		$datka=$queryKAedit['k'].".  ".$queryKAedit['nm_tukar_point'];
		$datkb=$queryKBedit['l'].". ".$queryKBedit['nm_tukar_point'];
		$datkc=$queryKCedit['m']." .  ".$queryKCedit['nm_tukar_point'];
		$datkd=$queryKDedit['n'].". ".$queryKDedit['nm_tukar_point'];
		$datke=$queryKEedit['o'];
       ...
       //items ----------------------
		  ...
		  'kode_Akun' => array(
			...
			'Kode_Rincian_Objek' => array(
						...
						<input type='text' name='nm_tukar_point' id='nm_tukar_point' value='".$dt['nm_tukar_point']."' size='36px'>
						</div>",
						 ),
	...
	//	$nip	 = '';
		...
		$readonly='readonly';
		$get = mysql_fetch_array(mysql_query("select * from tukar_point where id ='$dt'"));
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			...
			'nama' => array(
						...
						'labelWidth'=>100,
						'value'=>"<div style='float:left;'>
						<input type='text' name='namaTukar' id='namaTukar' value='".$nama_tukar."' placeholder='Nama Tukar' style='width:250px;'>
						</div>",
						 ),
			'jumlahPoint' => array(
						 			'label'=>'JUMLAH POINT',
						 			'labelWidth'=>100,
						 			'value'=>"<div style='float:left;'>
						 			<input type='text' name='jumlahPoint' id='jumlahPoint' value='".$jumlah_point."' placeholder='Jumlah Point' style='width:250px;'>
						 			</div>",
						 			 ),
			'jumlah_dapat' => array(
						 			'label'=>'JUMLAH DAPAT',
						 			'labelWidth'=>100,
						 			'value'=>"<div style='float:left;'>
						 			<input type='text' name='jumlahDapat' id='jumlahDapat' value='".$jumlah_dapat."' placeholder='Jumlah Dapat' style='width:250px;'>
						 			</div>",
						 			 ),
	...
	$this->form_width = 400;
	 $this->form_height = 100;
		...
		$readonly='readonly';
		$get = mysql_fetch_array(mysql_query("select * from tukar_point where email ='$dt'"));
		foreach ($get as $key => $value) {
			  $$key = $value;
			}
	...
	<A href=\"pages.php?Pg=tukar_point\" title='tukar_point' style='color:blue'  >tukar_point</a> |
	...
	function setKolomHeader($Mode=1, $Checkbox=''){
		...
		$headerTable =
			"<thead>
				...
				<th class='th01' >NAMA TUKAR </th>
				<th class='th01' >JUMLAH POINT</th>
				<th class='th01' >JUMLAH DAPAT </th>
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
		...
		$Koloms[] = array('', $isi['nama_tukar']);

		$Koloms[] = array('align = "right"', number_format($isi['jumlah_point'],0,',','.'));
		$Koloms[] = array('align = "right"', number_format($isi['jumlah_dapat'],0,',','.'));
	...
	$fmDESC1 = cekPOST('fmDESC1');
	 $arrOrder = array(
		 array('orderNama','Nama Tukar'),
		array('orderPoint','Jumlah Point'),
		array('orderDapat','Jumlah Dapat'),
					);
	$arr = array(
			//array('selectAll','Semua'),
			array('findNama','Nama Tukar'),
			array('findPoint','Jumlah Point'),
			array('findDapat','Jumlah Dapat'),
			);
	...
	function getDaftarOpsi($Mode=1){
		...
		switch($fmPILCARI){
			...
			case 'findNama': $arrKondisi[] = " nama_tukar like '%$fmPILCARIvalue%'"; break;
			case 'findPoint': $arrKondisi[] = " jumlah_point like '%$fmPILCARIvalue%'"; break;
			case 'findDapat': $arrKondisi[] = " jumlah_dapat like '%$fmPILCARIvalue%'"; break;
		...
		// }
		if(!empty($fmORDER1)){
			if($fmORDER1 == "orderNama"){
					$arrOrders[] = " nama_tukar $Asc1 ";
			}elseif($fmORDER1 == "orderPoint"){
					$arrOrders[] = " jumlah_point $Asc1 ";
			}elseif($fmORDER1 == "orderDapat"){
					$arrOrders[] = " jumlah_dapat $Asc1 ";
			}
...
$tukar_point = new tukar_pointObj();
$tukar_point->username = $_COOKIE['coID'];
?>
