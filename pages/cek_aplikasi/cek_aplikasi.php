...
class cek_aplikasiObj  extends DaftarObj2{	
	var $Prefix = 'cek_aplikasi';
	var $elCurrPage="HalDefault";
	var $SHOW_CEK = TRUE;
	var $TblName = 'tabel_cek_aplikasi'; //bonus
	var $TblName_Hapus = 'tabel_cek_aplikasi';
	var $MaxFlush = 10;
	...
	var $PageTitle = 'CEK APLIKASI';
	var $PageIcon = 'images/masterdata_ico.gif';
	var $pagePerHal ='';
	//var $cetak_xls=TRUE ;
	var $fileNameExcel='cek_aplikasi.xls';
	var $namaModulCetak='MASTER DATA';
	var $Cetak_Judul = 'pengguna_aplikasi';	
	var $Cetak_Mode=2;
	var $Cetak_WIDTH = '30cm';
	var $Cetak_OtherHTMLHead;
	var $FormName = 'cek_aplikasiForm';
	var $noModul=9; 
	...
	$aqry = "UPDATE cek_aplikasi set k='$dk',l='$dl',m='$dm',n='$dn',o='$do',nm_pengguna_aplikasi='$nama' where concat (k,' ',l,' ',m,' ',n,' ',o)='".$idplh."'";$cek .= $aqry;
		...
		if(empty($cmbModulForm)){
			...
			if($fmST == 0){
				if($err == ''){
							...
							mysql_query(VulnWalkerInsert("tabel_cek_aplikasi",$data));
							$cek = VulnWalkerInsert("tabel_cek_aplikasi",$data);
							...
							$data = array( 
									...
									mysql_query(VulnWalkerUpdate('tabel_cek_aplikasi',$data,"id = '$hubla'"));
	...
	function tabelItem(){
		...
		$aksi = '<a href="javascript:cek_aplikasi.newrincian_pekerjaan()" id="pengguna_aplikasiAtasButton"><img id="gambarAtasButton" src="datepicker/add-256.png" style="width:20px;height:20px;"></a>';
	...
	function tabelHistori($id){
		        ...
		        $query = mysql_query("select * from histori_cek_aplikasi where id_cek = '$id' order by id");
				$no = 1 ;
	  ...
	  switch($tipe){
		...
		case 'cekOK':{
				foreach ($_REQUEST as $key => $value) { 
					...
					mysql_query(VulnWalkerUpdate('tabel_cek_aplikasi',$data,"id = '$id'"));
					$dataHistory = array(
								 'status_cek' => "OK",
								 'tanggal_cek' => date("Y-m-d"),
								 'id_cek' => $id,
					...
					mysql_query(VulnWalkerInsert("histori_cek_aplikasi",$dataHistory));
		...
		case 'getdata':{
				...
				$n = substr($Id, 6,2);
				$o = substr($Id, 9,2);
				$get = mysql_fetch_array( mysql_query("select *, concat(k,'.',l,'.',m,'.',n,'.',o) as kodepengguna_aplikasi  from cek_aplikasi where k='$k' AND l='$l' AND m='$m' AND n='$n' AND o='$o'"));
		...
		case 'saveProgres':{
				...
				if(empty($progres)){
					...
					mysql_query(VulnWalkerUpdate("cek_aplikasi",$data, "id = '$hubla'"));
...
inner join ref_aplikasi on pengguna_aplikasi.id_aplikasi = ref_aplikasi.kode_aplikasi where pengguna_aplikasi.id_pemda = '$idPemda' and ref_aplikasi.kode_modul = '0'";
					$cmbAplikasi = cmbQuery('cmbAplikasi','',$querycmbAplikasi,"' onclick =$this->Prefix.cek_aplikasiChanged(); ",'-- Pilih cek_aplikasi --');
		...
		case 'simpancek_aplikasi':{
				foreach ($_REQUEST as $key => $value) { 
				  $$key = $value; 
				}
				if(mysql_num_rows(mysql_query("select * from ref_aplikasi where nama_pemda = '$namacek_aplikasi'")) > 0){
					$err = "cek_aplikasi Sudah Ada";
				}else{
					$data = array('nama_aplikasi' => $namacek_aplikasi);
					mysql_query(VulnWalkerInsert('ref_aplikasi',$data));
					$idnya = mysql_fetch_array(mysql_query("select * from ref_aplikasi where nama_aplikasi = '$namacek_aplikasi'"));
					$content = array('replacer' => cmbQuery('cmbPemda',$idnya['id'],"select id,nama_aplikasi from ref_aplikasi",'style="width:500;"','-- Pilih cek_aplikasi --') );
				}
		break;
		}
		...
		case 'editcek_aplikasi':{
				...
				$idnya = mysql_fetch_array(mysql_query("select * from ref_aplikasi where nama_aplikasi = '$namacek_aplikasi'"));
				$content = array('replacer' => cmbQuery('cmbAplikasi',$idnya['id'],"select id,nama_aplikasi from ref_aplikasi",'style="width:500;"','-- Pilih cek_aplikasi --') );
		break;
		}
		...
		case 'formBarucek_aplikasi':{			
				$idcek_aplikasi = $_REQUEST['idcek_aplikasi'];
				$fm = $this->setFormBarucek_aplikasi($idcek_aplikasi);				
				$cek = $fm['cek'];
				$err = $fm['err'];
				$content = $fm['content'];												
			break;
	 ...
	 function setFormBarucek_aplikasi($idcek_aplikasi){
		
		$this->form_fmST = 0;
		
		$fm = $this->Barucek_aplikasi($idcek_aplikasi);
		return	array ('cek'=>$fm['cek'], 'err'=>$fm['err'], 'content'=>$fm['content']);
	}
	...
	function Barucek_aplikasi($dt){	
	 ...
	 if(!empty($dt)){
	 	$this->form_caption = 'Edit cek_aplikasi';
		$kemana = "Editcek_aplikasi($dt)";
		$namacek_aplikasi = mysql_fetch_array(mysql_query("select * from ref_aplikasi where id='$dt'"));
		$namacek_aplikasi = $namacek_aplikasi['nama_aplikasi'];
	 }else{
	 	if ($this->form_fmST==0) {
		$this->form_caption = 'Baru cek_aplikasi';
		$nip	 = '';

			
		$kemana = 'Simpancek_aplikasi()';
	 ...
	 //items ----------------------
	  $this->form_fields = array(
			...
			'Kelompok' => array( 
						'label'=>'Nama cek_aplikasi',
						'labelWidth'=>100, 
						'value'=>"<div style='float:left;'>
						<input type='text' name='namacek_aplikasi' id='namacek_aplikasi' value='$namacek_aplikasi' style='width:255px;' >
	...
	function setPage_OtherScript(){
		...
		return 
			 ...
			 <script type='text/javascript' src='js/cek_aplikasi/cek_aplikasi.js' language='JavaScript' ></script>
	...
	function setFormBaru(){
  	...
  	function setFormEdit(){
		...
		$fm = $this->setForm($cek_aplikasi_cb[0]);
	...
	function setFormEditdata($dt){	
	 ...
	 $this->form_height = 150;
	  if ($this->form_fmST==1) {
		...
		$queryKAedit=mysql_fetch_array(mysql_query("SELECT k, nm_pengguna_aplikasi FROM cek_aplikasi WHERE k='$k' and l = '0' and m='0' and n='00' and o='00'")) ;
		$cek.=$queryKAedit;
		$queryKBedit=mysql_fetch_array(mysql_query("SELECT l, nm_pengguna_aplikasi FROM cek_aplikasi WHERE k='$k' and l='$l' and m= '0' and n='00' and o='00'")) ;
		$queryKCedit=mysql_fetch_array(mysql_query("SELECT m, nm_pengguna_aplikasi FROM cek_aplikasi WHERE k='$k' and l='$l' and m='$m' and n='00' and o='00'")) ;
		$queryKDedit=mysql_fetch_array(mysql_query("SELECT n, nm_pengguna_aplikasi FROM cek_aplikasi WHERE k='$k' and l='$l' and m='$m' and n='$n' and o='00'")) ;
		$queryKEedit=mysql_fetch_array(mysql_query("SELECT o, nm_pengguna_aplikasi FROM cek_aplikasi WHERE k='$k' and l='$l' and m='$m' and n='$n' and o='$o'")) ;
	...
	function setForm($dt){	
	 ...
	 foreach ($_REQUEST as $key => $value) { 
	  ...
	  if ($this->form_fmST==0) {
		...
		$readonly='readonly';
		$get = mysql_fetch_array(mysql_query("select * from tabel_cek_aplikasi where id ='$dt'"));
...
function Check($dt){	
	 ...
	 $got = mysql_fetch_array(mysql_query("select * from tabel_cek_aplikasi where id='$dt'"));
	 foreach ($got as $key => $value) { 
			  $$key = $value; 
			}
	 $arrayInstall = array(
	 ...
	 $headerTable =
	  "<thead>
	   <tr>
  	   ...
  	   $Checkbox		
		   <th class='th01' width='300' >MODUL / KATEGORI</th>
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 if($concatModulKategori == $this->concatModulKategori){
	 	...
	 	$nama_modul = "<b>".$nama_modul['nama_aplikasi']."</b><br>";
	 }
	 ...
	 $Koloms[] = array('align="left"',$nama_modul."<span style='margin-left:15px;'>".$getNamaKategori['nama']."</span>");
	 ...
	 $grabKeteranganHistori = mysql_fetch_array(mysql_query("select max(id) from histori_cek_aplikasi where id_cek = '$id'"));
	 $getKeteranganHistori = mysql_fetch_array(mysql_query("select * from histori_cek_aplikasi where id = '".$grabKeteranganHistori['max(id)']."'"));
	...
	function getDaftarOpsi($Mode=1){
		...
		$arrOrders[] = "concat(right((100 +id_aplikasi),2),'.',right((100 +id_modul),2),'.',right((100 +id_kategori),2),'.',right((100000 +id),5))";
...
$cek_aplikasi = new cek_aplikasiObj();
$cek_aplikasi->username = $_COOKIE['coID'];
?>