...
class rincianPekerjaanObj  extends DaftarObj2{	
	var $Prefix = 'rincianPekerjaan';
...
function setKolomHeader($Mode=1, $Checkbox=''){
	 ...
	 $headerTable =
	  "<thead>
	   <tr>
  	   ...
  	   <th class='th01' width='400'>MODUL</th>	
	...
	function setKolomData($no, $isi, $Mode, $TampilCheckBox){
	 ...
	 $aksi  = "<img src='images/administrator/images/edit_f2.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=po.edit('$id');></img> &nbsp  &nbsp  <img src='images/administrator/images/invalid.png' width='20px' heigh='20px' style='cursor : pointer;' onclick=$this->Prefix.hapus('$id');></img> ";
...
$baris = $_REQUEST['baris'];
	...
	/*$TampilOpt =
			...
			<td class='border:none' > 
				<a class='toolbar' id='$id' href='javascript:po.addNew()'  title='Baru Item' $aparams> 					
			...
			<td class='border:none'>
				<a class='toolbar' id='$id' href='javascript:rincianPekerjaan.Laporan()'  title='laporan' $aparams> 					
		...
		/*$TampilOpt ="<div class='FilterBar' style='margin-top:5px;'>".
			...
			<tr>
			<td align='right'> </td>
			<td align='right'> <input type='hidden' name ='idParent' id = 'idParent' value='' >".genPanelIcon("javascript:po.addNew()","sections.png","Baru Item", 'Baru Item')."</td>
			 </tr>
			</table>".
			"</div>";
	*/
	...
	function Laporan($xls =FALSE){
		...
		$grabpo = mysql_fetch_array(mysql_query("select * from cek_aplikasi where id = '".$_COOKIE['coParentCek']."'"));
		$namaAplikasi =mysql_fetch_array(mysql_query("select * from ref_aplikasi where id='".$grabpo['id_aplikasi']."'"));		
		$namaAplikasi = $namaAplikasi['nama_aplikasi'];
		$namaModul = mysql_fetch_array(mysql_query("select * from ref_modul where id = '".$grabpo['id_modul']."'"));
		$namaModul = $namaModul['nama_modul'];	
		$namaPegawai = mysql_fetch_array(mysql_query("select * from ref_pegawai where Id ='".$grabpo['dari']."'"));
		$namaPegawai = $namaPegawai['nama'];
		$status = $grabpo['status'];
			...
			"<body >
				...
				<table width=\"100%\" border=\"0\" class='subjudulcetak'>
					...
					<tr>
						...
						<td width='1%' valign='top'> : </td>
						<td valign='top'>".VulnWalkerTitiMangsa($grabpo['tanggal'])."</td>
					</tr>
					<tr>
...
$rincianPekerjaan = new rincianPekerjaanObj();
$rincianPekerjaan->username = $_COOKIE['coID'];
?>