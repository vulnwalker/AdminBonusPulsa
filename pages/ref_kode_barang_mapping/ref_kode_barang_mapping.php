...
//	$nama= $_REQUEST['nm'];
	 ...
	 $headerTable =
	  "<thead>
	   <tr>
  	   ...
  	   $Checkbox		
	   		<th class='th01' width='450'colspan=3>Kode Barang Lama</th>
	...
	//	$brg_baru=mysql_fetch_array(mysql_query("select * from ref_barang_baru"));
		/*$brg_baru="select * from ref_barang_baru";
		$brg_baru2=mysql_fetch_array(mysql_query("select nm_barang * from ref_barang_baru where f1='".$brg_baru['f1']."'"));
		*/
		$baru="select * from ref_barang_baru where f='".$isi['f']."' and g!='".$isi['g']."' and h!='00' and i!='00' and j!='000'";
		$baru2=mysql_fetch_array(mysql_query($baru));
	 		...
	 		$Koloms[] = array('align="center"', $no.'.' );
	 	if ($Mode == 1) $Koloms[] = array(" align='center'  ", $TampilCheckBox);
	 		$Koloms[] = array('align="left" width="10px"',$isi['f_lama'].'.'.$isi['g_lama'].'.'.$isi['h_lama'].'.'.$isi['i_lama'].'.'.$isi['j_lama']);
	 		$Koloms[] = array('align="left" width="250px"',"<span $margin>".$isi['nm_barang_lama']."</span>");
	 		$Koloms[] = array('align="left" width="250px"',$isi['mapping_lama']);
			$Koloms[] = array('align="left" width="50px"',$baru2['f1'].'.'.$baru2['f2'].'.'.$baru2['f'].'.'.$baru2['g'].'.'.$baru2['h'].'.'.$baru2['h'].'.'.$baru2['j']);
		//	$Koloms[] = array('align="left" width="50px"',$baru2['f1']);
	 		$Koloms[] = array('align="left" width="250px"',"<span $margin>".$baru2['nm_barang']."</span>");
	...
	function getDaftarOpsi($Mode=1){
		...
		if(empty($fmGolongan1) && empty($fmBidang1) && empty($fmKelompok1))// && empty($fmMapping))
		{
		}
		elseif(!empty($fmGolongan1) && empty($fmBidang1) && empty($fmKelompok1) && empty($fmMapping))
		{
			$arrKondisi[]= "f_lama =$fmGolongan1 and g_lama!='00' and h_lama!='00' and i_lama='00' and j_lama='000'";
		}
		...
		$OrderDefault = 'order by mapping_lama ';// Order By no_terima desc ';
