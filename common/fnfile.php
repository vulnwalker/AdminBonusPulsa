...
function genPanelIcon1($Link="",$Image="save2.png",$Isi="Isinya",$hint='',$id="",$ReadOnly="",$Disabled=FALSE,$Rid="",$aparams='') {
	global $Pg; $RidONLY = "" ;
	global $PATH_IMG;
...
function VulnWalkerTitiMangsa($date) {
...
function CmbUrusanBidangSkpd($prefix='', $tblwidth='100%', $kol1_width=100,$mode='') {
   //global $DisAbled;
    global $fmWIL, $fmSKPDBidang, $fmSKPDskpd, $fmSKPDUrusan, $fmSUBUNIT,$fmSEKSI, $fmTAHUNANGGARAN, $fmKEPEMILIKAN, $Main, $HTTP_COOKIE_VARS, $Pg, $SPg;
	...
	if ($HTTP_COOKIE_VARS["coSKPD"] !== "00") {
        $fmSKPDBidang = $HTTP_COOKIE_VARS["coSKPD"];
        $HTTP_COOKIE_VARS["cofmSKPDBidang"] = $fmSKPDBidang;
        $KondisiSKPD = " and c='$fmSKPDBidang'";
        $PilihSKPD = "";
    }else{
		// $KondisiSKPD = " and d='00' ";
		if (isset($HTTP_COOKIE_VARS['cofmSKPDBidang'])) {
	       $fmSKPDBidang = $HTTP_COOKIE_VARS['cofmSKPDBidang'];
	    }
		if(!($fmSKPDBidang == '00' || $fmSKPDBidang=='')){
			//$KondisiSKPD = " and c='$fmSKPDBidang'";//$PilihSKPD = "";
		}

	}
	if ($HTTP_COOKIE_VARS["coUNIT"] !== "00") {
        $fmSKPDskpd = $HTTP_COOKIE_VARS["coUNIT"];
        $HTTP_COOKIE_VARS["cofmSKPDskpd"] = $fmSKPDskpd;
        $KondisiUNIT = " and d='$fmSKPDskpd'";
        $PilihUNIT = "";
    }else{
		if (isset($HTTP_COOKIE_VARS['cofmSKPDskpd'])) {
	       $fmSKPDskpd = $HTTP_COOKIE_VARS['cofmSKPDskpd'];
	    }
		if(!($fmSKPDskpd == '00' || $fmSKPDskpd=='')){
			//$KondisiUNIT = " and d='$fmSKPDskpd'";//$PilihUNIT = "";
		}
    ...
    $aqry = "select * from ref_skpd where d='00' and c ='00' order by c1";
    $Qry = mysql_query($aqry);
    $Ops = "";
    while ($isi = mysql_fetch_array($Qry)) {
        $sel = $fmSKPDUrusan == $isi['c1'] ? "selected" : "";
        $Ops .= "<option $sel value='{$isi['c1']}'>{$isi['c1']}. {$isi['nm_skpd']}</option>\n";
		if ($fmSKPDUrusan == $isi['c1']) $fmSKPDBidangUrusan=$isi['nm_skpd'];
    }
	$ListUrusan =
		//$cekskpd . 'pref='.$prefix.
		"<div id='".$prefix."CbxUrusan'>
			<select $disSKPD name='fmSKPDUrusan' id='fmSKPDUrusan'
				onChange=\"".$prefix.".pilihBidang();\"
			> $PilihUrusan $Ops</select><div style='display:none'></div></div>";


/*	if (isset($fmSKPDUrusan) && $fmSKPDUrusan != '' && $fmSKPDUrusan != '00' ){
		$KondisiSKPD = $KondisiSKPD." and c1 = '$fmSKPDUrusan'";
	}*/


    $aqry = "select * from ref_skpd where 1=1 and d='00' and c !='00' and c1='$fmSKPDUrusan' order by c";
    $Qry = mysql_query($aqry);
    $Ops = "";
    while ($isi = mysql_fetch_array($Qry)) {
        $sel = $fmSKPDBidang == $isi['c'] ? "selected" : "";
        $Ops .= "<option $sel value='{$isi['c']}'>{$isi['c']}. {$isi['nm_skpd']}</option>\n";
		if ($fmSKPDBidang == $isi['c']) $fmSKPDBidangBidang=$isi['nm_skpd'];
    }
	$ListSKPD =
		//$cekskpd . 'pref='.$prefix.
		"<div id='".$prefix."CbxBidang'>
			<select $disSKPD name='fmSKPDBidang' id='fmSKPDBidang'
				onChange=\"".$prefix.".BidangAfter2()\"
			> $PilihSKPD $Ops</select><div style='display:none'></div></div>";

	//unit ------------------------
//		$KondisiUNIT='';
    $Qry = mysql_query("select * from ref_skpd where c='$fmSKPDBidang' and d <> '00' and e = '00' and c1='$fmSKPDUrusan' order by d");
    $Ops = "";
    while ($isi = mysql_fetch_array($Qry)) {
        $sel = $fmSKPDskpd == $isi['d'] ? "selected" : "";
        $Ops .= "<option $sel value='{$isi['d']}'>{$isi['d']}. {$isi['nm_skpd']}</option>\n";
		if ($fmSKPDskpd == $isi['d']) $fmSKPDBidangskpd=$isi['nm_skpd'];

    }
    $ListUNIT =
		"<div id='".$prefix."CbxUnit'>
			<select $disUNIT name='fmSKPDskpd' id='fmSKPDskpd'
				onChange=\"".$prefix.".comboSKPDChanged()\">
				$PilihUNIT $Ops
			</select></div>";
    ...
    $Hsl = "
		...
		<div style='float: left; width: 90%; height: auto; padding: 4px;'>
			<table width=\"100%\"   >
				...
				<input type=text > name='fmSKPDBidangBidang' id='fmSKPDBidangBidang' value='$fmSKPDBidangBidang' >
				<input type=hidden > name='fmSKPDBidang' id='fmSKPDBidang' value='$fmSKPDBidang' >
				</td> </tr>
				<tr valign=\"top\"> <td>SKPD</td> <td>:</td> <td>
				<input type=text > name='fmSKPDBidangskpd' id='fmSKPDBidangskpd' value='$fmSKPDBidangskpd' >
				<input type=hidden > name='fmSKPDskpd' id='fmSKPDskpd' value='$fmSKPDskpd' >
				</td> </tr>
			</table>
		</div>
	";
    ...
    $Hsl = "
		...
		<div style='float: left; width: 90%; height: auto; padding: 4px;'>
			<table width=\"100%\"   >
				<tr valign=\"top\"> <td width='$kol1_width'>URUSAN</td> <td width='10'>:</td> <td>$ListUrusan</td> </tr>
...
function WilSKPD_ajxVW($prefix='', $tblwidth='100%', $kol1_width=100,$mode='') {
    global $fmWIL, $fmUrusan,$fmSKPD, $fmUNIT, $fmSUBUNIT,$fmSEKSI, $fmTAHUNANGGARAN, $fmKEPEMILIKAN, $Main, $HTTP_COOKIE_VARS, $Pg, $SPg;
    $disSKPD = $DisAbled;
    $disUNIT = $DisAbled;
    $disSUBUNIT = $DisAbled;
    $disSEKSI = $DisAbled;
	...
	$ListUrusan =
		"<div id='".$prefix."CbxUrusan'>
			<select $disSKPD name='".$prefix."fmUrusan' id='".$prefix."fmUrusan'
				onChange=\"".$prefix.".pilihBidang()\"
			> $PilihUrusan $Ops</select><div style='display:none'></div></div>";



    $aqry = "select * from ref_skpd where 1=1 and c!= '00' and d='00' and c1='$fmUrusan' order by c";
    $Qry = mysql_query($aqry);
    $Ops = "";
    while ($isi = mysql_fetch_array($Qry)) {
    ...
    $Qry = mysql_query("select * from ref_skpd where c='$fmSKPD' and d <> '00' and e = '00' and c1='$fmUrusan' $KondisiUNIT order by d");
    $Ops = "";
    while ($isi = mysql_fetch_array($Qry)) {
    ...
    $Hsl = "
		...
		<div style='float: left; width: 90%; height: auto; padding: 4px;'>
			<table width=\"100%\"   >
				<tr valign=\"top\"> <td width='$kol1_width'>URUSAN</td> <td width='10'>:</td> <td>
				<input type=text > name='".$prefix."nmUrusan' id='".$prefix."nmUrusan' value='$nmUrusan' >
...
function cmb2Dv2($name='txtField', $value='', $arrList = '', $param='',$status) {
    global $Ref;
    $isi = $value;
	$nametxt=$name.'txt';
    $Input = "<option value=''>Pilih</option>";
    ...
    $Input = "<select $param name='$name'  id='$name'>$Input</select>";
	if ($status==TRUE){
		$Input="<input type=text name='$nametxt'  id='$nametxt' value='$ur' readonly><input type='hidden' name='$name'  id='$name' value='$ux' > ";
	}
    return $Input;
}
...
function cariInfo3($formNYA, $URL, $URLC, $ID="", $NM="", $ReadOnly="", $DisAbled="", $Param="", $paramURL='') {
    global $Act, $$ID, $$NM;
    $VALID = $$ID;
    $VALNM = $$NM;
    $NMIFRAME = "iframe$ID";
	...
	<iframe onblur=\"document.all.cari$NMIFRAME.style.visibility='hidden'\" id='$NMIFRAME'
		style='position:absolute;visibility:hidden' name='$NMIFRAME'
		src='$URL?objID=$ID&objNM=$NM&Cari2=$VALID$paramURL'>
	</iframe>
	<iframe id='cari$NMIFRAME' style=';position:absolute;visibility:hidden'
		name='cari$NMIFRAME' src='$URLC?objID=$ID&objNM=$NM&kdBrgOld=$VALID&Act=$Act'>
	</iframe>
...
function WilSKPD1($kol1_width=100,$mode='',$readonly='') {
	...
	//SEKSI ------------------------------------------------
	$aqry = "select * from ref_skpd where c='$fmSKPD' and d = '$fmUNIT' and e='$fmSUBUNIT' and e1<> '$kdSubUnit0' $KondisiSEKSI order by nm_skpd";
    $Qry = mysql_query($aqry);
    $Ops = "";
    while ($isi = mysql_fetch_array($Qry)) {
        $sel = $fmSEKSI == $isi['e1'] ? "selected" : "";
...
*/
	if ($mode=='1'){
		if ($Main->MODE_EDIT_SKPD==1){
			if($readonly==""){
				$Hsl = "
					<div style='float: left; width: 100%;  padding: 4px;'>
						<table width=\"100%\"   >

							<tr valign=\"top\"> <td width='$kol1_width'>BIDANG</td>
							<td width='10'>:</td>
							<td>
							<input type=text  name='nmSKPD' id='nmSKPD' value='$nmskpd' readonly size='80' >
							<input type=hidden  name='fmSKPD' id='fmSKPD' value='$fmSKPD' >

							</td> </tr>
							<tr valign=\"top\"> <td>SKPD</td> <td>:</td> <td>
							<input type=text  name='nmUNIT' id='nmUNIT' value='$nmunit'  readonly size='80' >
				...
				$Hsl = "
					<div style='float: left; width: 100%;  padding: 4px;'>
						<table width=\"100%\"   >
							...
							<tr valign=\"top\"> <td>UNIT</td> <td>:</td> <td>
							<input type=text  name='nmSUBUNIT' id='nmSUBUNIT' value='$nmsubunit'  readonly size='80'>
							<input type=hidden  name='fmSUBUNIT' id='fmSUBUNIT' value='$fmSUBUNIT' >

							</td> </tr>
							<tr valign=\"top\"> <td>SUBUNIT</td> <td>:</td> <td>
							<input type=text  name='nmSEKSI' id='nmSEKSI' value='$nmseksi'  readonly size='80' >
...
function WilSKPD_ajx3($prefix='', $tblwidth='100%', $kol1_width=100,$mode='') {
    //global $DisAbled;
    global $fmWIL, $fmSKPD, $fmUNIT, $fmSUBUNIT,$fmSEKSI, $fmTAHUNANGGARAN, $fmKEPEMILIKAN, $Main, $HTTP_COOKIE_VARS, $Pg, $SPg;
    //$disSKPD = ""; $disUNIT = ""; $disSUBUNIT = "";
    //echo "<br>Group=".login_getGroup();
...
*/
   ...
   // $KondisiSEKSI = "";
    ...
    $aqry = "select * from ref_skpd where 1=1 $KondisiSKPD and d='00' order by c";
    $Qry = mysql_query($aqry);
    $Ops = "";
    while ($isi = mysql_fetch_array($Qry)) {
        $sel = $fmSKPD == $isi['c'] ? "selected" : "";
...
function ComboBidangDanSKPD($prefix, $tblwidth='100%', $kol1_width=100,$mode='') {
    //global $DisAbled;
    global $fmWIL, $fmSKPDBidang, $fmSKPDskpd, $fmSUBUNIT,$fmSEKSI, $fmTAHUNANGGARAN, $fmKEPEMILIKAN, $Main, $HTTP_COOKIE_VARS, $Pg, $SPg;
    //$disSKPD = ""; $disUNIT = ""; $disSUBUNIT = "";
    //echo "<br>Group=".login_getGroup();
...
/*	$fmSKPDBidang = $_REQUEST[$prefix.'fmSKPDBidang'];
	$fmSKPDskpd = $_REQUEST[$prefix.'fmSKPDskpd'];*/
	$fmSKPDBidang = $_REQUEST['fmSKPDBidang'];
	$fmSKPDskpd = $_REQUEST['fmSKPDskpd'];
...
*/
   ...
   // $KondisiSEKSI = "";
    ...
    //$aqry = "select * from ref_skpd where d='00'   order by c";
    $aqry = "select * from ref_skpd where 1=1 $KondisiSKPD and d='00' and c != '00' order by c";
    $Qry = mysql_query($aqry);
    $Ops = "";
    while ($isi = mysql_fetch_array($Qry)) {
        $sel = $fmSKPDBidang == $isi['c'] ? "selected" : "";
	...
	//unit ------------------------
//		$KondisiUNIT='';
    $Qry = mysql_query("select * from ref_skpd where c='$fmSKPDBidang' and d <> '00' and e = '00' $KondisiUNIT order by d");
    $Ops = "";
    while ($isi = mysql_fetch_array($Qry)) {
        $sel = $fmSKPDskpd == $isi['d'] ? "selected" : "";
        $Ops .= "<option $sel value='{$isi['d']}'>{$isi['d']}. {$isi['nm_skpd']}</option>\n";
...
function WilSKPD1b( $fmSKPD, $fmUNIT, $fmSUBUNIT, $kol1_width=100, $fmSEKSI) {
    ...
    $Hsl = "
		<div style='float: left; width: 100%; height: 90px; padding: 4px;'>
			<table width=\"100%\"   >
				<tr valign=\"top\"> <td width='$kol1_width'>BIDANG</td>   <td width='10'>:</td>   <td>$ListSKPD</td> </tr>
				<tr valign=\"top\"> <td>SKPD</td> <td>:</td> <td>$ListUNIT</td> </tr>
				<tr valign=\"top\"> <td>UNIT</td> <td>:</td> <td>$ListSUBUNIT</td> </tr>
...
function PrintSKPD2($fmSKPD, $fmUNIT, $fmSUBUNIT,$fmSEKSI,$xls=FALSE) {
	...
	{
    $Hsl = "<table cellpadding=0 cellspacing=0 border=0 width='100%'>
			<tr valign=top> <td  colspan=2 >PROVINSI</td><td align=center >:</td><td style='font-weight:bold;font-size:10pt' >$PROVINSI</td> </tr>
			<tr valign=top> <td  colspan=2 >KABUPATEN/KOTA</td><td align=center >:</td><td style='font-weight:bold;font-size:10pt' > $WILAYAH</td> </tr>

			<tr valign=top> <td  colspan=2 >BIDANG</td><td align=center >:</td><td style='font-weight:bold;font-size:10pt' > $SKPD</td> </tr>
			<tr valign=top> <td  colspan=2 >SKPD</td><td align=center >:</td><td style='font-weight:bold;font-size:10pt' > $UNIT</td> </tr>
			<tr valign=top> <td  colspan=2 >UNIT</td><td align=center >:</td><td style='font-weight:bold;font-size:10pt' > $SUBUNIT</td> </tr>
			<tr valign=top> <td  colspan=2 >SUB UNIT</td><td align=center >:</td><td style='font-weight:bold;font-size:10pt' > $SEKSI</td> </tr>
			</table>";
...
function PrintTTD($pagewidth = '30cm', $xls=FALSE, $cp1='', $cp2='', $cp3='', $cp4='', $cp5='' ) {
    ...
    $NAMASKPD = "";
    $JABATANSKPD = "";
    $TITIMANGSA = "Demo, " . JuyTgl1(date("Y-m-d"));
    if (c == '04') {
...
function PrintTTD_($pagewidth = '30cm',$xls=FALSE) {
    global $fmWIL, $fmSKPD, $fmUNIT, $fmSUBUNIT, $fmTAHUNANGGARAN, $fmKEPEMILIKAN, $Main, $HTTP_COOKIE_VARS, $NAMASKPD, $JABATANSKPD, $NIPSKPD, $NAMASKPD1, $JABATANSKPD1, $NIPSKPD1, $TITIMANGSA;
...
function inputFormatRibuan3($obj="obj", $params="", $value=0, $ket=' pemisah pecahan dengan titik (mis: 1.5)',$obj2='obj2',$objsum='objsum') {
    global $$obj; //$$obj = round($$obj,0);
    //onKeyPress=\"return isNumberKey(event); \"
	//$str = " <input type=\"text\" name=\"$obj\" id=\"$obj\" value=\"" . $value . "\"
	if ($value == 0){
	...
	function HitungPerolehan(){
	var hasil=0;

	var nilai1 = parseFloat(document.getElementById('fmHARGABARANGBELI').value);
	var nilai2 = parseFloat(document.getElementById('fmHARGABARANGATRIBUSI').value);
	...
	document.getElementById('fmHARGABARANG').value=hasil;


}
	</script>
	<input type=\"text\" name=\"$obj\" id=\"$obj\" value=\"" . $value . "\"
			$params
			onKeyDown = \"oldValue=this.value;\"
			 onKeyPress=\"return isNumberKey(event); \"
			 onkeyup=\"TampilUang('Tampil$obj',this.value); HitungPerolehan();\"
			...
			&nbsp;&nbsp
				<span style='color:red'>$ket</span>";
			//	document.getElemenById('$objsum').value=document.getElemenById('$obj').value+document.getElemenById('$obj2').value;
    return $str;
}
	...
	)
{
	...
	switch ($Mode){
		...
		case 2 :{//tahun tanpa combo dan readonly
			$entry_thn =
				'<input ' . $dis . ' type="text"
					...
					value="' . $tgl[0] . '" size="1" maxlength="4"
					onkeypress="return isNumberKey(event)"
					onchange="TglEntry_createtgl(\'' . $elName . '\')" readonly
				>';
			break;
		}
		default :{ //tahun combo
	...
	$tglkosong1=0, $tglkosong2=0)
{
	...
	return
		//"<table><tr><td>".
		"<div style='float:left;height:22;width:500'>".
...
function setLastAktif() {
    ...
    if (CekLogin ()) {
        ...
        mysql_query("update admin set lastaktif= now(), ipaddr='" . $_SERVER['REMOTE_ADDR'] . "' where uid='{$UID}'");
...
function getList_RekapByOPD2($SPg, $noawal, $LimitHal, $kolomwidth, $isCetak, $isRibuan=TRUE, $tgl='2012-1-1' ) {
    ...
    if ($SPg != '10') {
        ...
        $sqry7 = RekapByOPD_GetQuery($tgl, ' and f="07" and g="24" '); //kib f
        $qry7 = mysql_query($sqry7 . ' ' . $LimitHal);
    }
    $ListData = '';
    $no = $noawal + 1;
    if ($isCetak) {
    ...
    while ($isi = mysql_fetch_array($qry)) {
        ...
        if ($SPg == '10') {
            ...
            if (!mysql_data_seek($qry7, $i)) {
                echo "7 Cannot seek to row $i: " . mysql_error() . "\n";
            };
            ...
            list($jmlBrg7, $jmlHrg7, $tampil_jmlharga7) = RekapByOPD_TampilJmldanHarga($isi7, $isRibuan, $cltd);
            $jmlBrgBI = $jmlBrg + $jmlBrg2 + $jmlBrg3 + $jmlBrg4 + $jmlBrg5 + $jmlBrg6+ $jmlBrg7;
            $jmlHrgBI = $jmlHrg + $jmlHrg2 + $jmlHrg3 + $jmlHrg4 + $jmlHrg5 + $jmlHrg6+ $jmlHrg7;
    ...
    if ($noawal == 0) {
        ...
        if ($SPg == '10') {
            ...
            $totBrg7 = $totBrgBI7 - $totBrgHPS7;
            $totHrg7 = ($totHrgPLH7 + $totHrgAman7 + $totHrgBI7) - $totHrgHPS7;

            $totBrgBI = $totBrg + $totBrg2 + $totBrg3 + $totBrg4 + $totBrg5 + $totBrg6+ $totBrg7; //$totBrgBI - $totBrgHPSBI;
            $totHrgBI = $totHrg + $totHrg2 + $totHrg3 + $totHrg4 + $totHrg5 + $totHrg6+ $totHrg7; //($totHrgPLHBI + $totHrgAmanBI + $totHrgBI) - $totHrgHPSBI;
        }
        //tampil tot
        ...
        if ($SPg == '10') {
            ...
            $tampilTot_jmlharga7 = RekapByOPD_TampilJmldanHarga_('00', $isRibuan, $cltd, $totBrg7, $totHrg7);
...
function Mutasi_RekapByBrg_GetList2($fmKEPEMILIKAN, $fmSKPD, $fmUNIT, $fmSUBUNIT,
	...
	$Style=1, $fmSEKSI='00', $fmKONDBRG='7', $xls=0) {
    // ------------------------------
	...
	//if($xls === 1) $clGaris = 'GCTK';
    //get kondisi (skpd) ----------------------------------------------------------------------------------
    $KondisiD = $fmUNIT == "00" ? "" : " and d='$fmUNIT' ";
    $KondisiD_ = $fmUNIT == "00" ? "" : " and bb.d='$fmUNIT' ";
    $KondisiE = $fmSUBUNIT == "00" ? "" : " and e='$fmSUBUNIT' ";
    $KondisiE_ = $fmSUBUNIT == "00" ? "" : " and bb.e='$fmSUBUNIT' ";
	//$KondisiE1 = $fmSEKSI =='' || $fmSEKSI == "000" || $fmSEKSI == "00"  ? "" : " and e1='$fmSEKSI' ";
    $Kondisi = "  a1='$fmKEPEMILIKAN' and a='{$Main->Provinsi[0]}'	and c='$fmSKPD' $KondisiD $KondisiE $KondisiE1 ";
    $Kondisi_ = "  bb.a1='$fmKEPEMILIKAN' and bb.a='{$Main->Provinsi[0]}'	and bb.c='$fmSKPD' $KondisiD_ $KondisiE_ $KondisiE1_ ";
    if ($fmSKPD == "00") {
        $Kondisi = "  a1='$fmKEPEMILIKAN' and a='{$Main->Provinsi[0]}'	$KondisiD $KondisiE $KondisiE1";
        $Kondisi_ = "  bb.a1='$fmKEPEMILIKAN' and bb.a='{$Main->Provinsi[0]}'	$KondisiD_ $KondisiE_ $KondisiE1_ ";
    }
	...
	switch($fmKONDBRG){
		case '1' :case '2': case '3': case '4' :{
			...
			$Kondisi_ .= $Kondisi==''? " bb.kondisi = '$fmKONDBRG' ": " and bb.kondisi = '$fmKONDBRG' ";
			break;
		}
		/*default : {
		...
		case '5' :{
			$Kondisi .= $Kondisi==''? " kondisi in('1','2','3') ": " and kondisi   in('1','2','3') ";
			$Kondisi .= $Kondisi==''? " bb.kondisi in('1','2','3') ": " and bb.kondisi   in('1','2','3') ";
			break;
		}
	}
	...
	*/
    ...
    $sqry =
		...
		"from ref_barang aa
			...
			left join (select f, g, sum(biaya_pemeliharaan ) as jmlHrgHPS_PLH_awal from v2_penghapusan_pelihara where $Kondisi and tgl_penghapusan < '$tglAwal' and tambah_aset=1  group by f) jj on aa.f=jj.f
			left join (select f, g, sum(biaya_pengamanan ) as jmlHrgHPS_Aman_awal from v2_penghapusan_pengaman where $Kondisi and tgl_penghapusan < '$tglAwal' and tambah_aset=1  group by f) kk on aa.f=kk.f

			left join (select f, g, sum(biaya_pemeliharaan ) as jmlHrgHPS_PLH_akhir from v2_penghapusan_pelihara where $Kondisi and tgl_penghapusan <= '$tglAkhir' and tambah_aset=1  group by f) ll on aa.f=ll.f
			left join (select f, g, sum(biaya_pengamanan ) as jmlHrgHPS_Aman_akhir from v2_penghapusan_pengaman where $Kondisi and tgl_penghapusan <= '$tglAkhir' and tambah_aset=1  group by f) mm on aa.f=mm.f

			left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, count(*) as jmlHSBG_awal, sum(harga_hapus) as jmlHrgHSBG_awal from v_hapus_sebagian where $Kondisi and tgl_penghapusan < '$tglAwal' group by f ) nn on aa.f=nn.f
			left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, count(*) as jmlHSBG_akhir, sum(harga_hapus ) as jmlHrgHSBG_akhir from v_hapus_sebagian where $Kondisi  and tgl_penghapusan <= '$tglAkhir' group by f ) oo on aa.f=oo.f
			left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, sum(harga_hapus ) as jmlHrgHPS_HSBG_awal from v2_penghapusan_hapussebagian where  tgl_penghapusan < '$tglAwal' $KondisiKIB group by f ) pp on aa.f=pp.f
			left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, sum(harga_hapus) as jmlHrgHPS_HSBG_akhir from v2_penghapusan_hapussebagian where  tgl_penghapusan <= '$tglAkhir'  $KondisiKIB group by f ) qq on aa.f=qq.f
		...
		"from ref_barang aa
			...
			left join (select f , g, sum(jml_barang) as jmlBrgBI_awal, sum(jml_harga ) as jmlHrgBI_awal from buku_induk where $Kondisi and tgl_buku < '$tglAwal'  group by f,g) ee on aa.f=ee.f and aa.g=ee.g
			...
			left join (select f, g, sum(biaya_pemeliharaan ) as jmlHrgHPS_PLH_awal from v2_penghapusan_pelihara where $Kondisi and tgl_penghapusan < '$tglAwal' and tambah_aset=1  group by f,g) jj on aa.f=jj.f  and aa.g=jj.g
			left join (select f, g, sum(biaya_pengamanan ) as jmlHrgHPS_Aman_awal from v2_penghapusan_pengaman where  $Kondisi and tgl_penghapusan < '$tglAwal' and tambah_aset=1  group by f,g) kk on aa.f=kk.f  and aa.g=kk.g
			left join (select f, g, sum(biaya_pemeliharaan ) as jmlHrgHPS_PLH_akhir from v2_penghapusan_pelihara where $Kondisi and tgl_penghapusan <= '$tglAkhir' and tambah_aset=1  group by f,g) ll on aa.f=ll.f  and aa.g=ll.g
			left join (select f, g, sum(biaya_pengamanan ) as jmlHrgHPS_Aman_akhir from v2_penghapusan_pengaman where $Kondisi and tgl_penghapusan <= '$tglAkhir' and tambah_aset=1  group by f,g) mm on aa.f=mm.f  and aa.g=mm.g

			left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, count(*) as jmlHSBG_awal, sum(harga_hapus) as jmlHrgHSBG_awal from v_hapus_sebagian where $Kondisi and tgl_penghapusan < '$tglAwal' group by f,g ) nn on aa.f=nn.f and aa.g=nn.g
			left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, count(*) as jmlHSBG_akhir, sum(harga_hapus ) as jmlHrgHSBG_akhir from v_hapus_sebagian where $Kondisi  and tgl_penghapusan <= '$tglAkhir' group by f,g  ) oo on aa.f=oo.f and aa.g=oo.g
			left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, sum(harga_hapus ) as jmlHrgHPS_HSBG_awal from v2_penghapusan_hapussebagian where  tgl_penghapusan < '$tglAwal' $KondisiKIB group by f,g  ) pp on aa.f=pp.f  and aa.g=pp.g
			left join (select IFNULL(f,'00') as f , IFNULL(g,'00') as g, sum(harga_hapus) as jmlHrgHPS_HSBG_akhir from v2_penghapusan_hapussebagian where  tgl_penghapusan <= '$tglAkhir'  $KondisiKIB group by f,g  ) qq on aa.f=qq.f  and aa.g=qq.g

			".
		" where aa.g<>'00' and aa.h='00'
		)
		order by f, g"; //echo "$sqry";
	// $cek.=$sqry;
    ...
    while ($isi = mysql_fetch_array($QryRefBarang)) {
        ...
        $Kondisi1 = " concat(f, g)= '{$isi['f']}{$isi['g']}' ";
        $Kondisi1_ = " concat(bb.f, bb.g)= '{$isi['f']}{$isi['g']}' ";
        $KondisiBi = " status_barang<>3 ";
        $KondisiBi_ = " bb.status_barang<>3 ";
		$KondisiFG = $isi['g'] == "00" ? "f='{$isi['f']}'" : "f='{$isi['f']}' and g='{$isi['g']}'";
		$KondisiFG_ = $isi['g'] == "00" ? " bb.f='{$isi['f']}'" : " bb.f='{$isi['f']}' and bb.g='{$isi['g']}'";
		$groupFG = $isi['g'] == "00" ? "group by f" : "group by f,g";
		$groupFG_ = $isi['g'] == "00" ? "group by bb.f" : "group by bb.f,bb.g";
...
//        $jmlBrgBI_awal = $isi['jmlBrgBI_awal'];
		...
		//hapus sebagian
        $jmlHrgHSBG_awal = $isi['jmlHrgHSBG_awal']==NULL? 0: $isi['jmlHrgHSBG_awal'];
        $jmlHrgHSBG_akhir = $isi['jmlHrgHSBG_akhir']==NULL? 0: $isi['jmlHrgHSBG_akhir'];
		$jmlHrgHSBG_curr = $jmlHrgHSBG_akhir - $jmlHrgHSBG_awal;
		//koreksi --------------------------------------------------------------------------
		$qrkoreksi = " select sum(if(  aa.tgl<'$tglAwal' , aa.harga_baru-aa.harga, 0) ) as hrgaw ,
				sum(if(  aa.tgl<='$tglAkhir' , aa.harga_baru-aa.harga, 0) ) as hrgak
			from t_koreksi aa
			left join buku_induk bb on aa.idbi = bb.id ".
			" where $Kondisi and $KondisiFG $groupFG  ";
		$get = mysql_fetch_array( mysql_query(
			$qrkoreksi
		));
		$jmlHrgKoreksi_awal = $get['hrgaw'];
		$jmlHrgKoreksi_akhir = $get['hrgak'];
		$jmlHrgKoreksi_curr = $jmlHrgKoreksi_akhir-$jmlHrgKoreksi_awal;
		//penilaian -----------------------------------------------------------------------------
		//$Kondisi_ =
		$qryNilai = " select sum(if(  aa.tgl_penilaian<'$tglAwal' , aa.nilai_barang-aa.nilai_barang_asal, 0) ) as hrgaw ,
				sum(if(  aa.tgl_penilaian<='$tglAkhir' , aa.nilai_barang-aa.nilai_barang_asal, 0) ) as hrgak
			from penilaian aa
			left join buku_induk bb on aa.idbi = bb.id ".
			" where $Kondisi_ and $KondisiFG_ $groupFG_  ";
		$qryNilai = str_replace('aa.idbi', 'aa.id_bukuinduk' ,$qryNilai );
		$get = mysql_fetch_array( mysql_query(
			$qryNilai
		));
		$jmlHrgPenilai_awal = $get['hrgaw'];
		$jmlHrgPenilai_akhir = $get['hrgak'];
		$jmlHrgPenilai_curr = $jmlHrgPenilai_akhir-$jmlHrgPenilai_awal;

		//hapus pelihara ------------------------------------------------------------------
		$jmlHrgHPS_PLH_awal = $isi['jmlHrgHPS_PLH_awal']==NULL? 0: $isi['jmlHrgHPS_PLH_awal'];;
		...
		//hapus koreksi
		$get = mysql_fetch_array( mysql_query(
			"select sum(if(  tgl<'$tglAwal' , harga_koreksi, 0) ) as hrgaw ,
				sum(if(  tgl<='$tglAkhir' , harga_koreksi, 0) ) as hrgak
			from v2_penghapusan_koreksi ".
			" where $Kondisi and $KondisiFG $groupFG  "
		));
		$jmlHrgHPS_Koreksi_awal = $get['hrgaw'];
		$jmlHrgHPS_Koreksi_akhir = $get['hrgak'];
		$jmlHrgHPS_Koreksi_curr = $jmlHrgHPS_Koreksi_akhir - $jmlHrgHPS_Koreksi_awal ;
		//hapus penilai
		$get = mysql_fetch_array( mysql_query(
			"select sum(if(  tgl<'$tglAwal' , harga_nilai, 0) ) as hrgaw ,
				sum(if(  tgl<='$tglAkhir' , harga_nilai, 0) ) as hrgak
			from v2_penghapusan_penilai ".
			" where $Kondisi and $KondisiFG $groupFG  "
		));
		$jmlHrgHPS_Penilai_awal = $get['hrgaw'];
		$jmlHrgHPS_Penilai_akhir = $get['hrgak'];
		$jmlHrgHPS_Penilai_curr = $jmlHrgHPS_Penilai_akhir - $jmlHrgHPS_Penilai_awal ;
		...
		$get = mysql_fetch_array( mysql_query(
			"select sum(if(  tgl<'$tglAwal' , harga_koreksi, 0) ) as hrgaw ,
				sum(if(  tgl<='$tglAkhir' , harga_koreksi, 0) ) as hrgak
			from v2_mutasi_koreksi ".
			" where $Kondisi and $KondisiFG $groupFG  "
		));
		$jmlHrgMut_Koreksi_awal = $get['hrgaw'];
		$jmlHrgMut_Koreksi_akhir = $get['hrgak'];
		$jmlHrgMut_Koreksi_curr = $jmlHrgMut_Koreksi_akhir - $jmlHrgMut_Koreksi_awal ;
		//mutasi penilai
		$get = mysql_fetch_array( mysql_query(
			"select sum(if(  tgl<'$tglAwal' , harga_nilai, 0) ) as hrgaw ,
				sum(if(  tgl<='$tglAkhir' , harga_nilai, 0) ) as hrgak
			from v2_mutasi_penilai ".
			" where $Kondisi and $KondisiFG $groupFG  "
		));
		$jmlHrgMut_Penilai_awal = $get['hrgaw'];
		$jmlHrgMut_Penilai_akhir = $get['hrgak'];
		$jmlHrgMut_Penilai_curr = $jmlHrgMut_Penilai_akhir - $jmlHrgMut_Penilai_awal ;

		//pindahtangan ----------------------------------------------------------
		$qrpindah = "select sum(jml_barang) as sumbrg, sum(jml_harga ) as sumbiaya from v1_pindahtangan
			where $Kondisi and tgl_pemindahtanganan < '$tglAwal' and $KondisiFG $groupFG " ; $cek .= "qrpindah awal = $qrpindah <br>";
		$get= mysql_fetch_array( mysql_query(
			$qrpindah
		));
		...
		$qrpindah = "select sum(jml_barang) as sumbrg, sum(jml_harga ) as sumbiaya from v1_pindahtangan
			where $Kondisi and tgl_pemindahtanganan <= '$tglAkhir' and $KondisiFG $groupFG " ; $cek .= "qrpindah akhir = $qrpindah <br>";
		$get= mysql_fetch_array( mysql_query(
			$qrpindah
		));
		...
		//pindahtangan koreksi
		$get = mysql_fetch_array( mysql_query(
			"select sum(if(  tgl<'$tglAwal' , harga_koreksi, 0) ) as hrgaw ,
				sum(if(  tgl<='$tglAkhir' , harga_koreksi, 0) ) as hrgak
			from v2_pindahtangan_koreksi ".
			" where $Kondisi and $KondisiFG $groupFG  "
		));
		$jmlHrgPindah_Koreksi_awal = $get['hrgaw'];
		$jmlHrgPindah_Koreksi_akhir = $get['hrgak'];
		$jmlHrgPindah_Koreksi_curr = $jmlHrgPindah_Koreksi_akhir - $jmlHrgPindah_Koreksi_awal ;
		//pindahtangan penilai
		$get = mysql_fetch_array( mysql_query(
			"select sum(if(  tgl<'$tglAwal' , harga_nilai, 0) ) as hrgaw ,
				sum(if(  tgl<='$tglAkhir' , harga_nilai, 0) ) as hrgak
			from v2_pindahtangan_penilai ".
			" where $Kondisi and $KondisiFG $groupFG  "
		));
		$jmlHrgPindah_Penilai_awal = $get['hrgaw'];
		$jmlHrgPindah_Penilai_akhir = $get['hrgak'];
		$jmlHrgPindah_Penilai_curr = $jmlHrgPindah_Penilai_akhir - $jmlHrgPindah_Penilai_awal ;

		//gantirugi --------------------------------------------------
		$qrGR = "select sum(jml_barang) as sumbrg, sum(jml_harga ) as sumbiaya from v1_gantirugi
			where $Kondisi and tgl_gantirugi < '$tglAwal' and $KondisiFG $groupFG " ; $cek .= "qrGR=$qrGR <br>";
		$get= mysql_fetch_array( mysql_query(
			$qrGR
		));
		...
		$get = mysql_fetch_array( mysql_query(
			"select sum(if(  tgl<'$tglAwal' , harga_koreksi, 0) ) as hrgaw ,
				sum(if(  tgl<='$tglAkhir' , harga_koreksi, 0) ) as hrgak
			from v2_gantirugi_koreksi ".
			" where $Kondisi and $KondisiFG $groupFG  "
		));
		$jmlHrgGantirugi_Koreksi_awal = $get['hrgaw'];
		$jmlHrgGantirugi_Koreksi_akhir = $get['hrgak'];
		$jmlHrgGantirugi_Koreksi_curr = $jmlHrgGantirugi_Koreksi_akhir - $jmlHrgGantirugi_Koreksi_awal ;
		//Gantirugi penilai
		$get = mysql_fetch_array( mysql_query(
			"select sum(if(  tgl<'$tglAwal' , harga_koreksi, 0) ) as hrgaw ,
				sum(if(  tgl<='$tglAkhir' , harga_koreksi, 0) ) as hrgak
			from v2_gantirugi_penilai ".
			" where $Kondisi and $KondisiFG $groupFG  "
		));
		$jmlHrgGantirugi_Penilai_awal = $get['hrgaw'];
		$jmlHrgGantirugi_Penilai_akhir = $get['hrgak'];
		$jmlHrgGantirugi_Penilai_curr = $jmlHrgGantirugi_Penilai_akhir - $jmlHrgGantirugi_Penilai_awal ;


        //hitung row --------------------------------------------------------------------------
        /*//saldo awal
		$jmlBrg_awal = $jmlBrgBI_awal - ($jmlBrgHPS_awal + $jmlBrgPindah_awal + $jmlBrgGantirugi_awal);   // 4799 -321

		$jmlHrg_awal =
			(($jmlHrgBI_awal )+ //perolehan
			($jmlHrgPLH_awal + $jmlHrgMut_PLH_awal )+ //pelihara
			($jmlHrgHPS_HSBG_awal + $jmlHrgPindah_HSBG_awal + $jmlHrgGantirugi_HSBG_awal)+ //hapus sebagian debet
			($jmlHrgKoreksi_awal + $jmlHrgMut_Koreksi_awal )+//koreksi
			($jmlHrgPenilai_awal + $jmlHrgMut_Penilai_awal)+ //penilai
			($jmlHrgAman_awal+ $jmlHrgMut_Aman_awal) //pengamananan
			)
			-
			( $jmlBrgHPS_curr + //penghapusan
				($jmlHrgHPS_PLH_awal + $jmlHrgPindah_PLH_awal + $jmlHrgGantirugi_PLH_awal  )+ //hapus, pindah.gantirugi pelihara
				($jmlHrgHPS_Aman_awal + $jmlHrgPindah_Aman_awal + $jmlHrgGantirugi_Aman_awal) + //hapus, pindah.gantirugi aman
				($jmlHrgHPS_Koreksi_awal + $jmlHrgPindah_Koreksi_awal + $jmlHrgGantirugi_Koreksi_awal  )+ //hapus, pindah.gantirugi koeksi
				($jmlHrgHPS_Penilai_awal + $jmlHrgPindah_Penilai_awal + $jmlHrgGantirugi_Penilai_awal  ) //hapus, pindah.gantirugi penilaian

			);
        //bertambah ------------------------------------------------------------------------------
		$jmlBrgTambah_curr = $jmlBrgBI_curr; 		//barang bertambah

		$jmlHrgTambahPLH_curr =
			($jmlHrgPLH_curr + $jmlHrgMut_PLH_awal )+ //pelihara
			($jmlHrgHPS_HSBG_curr + $jmlHrgPindah_HSBG_curr + $jmlHrgGantirugi_HSBG_curr)+ //hapus sebagian debet
			($jmlHrgKoreksi_curr + $jmlHrgMut_Koreksi_curr )+//koreksi
			($jmlHrgPenilai_curr + $jmlHrgMut_Penilai_curr ) //penilai
			;
		$jmlHrgTambahAman_curr = $jmlHrgAman_curr + $jmlHrgMut_Aman_curr ;
		$jmlHrgTambah_curr =
			($jmlHrgTambahPLH_curr ) + // +
			($jmlHrgTambahAman_curr )+// +
			($jmlHrgBI_curr ) //$jmlHrgTambahBi_curr+
			;//$jmlHrgTambahKoreksi_curr;

		//berkurang -------------------------------------------------------------------------------------
		$jmlBrgKurang_curr = $jmlBrgHPS_curr + $jmlBrgPindah_curr + $jmlBrgGantirugi_curr;

		$jmlHrgKurangPLH_curr =
			($jmlHrgHPS_PLH_curr + $jmlHrgPindah_PLH_curr + $jmlHrgGantirugi_PLH_curr  )+ //hapus, pindah.gantirugi pelihara
			($jmlHrgHPS_Koreksi_curr + $jmlHrgPindah_Koreksi_curr + $jmlHrgGantirugi_Koreksi_curr  )+ //hapus, pindah.gantirugi koeksi
			($jmlHrgHPS_Penilai_curr + $jmlHrgPindah_Penilai_curr + $jmlHrgGantirugi_Penilai_curr  ); //hapus, pindah.gantirugi penilaian
		...
		$jmlHrgKurang_curr =  ($jmlHrgKurangPLH_curr) +	($jmlHrgKurangAman_curr)+($jmlHrgKurangBi_curr );
		*/
		//hitung row --------------------------------------------------------------------------
        //saldo awal
		$jmlBrg_awal = $jmlBrgBI_awal - ($jmlBrgHPS_awal + $jmlBrgPindah_awal + $jmlBrgGantirugi_awal);   // 4799 -321
		...
		$jmlHrg_awal =
			($jmlHrgBI_awal +
			$jmlHrgPLH_awal + $jmlHrgMut_PLH_awal +  //pelihara
			$jmlHrgAman_awal + $jmlHrgMut_Aman_awal + //pengamanan
			$jmlHrgHPS_HSBG_awal + $jmlHrgPindah_HSBG_awal+ $jmlHrgGantirugi_HSBG_awal+ //hapus sebagian debet
			($jmlHrgKoreksi_awal + $jmlHrgMut_Koreksi_awal )+ //koreksi
			($jmlHrgPenilai_awal + $jmlHrgMut_Penilai_awal) //penilai
			) -
			($jmlHrgHPS_awal + $jmlHrgHPS_PLH_awal + $jmlHrgHPS_Aman_awal + $jmlHrgHPS_Koreksi_awal + $jmlHrgHPS_Penilai_awal +//hapus
			$jmlHrgPindah_awal + $jmlHrgPindah_PLH_awal + $jmlHrgPindah_Aman_awal + $jmlHrgPindah_Koreksi_awal + $jmlHrgPindah_Penilai_awal+//pindah
			$jmlHrgGantirugi_awal + $jmlHrgGantirugi_PLH_awal + $jmlHrgGantirugi_Aman_awal + $jmlHrgGantirugi_Koreksi_awal + $jmlHrgGantirugi_Penilai_awal+//ganti rugi
			$jmlHrgHSBG_awal + $jmlHrgMut_HSBG_awal //hapus sebagian
			);
        //bertambah ------------------------------------------------------------------------------
		$jmlBrgTambah_curr = $jmlBrgBI_curr; 		//barang bertambah
		$jmlHrgTambahPLH_curr =
			($jmlHrgPLH_curr + $jmlHrgMut_PLH_curr )+ //pelihara
			($jmlHrgHSBG_curr + $jmlHrgPindah_HSBG_curr+ $jmlHrgGantirugi_HSBG_curr  )+ //hapus sebagian debet
			($jmlHrgKoreksi_curr + $jmlHrgMut_Koreksi_curr )+ //koreksi
			($jmlHrgPenilai_curr + $jmlHrgMut_Penilai_curr ); //penilai
		$jmlHrgTambahAman_curr = $jmlHrgAman_curr + $jmlHrgMut_Aman_curr ;
		$jmlHrgTambahBi_curr= $jmlHrgBI_curr;
		$jmlHrgTambah_curr = ($jmlHrgTambahPLH_curr ) + ($jmlHrgTambahAman_curr )+ ($jmlHrgBI_curr ) ;//$jmlHrgTambahKoreksi_curr;

		//berkurang -------------------------------------------------------------------------------------
		$jmlBrgKurang_curr = $jmlBrgHPS_curr + $jmlBrgPindah_curr + $jmlBrgGantirugi_curr;
		$jmlHrgKurangPLH_curr =
			(//$jmlHrgHPS_curr +
			$jmlHrgHPS_PLH_curr + $jmlHrgHPS_Koreksi_curr + $jmlHrgHPS_Penilai_curr ) + //hapus
			$jmlHrgPindah_PLH_curr + $jmlHrgPindah_Koreksi_curr + $jmlHrgPindah_Penilai_curr+//pindah
			$jmlHrgGantirugi_PLH_curr + $jmlHrgGantirugi_Koreksi_curr + $jmlHrgGantirugi_Penilai_curr+//ganti rugi
			$jmlHrgHSBG_curr + $jmlHrgMut_HSBG_curr; //hapus sebagian
		...
		$jmlBrg_akhir = $jmlBrgBI_akhir - $jmlBrgHPS_akhir - $jmlBrgPindah_akhir - $jmlBrgGantirugi_akhir; //60-10-
		$jmlHrg_akhir = $jmlHrg_awal + $jmlHrgTambah_curr - $jmlHrgKurang_curr;
		...
		if($xls==1) $tampil_jmlHrgHPS_Aman_curr = $dlmRibuan == TRUE ? $jmlHrgKurangAman_curr / 1000 : $jmlHrgKurangAman_curr;

		$tampil_jmlHrg_akhir = $dlmRibuan == TRUE ? number_format(($jmlHrg_akhir / 1000), 2, ',', '.') : number_format($jmlHrg_akhir, 2, ',', '.');
		if($xls==1) $tampil_jmlHrg_akhir = $dlmRibuan == TRUE ? $jmlHrg_akhir / 1000 : $jmlHrg_akhir ;
		...
		//bold
		$vjmlBrg_awal = $xls==1 ? $jmlBrg_awal : number_format($jmlBrg_awal, 0, ',', '.') ;
        $tampil_jmlBrg_awal = $isi['g'] == "00" ? "<b>" . $vjmlBrg_awal  . "" : "" . $vjmlBrg_awal . "";
		$vjmlBrgHPS_curr = $xls == 1 ? $jmlBrgHPS_curr : number_format($jmlBrgHPS_curr, 0, ',', '.') ;
        $tampil_jmlBrgHPS_curr = $isi['g'] == "00" ? "<b>" . $vjmlBrgHPS_curr . "" : "" . $vjmlBrgHPS_curr . "";
		$vjmlBrgTambah_curr = $xls == 1 ? $jmlBrgTambah_curr : number_format($jmlBrgTambah_curr, 0, ',', '.') ;
        $tampil_jmlBrgTambah_curr = $isi['g'] == "00" ? "<b>" . $vjmlBrgTambah_curr. "" : "" . $vjmlBrgTambah_curr . "";
		$vjmlBrg_akhir= $xls == 1 ? $jmlBrg_akhir:  number_format($jmlBrg_akhir, 0, ',', '.');
        $tampil_jmlBrg_akhir = $isi['g'] == "00" ? "<b>" . $vjmlBrg_akhir . "" : "" .$vjmlBrg_akhir . "";
		...
		$tampil_jmlHrgMut_PLH_curr = addbold( $xls==1 ? $jmlHrgMut_PLH_curr : number_format_ribuan($jmlHrgMut_PLH_curr, $dlmRibuan) , $isi['g'] == "00" );
		$tampil_jmlHrgMut_Aman_curr = addbold( $xls==1 ? $jmlHrgMut_Aman_curr : number_format_ribuan($jmlHrgMut_Aman_curr, $dlmRibuan) , $isi['g'] == "00" );
        //with td
		...
		//$clGaris1 = $xls==1 ? 'nfmt1': $clGaris;
		//$clGaris4 = $xls==1 ? 'nfmt4': $clGaris;
        ...
        //$tampil_jmlHrgTambah_curr='';
		//$clGaris5 = $xls==1 ? 'nfmt5' : $clGaris;

        $ListData .= "
			<tr class='$clRow'>
			<td class=\"$clGaris\" align=center width=\"$kolomwidth[0]\">$no.</td>
			<td class=\"$clGaris5\" align=center width=\"$kolomwidth[1]\"><div class=\"nfmt5\"> {$isi['f']}</div></td>
			<td class=\"$clGaris\" align=center width=\"$kolomwidth[2]\"><div class=\"nfmt5\"> $kdBidang </div> </td>
			<td class=\"$clGaris\" width=\"$kolomwidth[3]\">
			<!--
			<input type='hidden' value='jmlBrg_akhir = $jmlBrgBI_akhir - $jmlBrgHPS_akhir - $jmlBrgPindah_akhir - $jmlBrgGantirugi_akhir' >
			<input type='hidden' value='($jmlHrgBI_awal + $jmlHrgPLH_awal + $jmlHrgAman_awal +  $jmlHrgMut_PLH_awal+ $jmlHrgMut_Aman_awal) -
			...
			<input type='hidden' value='$jmlHrgHPS_PLH_awal' id='jmlHrgHSBG_akhir'>
			-->
			$nmBarang</td>
	...
	$vtotBrg_awal = $xls==1 ? $totBrg_awal :  number_format(($totBrg_awal), 0, ',', '.');
	$vtotBrgHPS_curr = $xls == 1 ? $totBrgHPS_curr : number_format(($totBrgHPS_curr), 0, ',', '.') ;
	$vtotBrgTambah_curr = $xls == 1 ? $totBrgTambah_curr : number_format(($totBrgTambah_curr), 0, ',', '.');
	$vtotBrg_akhir = $xls==1 ? $totBrg_akhir : number_format(($totBrg_akhir), 0, ',', '.') ;
    switch ($Style) {
        case 1: {
                $tampilKet = $cetak ? "<td class=\"$clGaris\">&nbsp;</td>" : '';
                $TampilStyleTot = "
				<td align=right class=\"$clGaris\"><div class=\"nfmt1\"><b> " . $vtotBrg_awal . "</div></td>
				<td align=right class=\"$clGaris\"><div class=\"nfmt4\"><b> " . $tampil_totHrg_awal . "</div></td>
				<td align=right class=\"$clGaris\"><div class=\"nfmt1\"><b> " . $vtotBrgHPS_curr. "</div></td>
				$tampil_totHrgKurang_curr
				<td align=right class=\"$clGaris\"><div class=\"nfmt1\"><b> " . $vtotBrgTambah_curr . "</div></td>
				$tampil_totHrgTambah_curr
				<td align=right class=\"$clGaris\"><div class=\"nfmt1\"><b> " . $vtotBrg_akhir . "</div></td>
	...
	$cek = $cetak? '': " $qrkoreksi ; $qryNilai";
    $ListData .= "
			<tr class=''>
			<td colspan=4 class=\"$clGaris\"><b>TOTAL <div style='display:none' >$cek </div></td>
			$TampilStyleTot
			</tr>
			";//.'no awal='.$noawal
...
function Mutasi_RekapByBrg_GetList2_bck20161215($fmKEPEMILIKAN, $fmSKPD, $fmUNIT, $fmSUBUNIT,
	$noawal, $limitHal, $kolomwidth, $dlmRibuan=TRUE, $cetak=FALSE,
...
//        $jmlBrgBI_awal = $isi['jmlBrgBI_awal'];
		...
		//koreksi --------------------------------------------------------------------------
		$get = mysql_fetch_array( mysql_query(
			" select sum(if(  aa.tgl<'$tglAwal' , aa.harga_baru-aa.harga, 0) ) as hrgaw ,
				sum(if(  aa.tgl<'$tglAkhir , aa.harga_baru-aa.harga, 0) ) as hrgak
			from t_koreksi aa
			left join buku_induk bb on aa.idbi = bb.id ".
			" where $Kondisi and $KondisiFG $groupFG  "
		...
		$jmlHrgTambahKoreksi_curr = $jmlHrgKoreksi_curr + $jmlHrgMut_Koreksi_curr;

		$jmlHrgTambah_curr = $jmlHrgTambahPLH_curr + $jmlHrgTambahAman_curr + $jmlHrgTambahBi_curr+$jmlHrgTambahHSBG_curr + $jmlHrgTambahKoreksi_curr;
		//berkurang ------------------------------------------------------------
		$jmlBrgKurang_curr = $jmlBrgHPS_curr + $jmlBrgPindah_curr + $jmlBrgGantirugi_curr;
        ...
        */
        ...
        $tampil_jmlHrgTambah_curr = "<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlHrgTambah_curr</td>";
        $tampil_jmlHrgPLH_curr = "<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlHrgPLH_curr</td>";
        $tampil_jmlHrgAman_curr = "<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlHrgAman_curr</td>";
        $tampil_jmlHrgBI_curr = "<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlHrgBI_curr</td>";

		$tampil_jmlHrgKurang_curr = "<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlHrgKurang_curr</td>";
		$tampil_jmlHrgHPS_curr = "<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlHrgHPS_curr</td>";
		$tampil_jmlHrgHPS_PLH_curr = "<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlHrgHPS_PLH_curr</td>";
        $tampil_jmlHrgHPS_Aman_curr = "<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlHrgHPS_Aman_curr</td>";
		$tampil_jmlHrgMut_PLH_curr = "<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlHrgMut_PLH_curr</td>";
		$tampil_jmlHrgMut_Aman_curr = "<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlHrgMut_Aman_curr</td>";
        ...
        switch ($Style) {
            case 1: {
                    ...
                    $tampilKet = $cetak ? "<td class=\"$clGaris\">&nbsp;</td>" : '';
                    $TampilStyle = "
					<td class=\"$clGaris nfmt5\" align=right width=\"$kolomwidth[4]\" >$tampil_jmlBrg_awal</td>
					<td class=\"$clGaris nfmt5\" align=right width=\"$kolomwidth[5]\" >$tampil_jmlHrg_awal</td>

					<td class=\"$clGaris nfmt5\" align=right width=\"$kolomwidth[4]\" >$tampil_jmlBrgHPS_curr</td>
					$tampil_jmlHrgKurang_curr
					<td class=\"$clGaris nfmt5\" align=right width=\"$kolomwidth[4]\" >$tampil_jmlBrgTambah_curr</td>
					$tampil_jmlHrgTambah_curr

					<td class=\"$clGaris nfmt5\" align=right width=\"$kolomwidth[4]\" >$tampil_jmlBrg_akhir</td>
					<td class=\"$clGaris nfmt5\" align=right width=\"$kolomwidth[5]\" >$tampil_jmlHrg_akhir</td>
					$tampilKet
            ...
            case 2: {
                    $tampilKet = $cetak ? "<td class=\"$clGaris\">&nbsp;</td>" : '';
                    $TampilStyle = "
					<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlBrg_awal</td>
					<td class=\"$clGaris\" align=right width=\"$kolomwidth[5]\" class='nfmt5'>$tampil_jmlHrg_awal</td>
					<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlBrgHPS_curr</td>
					$tampil_jmlHrgHPS_PLH_curr
					$tampil_jmlHrgHPS_Aman_curr
					$tampil_jmlHrgHPS_curr
					<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlBrgTambah_curr</td>
					$tampil_jmlHrgPLH_curr
					$tampil_jmlHrgAman_curr
					$tampil_jmlHrgBI_curr
					<td class=\"$clGaris\" align=right width=\"$kolomwidth[4]\" class='nfmt5'>$tampil_jmlBrg_akhir</td>
					<td class=\"$clGaris\" align=right width=\"$kolomwidth[5]\" class='nfmt5'>$tampil_jmlHrg_akhir</td>
					$tampilKet
					<!--<td class=\"$clGaris\">&nbsp;</td>-->
				";
                    break;
        ...
        $ListData .= "
			<tr class='$clRow'>
			<td class=\"$clGaris\" align=center width=\"$kolomwidth[0]\">$no.</td>
			<td class=\"$clGaris nfmt5\" align=center width=\"$kolomwidth[1]\" >{$isi['f']}</td>
	...
	$tampil_totHrgHPS_Aman_curr = $dlmRibuan == TRUE ? number_format(($totHrgHPS_Aman_curr / 1000), 2, ',', '.') : number_format($totHrgHPS_Aman_curr, 2, ',', '.');
    //bold
    $tampil_totHrgTambah_curr = "<td align=right class=\"$clGaris\" class='nfmt5'><b>" . $tampil_totHrgTambah_curr . "</td>";
    $tampil_totHrgPLH_curr = "<td align=right class=\"$clGaris\" class='nfmt5'><b>" . $tampil_totHrgPLH_curr . "</td>";
    $tampil_totHrgAman_curr = "<td align=right class=\"$clGaris\" class='nfmt5'><b>" . $tampil_totHrgAman_curr . "</td>";
    $tampil_totHrgBI_curr = "<td align=right class=\"$clGaris\" class='nfmt5'><b>" . $tampil_totHrgBI_curr . "</td>";

	$tampil_totHrgKurang_curr = "<td align=right class=\"$clGaris\" class='nfmt5'><b>" . $tampil_totHrgKurang_curr . "</td>";
	$tampil_totHrgHPS_curr = "<td align=right class=\"$clGaris\" class='nfmt5'><b>" . $tampil_totHrgHPS_curr . "</td>";
	$tampil_totHrgHPS_PLH_curr = "<td align=right class=\"$clGaris\" class='nfmt5'><b>" . $tampil_totHrgHPS_PLH_curr . "</td>";
	$tampil_totHrgHPS_Aman_curr = "<td align=right class=\"$clGaris\" class='nfmt5'><b>" . $tampil_totHrgHPS_Aman_curr . "</td>";
    switch ($Style) {
        case 1: {
                $tampilKet = $cetak ? "<td class=\"$clGaris\">&nbsp;</td>" : '';
                $TampilStyleTot = "
				<td align=right class=\"$clGaris\" class='nfmt5'><b>" . number_format(($totBrg_awal), 0, ',', '.') . "</td>
				<td align=right class=\"$clGaris\" class='nfmt5'><b>" . $tampil_totHrg_awal . "</td>
				<td align=right class=\"$clGaris\" class='nfmt5'><b>" . number_format(($totBrgHPS_curr), 0, ',', '.') . "</td>
				$tampil_totHrgKurang_curr
				<td align=right class=\"$clGaris\" class='nfmt5'><b>" . number_format(($totBrgTambah_curr), 0, ',', '.') . "</td>
				$tampil_totHrgTambah_curr
				<td align=right class=\"$clGaris\" class='nfmt5'><b>" . number_format(($totBrg_akhir), 0, ',', '.') . "</td>
				<td align=right class=\"$clGaris\" class='nfmt5'><b>" . $tampil_totHrg_akhir . "</td>
				$tampilKet
				<!--<td class=\"$clGaris\">&nbsp;</td>-->
				";
                break;
...
function tampilNmSubUnit($isi) {
    ...
    global $fmSKPD, $fmUNIT, $fmSUBUNIT,$fmSEKSI, $Main;

    $nmSKPD = '';
    $nmUNIT = '';
    $nmSUBUNIT = '';
    $nmSEKSI = '';
	$def_e1 =  genNumber(0, $Main->SUBUNIT_DIGIT );
    	/* if($fmSKPD == '00'  ){
    ...
    if ($fmSKPD == '00') {
        //if($fmSKPD == '00'  ){
        $astr = "select nm_skpd from ref_skpd where concat(c,d,e,e1)='" . $isi['c'] . "0000000'"; //echo "<br>astr = ".$astr;
        $nmSKPD = table_get_value($astr, 'nm_skpd');
        //}
        if ($isi['d'] != '00') {
            $astr = "select nm_skpd from ref_skpd where concat(c,d,e,e1)='" . $isi['c'] . $isi['d'] . "00000'"; //echo "<br>astr = ".$astr;
            $nmUNIT = table_get_value($astr, 'nm_skpd');
        }
        if ($isi['e'] != '00') {

            $astr = "select nm_skpd from ref_skpd where concat(c,d,e,e1)='" . $isi['c'] . $isi['d'] . $isi['e'] . "$def_e1'"; //echo "<br>astr = ".$astr;
            $nmSUBUNIT = table_get_value($astr, 'nm_skpd');
        }
        if ($isi['e1'] != '00' && $isi['e1'] != '000') {
            $astr = "select nm_skpd from ref_skpd where concat(c,d,e,e1)='" . $isi['c'] . $isi['d'] . $isi['e'] .$isi['e1'] . "'"; //echo "<br>astr = ".$astr;
            $nmSEKSI = table_get_value($astr, 'nm_skpd');
        }

        $str = $nmSKPD . ' - ' . $nmUNIT . ' - ' . $nmSUBUNIT .' - ' . $nmSEKSI;
    } else if ($fmSKPD != '00' && $fmUNIT == '00' && $fmSUBUNIT == '00' && $fmSEKSI == $def_e1) {
        ...
        if ($isi['e'] != '00' ) {
            $astr = "select nm_skpd from ref_skpd where concat(c,d,e,e1)='" . $isi['c'] . $isi['d'] . $isi['e'] . "000'"; //echo "<br>astr = ".$astr;
            $nmSUBUNIT = table_get_value($astr, 'nm_skpd');
        }
        if ($isi['e1'] != '00' && $isi['e1'] != '000' ) {
            $astr = "select nm_skpd from ref_skpd where concat(c,d,e,e1)='" . $isi['c'] . $isi['d'] . $isi['e'] .$isi['e1'] . "'"; //echo "<br>astr = ".$astr;
        ...
        $str = $nmUNIT . ' - ' . $nmSUBUNIT. ' - ' . $nmSEKSI;
    } elseif ($fmSKPD != '00' && $fmUNIT != '00' && $fmSUBUNIT == '00' && $fmSEKSI == $def_e1) { //echo "<br> d =".$isi['d'];
        $astr = "select nm_skpd from ref_skpd where concat(c,d,e,e1)='" . $isi['c'] . $isi['d'] . $isi['e']. $isi['e1'] . "'"; //echo "<br>astr = ".$astr;
        $nmSEKSI = table_get_value($astr, 'nm_skpd');
        $str = $nmSEKSI;
    }
...
function tampilNmSubUnit2($isi) {
  ...
  global $fmSKPD, $fmUNIT, $fmSUBUNIT,$fmSEKSI;
    ...
    if ($fmSKPD == '00') {
        ...
        $str = $nmSKPD . ' - ' . $nmUNIT . ' - ' . $nmSUBUNIT .' - ' . $nmSEKSI;
    } else if ($fmSKPD != '00' && $fmUNIT == '00' && $fmSUBUNIT == '00' && $fmSEKSI == '000') {
        ...
        $str = $nmUNIT . ' - ' . $nmSUBUNIT. ' - ' . $nmSEKSI;
    } elseif ($fmSKPD != '00' && $fmUNIT != '00' && $fmSUBUNIT == '00' && $fmSEKSI == '000') { //echo "<br> d =".$isi['d'];
        $astr = "select nm_skpd from ref_skpd where concat(c,d,e,e1)='" . $isi['c'] . $isi['d'] . $isi['e']. $isi['e1'] . "'"; //echo "<br>astr = ".$astr;
        $nmSEKSI = table_get_value($astr, 'nm_skpd');
        $str = $nmSEKSI;
    }
...
function tampilNmSubUnit3($isi) {
    ...
    $astr = "select * from view_sub_unit where c='" . $isi['c']."'".
	" and d='". $isi['d'] . "'".
	" and e='". $isi['e'] . "'".
	" and e1='". $isi['e1'] . "'"	;

    $nmSKPD = table_get_value($astr, 'nm_bidang');
    $nmUNIT = table_get_value($astr, 'nm_skpd');
    $nmSUBUNIT = table_get_value($astr, 'nm_unit');
    $nmSEKSI = table_get_value($astr, 'nm_sub_unit');
	...
	if ($nmSUBUNIT==$nmSEKSI) {
		$nmSUBUNIT ='';
	} else {
		$str = ($str=='')?$nmSUBUNIT:' - '.$nmSUBUNIT;
	}

	$str = ($str=='')?$nmSEKSI:' - '.$nmSEKSI;
...
function Mutasi_GetList($cetak=0) {
	...
	if($kodebarang != ''){
		$Kondisi .= " and concat(f,'.',g,'.',h,'.',i,'.',j) like '%$kodebarang%' ";
	}
...
/*
  ...
  <TH class=\"th01\" rowspan=2><input type=\"checkbox\" name=\"toggler2\" value=\"\" onClick=\"checkAll1($jmlDataDPB,'cbDPB','toggler2');\" /></TD>";
...
function bilang($x){
  ...
  elseif ($x < 1000000000000000)
	...
	function Penatausahaan_FormEntry($currID=0, $btCariBrg=TRUE , $entryMutasi=FALSE){
		...
		global $lastNoReg, $fmHARGABARANG, $fmJUMLAHBARANG, $jmlBrgReadonly, $fmASALUSUL, $fmJENISHIBAH;
		...
		global $status_penguasaan_,$fmSTATUSASET,$fmVERIFIKASI,$fmHARGABARANGBELI,$fmHARGABARANGATRIBUSI,$ref_idatribusi;
		global $HTTP_COOKIE_VARS;
		global $no_ba, $tgl_ba, $no_spk, $tgl_spk, $penggunabi;
		global $fmPROGRAM, $fmKEGIATAN, $bk, $ck, $dk, $bk_p, $ck_p, $dk_p, $fmNmProgram, $fmNmKegiatan;

		$ULevel = $HTTP_COOKIE_VARS['coLevel'];
		$jns = $_REQUEST['jns'];
		$err = '';

		//cek data old
		$fmIDLama 	= $_POST['fmIDLama'];
		$old = mysql_fetch_array(mysql_query("select * from buku_induk where id='$fmIDLama'"));

		//udah ada transaksi lain  tidk boleh edit barang
		$trans = mysql_fetch_array(mysql_query(
			"select count(*) as cnt from t_transaksi where idbi='$fmIDLama' "
		));
		$jmltrans = $trans['cnt'];

		$fmSKPD = $_REQUEST['fmSKPD']=='00'?$old['c']:$_REQUEST['fmSKPD'];
		$fmUNIT = $_REQUEST['fmUNIT']=='00'?$old['d']:$_REQUEST['fmUNIT'];

		//Ambil data Mapping skpd keuangan dan aset
		//query mendapatkan ref_skpd_urusan
		$su=mysql_fetch_array(mysql_query("select * from ref_skpd_urusan WHERE c='".$fmSKPD."' AND d='".$fmUNIT."'"));

		if ($tgl_buku<($Main->TAHUN_CUTOFF+1).'-1-1' && $Baru==0)
		{
		   	$DataReadonly=true;
			// read only tgl_buku, jml barang ,harga, kode barang, kondisi

			if ($Main->VerifikasiEnable){
		    	if 	($fmVERIFIKASI =='1')
		    	{
		   	  	$DataReadonly=true;
		   		} else {
			 ...
			 // cekskpd closing ada data_readonly
...
*/
		...
		//ECHO ($Main->TAHUN_CUTOFF+1).'-1-1'.'  - '.$tgl_buku;
		...
		$gedung = mysql_fetch_array(mysql_query(
			"select * from ref_ruang where c='".$ruang['c']."' and d='".$ruang['d']."' and e='".$ruang['e']."' and e1='".$ruang['e1']."' and p='".$ruang['p']."' and q='0000'"
		));
		...
		if ($btCariBrg && !$DataReadonly ){
			$entryBrg =
				...
				'width="200"',' width="10" ','','valign="top" height="24"');
		...
		//edit sudah closing
		if($Baru == 0 && ((sudahClosing($tgl_buku, $old['c'], $old['d'], $old['e'], $old['e1']) || ($old['id'] <> $old['idawal'])) || ($old['status_barang']=='2' || $old['staset']=='7'))){
				$entryBrg =
					formEntryBase2("NamaBarang",":",$fmIDBARANG." - ".$fmNMBARANG
		...
		//sudah ada transaksi lain tidak boleh
		if($Baru == 0 && $jmltrans > 1 ){
			$entryBrg =
					formEntryBase2("NamaBarang",":",$fmIDBARANG." - ".$fmNMBARANG
					."<input type='hidden'  id='fmIDBARANG' name='fmIDBARANG' value='$fmIDBARANG'>
					<input type='hidden'  id='fmNMBARANG' name='fmNMBARANG' value='$fmNMBARANG'>",
		...
		//set array status aset ----------------------------------------
		$ARStatusAset=$Main->StatusAset;
		$STADataReadonly=$DataReadonly;
		if ($jns=='pindah')	{
		 	$STADataReadonly=TRUE;
		 	$ARStatusAset=$Main->StatusAsetTPA;
		}
		else if ($jns=='tgr'){
		 	$STADataReadonly=TRUE;
		 	$ARStatusAset=$Main->StatusAsetTGR;
		}
		else if ($jns=='mitra')	{
		 	$STADataReadonly=TRUE;
		 	$ARStatusAset=$Main->StatusAsetMITRA;
		} if ($jns=='atb' || $KIB=='kibg'){
		 	// $STADataReadonly=TRUE;
		 	$ARStatusAset=$Main->StatusAsetATB;
		}

		//st aset ---------------------------------------
		$vstAset = formEntryBase2('Status Aset',':',
						"<input type='hidden' id='fmSTATUSASET' name='fmSTATUSASET' value='$fmSTATUSASET'>	"
						,'','','','valign="top" height="24"');
		switch ($Main->VERSI_NAME){
			case 'BDG_BARAT': case 'BOGOR': {
				if( sudahClosing($tgl_buku, $old['c'], $old['d'], $old['e'], $old['e1']) == FALSE || $Baru==1){//tidak closing
					$vstAset = //'';//"<input type=''>";
						formEntryBase2("Status Aset",":",
						$Main->StatusAsetView[$fmSTATUSASET-1][1]."<input type='hidden'  id='fmSTATUSASET' name='fmSTATUSASET' value='$fmSTATUSASET'>" ,
						"","","","valign='top' height='24' ");
				}
				break;
			}
			default:{
				$STADataReadonly = $entryMutasi;
				if(  sudahClosing($tgl_buku, $old['c'], $old['d'], $old['e'], $old['e1']) == FALSE || $Baru==1){//tidak closing

					$vstAset =
						formEntryBase2("Status Aset",":",
						cmb2Dv2('fmSTATUSASET',$fmSTATUSASET,$ARStatusAset,'',$STADataReadonly),
						"","","","valign='top' height='24' ");
		...
		/*if (($Main->MODUL_ASET_LAINNYA || $Main->MODUL_KAPITALISASI ) && $Baru==0 ){
			$vstAset = //'';//"<input type=''>";
				formEntryBase2("Status Aset",":",
				$Main->StatusAsetView[$fmSTATUSASET-1][1]."<input type='hidden'  id='fmSTATUSASET' name='fmSTATUSASET' value='$fmSTATUSASET'>" ,
				"","","","valign='top' height='24' ");
		...
		$btCariNoReg = $DataReadonly==TRUE? '':$btCariNoReg;

		//entry bast ---------------------------------------------------------------------------
		if($Main->MODUL_BAST){
			if($Baru == 0 && ((sudahClosing($tgl_buku, $old['c'], $old['d'], $old['e'], $old['e1']) || ($old['id'] <> $old['idawal'])) || ($old['status_barang']=='2' || $old['staset']=='7'))){
				$entryba_no = $old['no_ba'] == ''?"<input tipe='text' id='no_ba' name='no_ba' value='$no_ba' style='width:430px'>":$no_ba."<input type=hidden name='no_ba' id='no_ba' value='$no_ba'>";
				$entryba_tgl = ($old['tgl_ba']== ''  || $old['tgl_ba'] =='0000-00-00') ?createEntryTgl(	'tgl_ba', $tgl_ba, ($DataReadonly==true || $entryMutasi)?'1':'', '', '','adminForm'):JuyTgl1($tgl_ba)."<input type=hidden name='tgl_ba' id='tgl_ba' value='$tgl_ba'>";
				$vbast =
					formEntryBase2('Berita Acara Serah Terima','','').
					formEntryBase2('&nbsp;&nbsp;&nbsp;Nomor', ':',$entryba_no).
					formEntryBase2('&nbsp;&nbsp;&nbsp;Tanggal', ':',
						 $entryba_tgl
						,'','','','valign="top" height="24"').
					"";
			}else{
				$tgl_ba = ifempty($tgl_ba,$HTTP_COOKIE_VARS['coThnAnggaran']);
				$vbast =
					formEntryBase2('Berita Acara Serah Terima','','').
					formEntryBase2('&nbsp;&nbsp;&nbsp;Nomor', ':',"<input tipe='text' id='no_ba' name='no_ba' value='$no_ba' style='width:430px'>").
					formEntryBase2('&nbsp;&nbsp;&nbsp;Tanggal', ':',
						createEntryTgl(	'tgl_ba', $tgl_ba, ($DataReadonly==true || $entryMutasi)?'1':'', '', '','adminForm')
						,'','','','valign="top" height="24"').
					"";
			}
		}
		...
		if($Main->MODUL_SPK){
		if($Baru == 0 && ((sudahClosing($tgl_buku, $old['c'], $old['d'], $old['e'], $old['e1']) || ($old['id'] <> $old['idawal'])) || ($old['status_barang']=='2' || $old['staset']=='7'))){
			$entryspk_no = $old['no_spk'] == ''?"<input tipe='text' id='no_spk' name='no_spk' value='$no_spk' style='width:430px'>":$no_spk."<input type=hidden name='no_spk' id='no_spk' value='$no_spk'>";
			$entryspk_tgl = ($old['tgl_spk'] =='' || $old['tgl_spk'] =='0000-00-00') ? createEntryTgl('tgl_spk', $tgl_spk, ($DataReadonly==true || $entryMutasi)?'1':'', '', '','adminForm'):JuyTgl1($tgl_spk)."<input type=hidden name='tgl_spk' id='tgl_spk' value='$tgl_spk'>";
			$vspk =
				formEntryBase2('SPK/ Kontrak','','').
				formEntryBase2('&nbsp;&nbsp;&nbsp;Nomor', ':',$entryspk_no).
				formEntryBase2('&nbsp;&nbsp;&nbsp;Tanggal', ':',
					$entryspk_tgl
					,'','','','valign="top" height="24"').
				"";
		}else{
			//$tgl_spk = ifempty($tgl_spk,$HTTP_COOKIE_VARS['coThnAnggaran']);
			$vspk =
				formEntryBase2('SPK/ Kontrak','','').
				formEntryBase2('&nbsp;&nbsp;&nbsp;Nomor', ':',"<input tipe='text' id='no_spk' name='no_spk' value='$no_spk' style='width:430px'>").
				formEntryBase2('&nbsp;&nbsp;&nbsp;Tanggal', ':',
					createEntryTgl(	'tgl_spk', $tgl_spk, ($DataReadonly==true || $entryMutasi)?'1':'', '', '','adminForm')
					,'','','','valign="top" height="24"').
				"";
		}
		...
		if($Main->MODUL_PENGGUNAAN){
			$vpenggunabi =
				formEntryBase2('Penggunaan', ':',"<input tipe='text' id='penggunabi' name='penggunabi' value='$penggunabi' style='width:430px'>").
				"";
		}

		if($Baru == 0 && ((sudahClosing($tgl_buku, $old['c'], $old['d'], $old['e'], $old['e1']) || ($old['id'] <> $old['idawal'])) || ($old['status_barang']=='2' || $old['staset']=='7'))){//jika edit, cek tgl closing, readonly yg tidak bisa di edit
				$enThnPerolehan=formEntryBase2('Tahun Perolehan',':',
							$fmTAHUNPEROLEHAN."<input type='hidden'  id='fmTAHUNPEROLEHAN' name='fmTAHUNPEROLEHAN' value='$fmTAHUNPEROLEHAN'>",
							'','','','valign="top" height="24"');
				$enNoreg=formEntryBase2('Nomor Register',':',
						$fmREGISTER."<input type='hidden'  id='fmREGISTER' name='fmREGISTER' value='$fmREGISTER'>",
						'','','','valign="top" height="24"');
				$enHrgBeli=formEntryBase2('Harga Beli Satuan ',':',
							'Rp.'.number_format($fmHARGABARANGBELI,'2',',','.')
							."<input type='hidden'  id='fmHARGABARANGBELI' name='fmHARGABARANGBELI' value='$fmHARGABARANGBELI'>",
							'','','','valign="top" height="24"');
				$enHrgAtribusi=formEntryBase2('Harga Atribusi Satuan',':',
							'Rp.'.number_format($fmHARGABARANGATRIBUSI,'2',',','.')
							."<input type='hidden'  id='fmHARGABARANGATRIBUSI' name='fmHARGABARANGATRIBUSI' value='$fmHARGABARANGATRIBUSI'>",
							'','','','valign="top" height="24"');
				$enHrgBarang=formEntryBase2('Harga Perolehan Satuan ',':',
							'Rp.'.number_format($fmHARGABARANG,'2',',','.')
							."<input type='hidden'  id='fmHARGABARANG' name='fmHARGABARANG' value='$fmHARGABARANG'>",
							'','','','valign="top" height="24"');

				$enJmlSatuan=formEntryBase2("Jumlah Barang",":",
							"$fmJUMLAHBARANG
							<input type='hidden'  id='fmJUMLAHBARANG' name='fmJUMLAHBARANG' value='$fmJUMLAHBARANG'>
							&nbsp&nbsp&nbsp&nbsp &nbsp&nbsp &nbsp;&nbsp;
							Satuan : $fmSATUAN
							<input type='hidden'  id='fmSATUAN' name='fmSATUAN' value='$fmSATUAN'>
							&nbsp&nbsp",
							"","","","valign='top' height='24'");

				$enAsalUsul=formEntryBase2("Asal Usul/Cara Perolehan",":",
										//cmb2D('fmASALUSUL',$fmASALUSUL,$Main->AsalUsul,''),
										$Main->AsalUsul[$fmASALUSUL-1][1]
										."<input type='hidden'  id='fmASALUSUL' name='fmASALUSUL' value='$fmASALUSUL'>",
											"","","","valign='top' height='24'");

				$enJenisHibah=formEntryBase2("Sumber Dana",":",
										($fmJENISHIBAH=='0'?'-':$fmJENISHIBAH)
										."<input type='hidden'  id='fmJENISHIBAH' name='fmJENISHIBAH' value='$fmJENISHIBAH'>",
											"","","","valign='top' height='24'");
		if($Main->MODUL_PROGRAM){
		// program&kegiatan ----------------------------------------------
				/*$queryProgram = "SELECT bk,ck,dk,p,nama FROM ref_program WHERE bk='".$su['bk']."' AND ck='".$su['ck']."' AND dk='".$su['dk']."' AND p!='0' AND q='0'";
				$res = mysql_num_rows(mysql_query($queryProgram));
				if($res == 0){
					$queryProgram = "SELECT bk,ck,dk,p,nama FROM ref_program WHERE bk='".$su['bk']."' AND ck='".$su['ck']."' AND dk='0' AND p!='0' AND q='0'";
					$res2 = mysql_num_rows(mysql_query($queryProgram));
					if($res2 == 0){
						$queryProgram = "SELECT bk,ck,dk,p,nama FROM ref_program WHERE bk='".$su['bk']."' AND ck='0' AND dk='0'  AND p!='0' AND q='0'";
					}
				}
				$prg=mysql_fetch_array(mysql_query($queryProgram));*/
				$bk = $bk=='0'?$su['bk']:$bk;
				$ck = $ck=='0'?$su['ck']:$ck;
				$dk = $dk=='0'?$su['dk']:$dk;
				$entryprog = $old['p']==0 ?
							/*genComboBoxQry('fmPROGRAM',$fmPROGRAM,
							$queryProgram,
							'p', 'nama','Pilih','onChange="adminForm.action=\'#\';adminForm.submit();" ').*/

							"<input type='text'  id='kdprogram' name='kdprogram' value='".$kdprogram."' readonly size=4>&nbsp;".
							"<input type='text'  id='nmprogram' name='nmprogram' value='".$fmNmProgram."' readonly size='84'>&nbsp;".

							"<input type=\"button\" value=\"Cari\"
							  onclick=\"refprogram.windowShow2('bk_p','ck_p','dk_p','fmPROGRAM','fmKEGIATAN',
							  'kdprogram', 'nmprogram', 'kdkegiatan','nmkegiatan')\" $disableCariProg>"
							:$bk_p.".".$ck_p.".".$dk_p.".".$fmPROGRAM." - ".$fmNmProgram;



				$enProgram=formEntryBase2("Program",":",
										$entryprog.
										"<input type='hidden'  id='fmPROGRAM' name='fmPROGRAM' value='$fmPROGRAM'>
										<input type='hidden'  id='bk_p' name='bk_p' value='".$bk_p."'>
										<input type='hidden'  id='ck_p' name='ck_p' value='".$ck_p."'>
										<input type='hidden'  id='dk_p' name='dk_p' value='".$dk_p."'>".
										"<input type='hidden'  id='bk' name='bk' value='".$bk."'>
										<input type='hidden'  id='ck' name='ck' value='".$ck."'>
										<input type='hidden'  id='dk' name='dk' value='".$dk."'>",
											"","","","valign='top' height='24'");

				//$keg = mysql_fetch_array(mysql_query("select nama from ref_program where bk='".$prg['bk']."' and ck='".$prg['ck']."' and dk='".$prg['dk']."' and p='".$fmPROGRAM."' and q='".$fmKEGIATAN."'"));
				$entrykeg = $old['q'] ==0 ?
							/*genComboBoxQry('fmKEGIATAN',$fmKEGIATAN,
							"SELECT q,nama FROM ref_program WHERE bk='".$su['bk']."' AND ck='".$su['ck']."' AND dk='".$su['dk']."' AND p='$fmPROGRAM' AND q!='0'",
							'q', 'nama','Pilih') */
							"<input type='text'  id='kdkegiatan' name='kdkegiatan' value='".$kdkegiatan."' readonly size=4>&nbsp;".
							"<input type='text'  id='nmkegiatan' name='nmkegiatan' value='".$fmNmKegiatan."' readonly size='84'>"
							:$bk_p.".".$ck_p.".".$dk_p.".".$fmPROGRAM.".".$fmKEGIATAN." - ".$fmNmKegiatan;
				$enKegiatan=formEntryBase2("Kegiatan",":",
											$entrykeg.
											"<input type='hidden'  id='fmKEGIATAN' name='fmKEGIATAN' value='$fmKEGIATAN'>",
											"","","","valign='top' height='24'");
		//end program&kegiatan----------------------------------------------------------------------------------------------------------------------------------------
		}
				$enKondisiBrg=formEntryBase2("Kondisi Barang",":",
											$Main->KondisiBarang[$fmKONDISIBARANG-1][1]
											."<input type='hidden'  id='fmKONDISIBARANG' name='fmKONDISIBARANG' value='$fmKONDISIBARANG'>",
											"","","","valign='top' height='24'");
				$enTglBuku=formEntryBase2('Tanggal Dibukukan',':',
								JuyTgl1($tgl_buku) //createEntryTgl('tgl_buku', false, 'tanggal bulan tahun (mis: 1 Januari 1998). Tanggal pembukuan diisi 31 Desember 2009 sebagai cut off data kecuali pengadaan barang diatas 2009', '','adminForm',1).
							."<input type='hidden'  id='tgl_buku' name='tgl_buku' value='$tgl_buku'>",
							'','','','valign="top" height="24"');

		}else{
			$fmTAHUNPEROLEHAN = ifempty($fmTAHUNPEROLEHAN,$HTTP_COOKIE_VARS['coThnAnggaran']);
			$enThnPerolehan=formEntryBase2('Tahun Perolehan',':',
							'<input type="text" name="fmTAHUNPEROLEHAN" value="'.$fmTAHUNPEROLEHAN.'" size="4" maxlength=4
							onkeypress="return isNumberKey(event)"
							'.($entryMutasi==FALSE? " $htmlreadonly ":' readonly="" ').'
							/>
			...
			$enNoreg=formEntryBase2('Nomor Register',':',
						...
						onkeypress="return isNumberKey(event)"'." $htmlreadonly ".'
						>
						<span style="color:red;">'.$Main->NOREG_SIZE.' digit (mis: '.$Main->NOREG_SAMPLE.')</span>
						'.$btCariNoReg,
						'','','','valign="top" height="24"');
			$enHrgBeli=formEntryBase2('Harga Beli Satuan ',':',
						'Rp.'.inputFormatRibuan3("fmHARGABARANGBELI", ($entryMutasi==FALSE? $htmlreadonly:' readonly="" ')),'','','','valign="top" height="24"');
			$enHrgAtribusi=formEntryBase2('Harga Atribusi Satuan',':',
							'Rp.'.inputFormatRibuan3("fmHARGABARANGATRIBUSI", ($entryMutasi==FALSE? $htmlreadonly:' readonly="" ') ),
							'','','','valign="top" height="24"');
			$enHrgBarang=formEntryBase2('Harga Perolehan Satuan ',':',
							'Rp.'.inputFormatRibuan("fmHARGABARANG", (' readonly="" ') ),'','','','valign="top" height="24"');
			...
			$enAsalUsul=formEntryBase2("Asal Usul/Cara Perolehan",":",
										//cmb2D('fmASALUSUL',$fmASALUSUL,$Main->AsalUsul,''),
										cmb2Dv2('fmASALUSUL',$fmASALUSUL,$Main->AsalUsul,'',$DataReadonly || $entryMutasi),
											"","","","valign='top' height='24'");

			$enJenisHibah=formEntryBase2("Sumber Dana",":",
										//cmb2Dv2('fmJENISHIBAH',$fmJENISHIBAH,$Main->JenisHibah,'',$DataReadonly || $entryMutasi),
										genComboBoxQry('fmJENISHIBAH',$fmJENISHIBAH,
	...
	// program&kegiatan ------------------------------------------------------------------------------------------------------------------------------------------------
			...
			if($Baru==0){
				...
				$bk = $su['bk']; $ck = $su['ck']; $dk = $su['dk'];
			}
			$kdprogram = $fmPROGRAM == '' || $fmPROGRAM == '0'?'':$bk_p.".".$ck_p.".".$dk_p.".".$fmPROGRAM;
			$nmprogram = $fmNmProgram;
			$enProgram=formEntryBase2("Program",":",
				/**
				genComboBoxQry('fmPROGRAM',$fmPROGRAM,
					$queryProgram,
				...
				<input type='hidden'  id='dk' name='dk' value='".$dk."'>".
				"<input type='hidden'  id='fmPROGRAM' name='fmPROGRAM' value='".$fmPROGRAM."'>".
				"<input type='text'  id='kdprogram' name='kdprogram' value='".$kdprogram."' readonly size=6>&nbsp;".
				"<input type='text'  id='nmprogram' name='nmprogram' value='".$nmprogram."' readonly size='84'>&nbsp;".
			...
			//query Kegiatan
			//$queryKegiatan = "SELECT q,nama FROM ref_program WHERE bk='".$prg['bk']."' AND ck='".$prg['ck']."' AND dk='".$prg['dk']."' AND p='".$fmPROGRAM."' AND q!='0'";
			$kdkegiatan = $fmKEGIATAN == '' || $fmKEGIATAN == '0'?'':$bk_p.".".$ck_p.".".$dk_p.".".$fmPROGRAM.".".$fmKEGIATAN;
			$nmkegiatan = $fmNmKegiatan;
			$enKegiatan=formEntryBase2("Kegiatan",":",
				/*genComboBoxQry('fmKEGIATAN',$fmKEGIATAN,
					$queryKegiatan,
					'q', 'nama','Pilih')
					*/
					"<input type='hidden'  id='fmKEGIATAN' name='fmKEGIATAN' value='".$fmKEGIATAN."'>".
					"<input type='text'  id='kdkegiatan' name='kdkegiatan' value='".$kdkegiatan."' readonly size=6>&nbsp;".
					"<input type='text'  id='nmkegiatan' name='nmkegiatan' value='".$nmkegiatan."' readonly size='84'>",
					"","","","valign='top' height='24'"
			);
	// end program&kegiatan -------------------------------------------------------------------------------------------------------------------------------------------
	}
			//kondisi barang-----------------------------------------------------------------------------------------
			$cekKondisi=mysql_fetch_array(mysql_query("select count(*) as cnt from t_kondisi where idbi='".$fmIDLama."' and kond_awal<>0"));
			if($cekKondisi['cnt']>0) {$DataReadonly=TRUE;}
			$enKondisiBrg=formEntryBase2("Kondisi Barang",":",
											cmb2Dv2('fmKONDISIBARANG',$fmKONDISIBARANG,$Main->KondisiBarang,'',$DataReadonly || $entryMutasi),
											"","","","valign='top' height='24'");
			$tgl_buku = ifempty($tgl_buku,$HTTP_COOKIE_VARS['coThnAnggaran']);
			$enTglBuku=formEntryBase2('Tanggal Dibukukan',':',
								createEntryTgl(
									'tgl_buku',
									$tgl_buku,
									($DataReadonly==true || $entryMutasi)?'1':'',
									//($DataReadonly==true )?'1':'',
		...
		/***
		$cek .= ($Baru == 0 && ((sudahClosing($tgl_buku) || ($old['id'] <> $old['idawal'])) || ($old['status_barang']=='2' || $old['staset']=='7')))? '<br>readonly' : '<br>not readonly';
		$cek .= '<br>$baru='.$Baru;
		$cek .= '$old[id]='.$old['id'];
		$cek .= '$old[idawal]='.$old['idawal'];
		$cek .= '$old[status_barang]='.$old['status_barang'];
		$cek .= '$old[staset]='.$old['staset'];
		$cek .= sudahClosing($tgl_buku) ? '<br>sudah closing = TRUE':'<br>sudah closing = FALSE';
		$cek .= '<br> tgl buku ='.$tgl_buku;
		$cek .= '<br> tgl closing ='.getTglClosing();
		$cek .= '<br> compare tgl ='.compareTanggal($tgl_buku, getTglClosing() );
		**/
		$cek = '';
		//Entry BI =============================================================================
		$Entry_BI ="
		...
		<input type='hidden' id='ref_idatribusi' name='ref_idatribusi' value='$ref_idatribusi'>
		</td></tr>
		...
		".formEntryBase2('Tanggal Sensus Barang',':',
			createEntryTgl('tgl_sensus', $tgl_sensus, ($DataReadonly==true || $entryMutasi)?'1':'',
		...
		"<tr><td>Gedung/Ruang</td><td>:</td><td>".
			...
			<script>
				pilihRuang = function(){
					...
					RuangPilih.fmIDBARANG = parent.document.adminForm.fmIDBARANG.value;
		...
		if ($Baru==0){
			...
			//echo "koreksi=".$Main->MODUL_KOREKSI;
			if($Main->MODUL_KOREKSI){
				//koreksi -----------------------------------------------------------
				//global $disModul06, $ridModul06;
				//echo "<br>=$disModul06 =$ridModul06";
				//if ($disModul06 == ''){
					$Kondisi = " where idbi_awal='$idbiawal'";
					//include_once('common/fnkoreksi.php');
					global $Koreksi2; //$Pemanfaat2 = new PemanfaatCls($Main);
					$KoreksiBar =
					"<div id='divKoreksi2' style='margin:8'>".
					$Koreksi2->createScriptJs().
					"<div id='divKoreksi2List'>".
					$Koreksi2->GetList($Kondisi, '', '', 1, 'koptable2', FALSE, 0, FALSE).
					"</div>".
					"</div>";
					$KoreksiBar = createBarPanel('barKoreksi2', 'Koreksi', 25, $KoreksiBar);
				//}
			}
		...
		return $Entry_BI.$DetilKIB.'<br>'.$idbiHidden.$Pemeliharaan.$Pengaman.$PemanfaatBar.$KoreksiBar;
	}
	...
	function Penatausahaan_FormEntry_Kib($kd){
		...
		$fmTGLGUDANG_KIB_C, $fmNOGUDANG_KIB_C, $fmLUAS_KIB_C, $fmSTATUSTANAH_KIB_C,  $fmNOGEDUNG_KIB_C,
		$fmNOKODETANAH_KIB_C,  $fmKET_KIB_C;
		global $fmKONSTRUKSI_KIB_D, $fmPANJANG_KIB_D, $fmLEBAR_KIB_D, $fmLUAS_KIB_D, $fmALAMAT_KIB_D,
		$fmTGLDOKUMEN_KIB_D, $fmNODOKUMEN_KIB_D, $fmSTATUSTANAH_KIB_D, $fmNOKODETANAH_KIB_D, $fmNORUASJALAN_KIB_D, $fmKONDISI_KIB_D,
		$fmKET_KIB_D;
		...
		switch($kd){
			...
			case 1:{//KIB A
				$hsl="
				<tr valign=\"top\">
				<td width='200'>Luas (m<sup>2</sup>)</td>
				<td width='10'>:</td>
				<td>
				...
				".formEntryBase2('Kampung/Komplek',':','<input type="text" name="kampung" value="'.$kampung.'" style="width:187px">',
				'','width=""','','valign="top" height="24"')."
				".formEntryBase2('RT',':','<input type="text" name="rt" value="'.$rt.'" size="4" maxlength="3">',
				'','width=""','','valign="top" height="24"')."
				".formEntryBase2('RW',':','<input type="text" name="rw" value="'.$rw.'" size="4" maxlength="4">',
				'','width=""','','valign="top" height="24"')."
			...
			case 3:{//kib c
				$hsl="<tr valign=\"top\">
				<td width='200'>Kontruksi Bangunan</td>
				<td width='10'>:</td>
				...
				<tr><td>Nomor Gedung</td><td>:</td><td>
				".txtField('fmNOGEDUNG_KIB_C',$fmNOGEDUNG_KIB_C,'100','20','text','')."
				</td></tr>
				<tr valign=\"top\">
			...
			case 4:{//kib d
				...
				<td width=''>:</td>
				<td>".txtField('fmNORUASJALAN_KIB_D',$fmNORUASJALAN_KIB_D,'100','50','text','')."</td>
				</tr>
				<tr valign=\"top\">
				<td >Nomor Kode Tanah</td>
				<td width=''>:</td>
			...
			case 5:{//kib e
				...
				<tr valign=\"top\">
				<td width='200'>&nbsp;&nbsp;&nbsp;&nbsp;Judul/Pencipta</td>
				<td width='10'>:</td>
...
//				'<input type="text" name="alamat_kec" value="'.$alamat_kec.'">'
	...
	function Penatausahaan_GetData($idbi, $hanyaStatusInv = TRUE ){
		...
		$fmLUAS_KIB_C, $fmSTATUSTANAH_KIB_C, $fmNOGEDUNG_KIB_C, $fmNOKODETANAH_KIB_C, $fmKET_KIB_C;
		...
		global $fmSTATUSBARANG,$fmSTATUSASET,$fmHARGABARANGBELI,$fmHARGABARANGATRIBUSI;
		global $tahun_sensus, $ref_idpemegang, $ref_idpemegang2, $ref_idpenanggung, $ref_idruang,$ref_idatribusi;
		global $no_ba, $tgl_ba, $no_spk, $tgl_spk, $penggunabi;
		global $kampung, $rt, $rw;
		global $fmPROGRAM, $fmKEGIATAN, $bk, $ck, $dk, $bk_p, $ck_p, $dk_p, $fmNmProgram, $fmNmKegiatan;
		...
		if ($errmsg == ''){
			...
			$fmJENISHIBAH = $isi['jns_hibah']; //jenis hibah
			...
			$fmSTATUSASET = $isi['staset'];
			$fmHARGABARANGBELI=$isi['harga_beli'];
			$fmHARGABARANGATRIBUSI=$isi['harga_atribusi'];
			$ref_idatribusi=$isi['ref_idatribusi'];
			...
			$fmPROGRAM = $isi['p'];
			$fmKEGIATAN = $isi['q'];
			$fmNmProgram = $isi['nmprogram'];
			$fmNmKegiatan = $isi['nmkegiatan'];
			...
			switch( $isi['f'] ){
				...
				case '03':{
					$fmNOGEDUNG_KIB_C = $isi['no_barang'];
				...
				case '04':{
					$fmNORUASJALAN_KIB_D = $isi['no_barang'];
				...
				case '07':{
					...
					$fmPENCIPTA_KIB_G=$isi['pencipta'];
					$fmJENIS_KIB_G=$isi['jenis'];
					$fmKET_KIB_G=$isi['ket'];
					break;
				}
			}
...
*/
	...
	function Penatausahaan_genTableRowKibC($clRow, $clGaris, $tampilCheckbox,  $isi, $no, $tampilCbxKeranjang='', $tipe='', $ketpenanggungjawab=''){
		global $Main;
		...
		$status_survey=$Main->STATUS_SURVEY==1 ? "<td class=\"$clGaris\" align=center style=\"min-width:50;\">".$ss['nama']."</td>":"";
		$kolTampilSusut=$Main->TAMPIL_SUSUT==1 ? "
			...
			<td class=\"$clGaris\" align=right>".number_format($isi['nilai_susutpertahun'],2,',','.')."</td>
		...
		if ($Main->JNS_EKSTRA==1 && $jns=='ekstra' ){
			...
			$jnsekstra = "<td class=\"$clGaris\" align=center style=\"min-width:50;\">". $Main->arrJnsEkstra[$biekstra['jns_ekstra']][1] ."</td>";
		}
		...
		$vkegiatan = $isi['bk_p']==0 && $isi['p']==0 ? '' : "$keg";
		$vbast = "<td class=\"$clGaris\" >".$isi['no_ba']."/<br>".TglInd($isi['tgl_ba'])."/<br>$vkegiatan</td>";
		$vspk = "<td class=\"$clGaris\" >".$isi['no_spk']."/<br>".TglInd($isi['tgl_spk'])."</td>";
		...
		if($tipe=='kertaskerja'){
			return
				...
				<td class=\"$clGaris\" align=center><div class='nfmt3'>".$isi['noreg'].$vNoGedung."</div></td>
			...
			return
				...
				<td class=\"$clGaris\" align=center>".$no."</td>
				$tampilCheckbox
				<td class=\"$clGaris\" align=left>".$tampilIdBrg."/<br>{$isi['idbi']}/<br>{$isi['idawal']}</td>
				<td class=\"$clGaris\" align=center><div class='nfmt3'>".$isi['noreg'].$vNoGedung."</div></td>
				<td class=\"$clGaris\">".$isi['nm_barang']."</td>
				<td class=\"$clGaris\" align=center>".$isi['tahun']."</td>
				<td class=\"$clGaris\">".$vKondisi."</td>
				...
				<td class=\"$clGaris\" align=right>".$vluas."</td>
				<td class=\"$clGaris\">".$vStatus."</td>
				<td class=\"$clGaris\" align=left>".$vKodeTanah."</td>
				<td class=\"$clGaris\" style='min-width:100'>".$vAsal."</td>
				<td class=\"$clGaris\" align=right>".$tampilHarga."</td>
				$vbast
				$vspk
				<td class=\"$clGaris\">".$ket.$ketgbr.$ketpenanggungjawab.'<br>'.$isi['vBidang']."</td>".
				$tampilCbxKeranjang.
				//$isi['vBidang'].
				$status_survey.
				$jnsekstra.
	...
	function Penatausahaan_genTableRowJml($clRow, $clGaris, $colspan, $arr, $susut=FALSE,$fmKIB, $idBI='', $colapse=TRUE , $cetak=FALSE ){
		global $Main;

		if($cetak == FALSE){
			$ids = $colapse ? "det_$idBI" : '';
			$toggler =  $colapse ==FALSE ?  "<a href='#' class='toggler' data-prod-cat='".$idBI."' state='colapse'> ".
				"<img id='toggler_img' src='images/tumbs/down_2.png' style='cursor:pointer;width:10px'> </a>" : '';

			$display = $colapse ? 'none' : 'table-row' ;
		}

		//$colspan = $isTampilCheckbox? 13 :12;
		$vstatussurvey = $Main->STATUS_SURVEY? "<td class=\"$clGaris\"></td>" :'';
		$kolTampilSusut = $Main->TAMPIL_SUSUT && $fmKIB != '01'? "
				<td class=\"$clGaris\">&nbsp;</td>
				<td class=\"$clGaris\">&nbsp;</td>
				<td class=\"$clGaris\">&nbsp;</td>
				<td class=\"$clGaris\">&nbsp;</td>
				...
				<td class=\"$clGaris\">&nbsp;</td>
				" :'';
		if($Main->MODUL_BAST) $vbast	= "<td class=\"$clGaris\"></td>";
		if($Main->MODUL_SPK)$vspk	= "<td class=\"$clGaris\"></td>";
		if($susut==FALSE){
			//$colspan = 11;
			...
			if($fmKIB=='sensusAda') {
				return
					"<tr class='$clRow' valign='top' id='$ids' style='display:$display'>
					<td class=\"$clGaris\" colspan='$colspan'> ".$arr[0]."</td>
					<td class=\"$clGaris\" align=right>".$arr[1]."</td>
					<td class=\"$clGaris\">$toggler</td>
					$vstatussurvey
					$vbast
					$vspk
					$kolTampilSusut
	...
	function Penatausahaan_genTableRowBIJml($clRow, $clGaris, $tampilCheckbox, $arr, $susut=FALSE){
		if($susut==FALSE){
			return
			"<tr class='$clRow' valign='top'>
...
*/
	...
	function Penatausahaan_genTableRowKibA($clRow, $clGaris, $tampilCheckbox,  $isi, $no=0, $tampilCbxKeranjang='',$tipe='', $ketpenanggungjawab=''){
		global $Main;
		$nm_kota="";
		$nm_kec ="";
		$get = mysql_fetch_array(mysql_query(
		...
		$jnshibah = $isi['jns_hibah'] == '0' ? '' : $isi['jns_hibah'];
		$asal = ifempty( $Main->AsalUsul[$isi['asal_usul']-1][1], '-')."<br>/".
			ifempty( $jnshibah, '-')."<br>/".
			ifempty( $Main->StatusBarang[$isi['status_barang']-1][1], '-')."<br>/".
			ifempty( $Main->KondisiBarang[$isi['kondisi']-1][1], '-');
		...
		$gbr = mysql_fetch_array(mysql_query("select count(*) as cnt from gambar where idbi='".$isi['idawal']."'"));
		$ketgbr = "<br> Gambar : ".$gbr['cnt'];
		//$ketgbr = "<br> Gambar : ".$isi['jml_gambar'];
		//ambil data ref status survey
		$ss = mysql_fetch_array(mysql_query("select * from ref_statusbarang2 where Id='".$isi['ref_idstatussurvey']."'"));
		$status_survey=$Main->STATUS_SURVEY==1 ? "<td class=\"$clGaris\" align=center style=\"min-width:50;\">".$ss['nama']."</td>":"";
		...
		if($tipe=='kertaskerja'){
			return
				...
				<td class=\"$clGaris\"><div class='nfmt5'>".$sertifikatNo."</div></td>
				<td class=\"$clGaris\">".$penggunaan."</td>
				<td class=\"$clGaris\">".$asal."</td>
				<td class=\"$clGaris\" align=right>".$tampilHarga."</td>
				<td class=\"$clGaris\"  style='min-width:120'>".$status_penguasaan."</td>".
				"<td class=\"$clGaris\">".$dttd."</td>".
				"<td class=\"$clGaris\">".$keterangan.'<br>'.$isi['vBidang']."</td>".
				//$isi['vBidang'].
				$tampilCbxKeranjang.
				"</tr>";
		}
	...
	function Penatausahaan_genTableRowDet($fmKIB,$clRow, $clGaris, $tampilCheckbox, $arr, $susut=FALSE, $idBI='', $colapse = TRUE, $cetak=FALSE){
		global $Main;

		//$ids = $withtoggler  ? "det_$idBI" : '';
		/**$ids = $colapse && $cetak==FALSE  ? "det_$idBI" : '';
		$toggler =  $colapse && $cetak==FALSE ?  "<a href='#' class='toggler' data-prod-cat='".$idBI."' state='colapse'> <img id='toggler_img' src='images/tumbs/down_2.png' style='cursor:pointer;width:10px'> </a>" : '';
		$display = $colapse && $cetak==FALSE  ? 'none' : 'table-row' ;
		$display = $cetak ? 'table-row' : 'none';
		**/
		...
		if($Main->STATUS_SURVEY==1) $kolsurvey = "<td class=\"$clGaris\"></td>";
		if($Main->TAMPIL_SUSUT==1) $koltampilsusut = "
		<td class=\"$clGaris\">&nbsp;</td>
		<td class=\"$clGaris\">&nbsp;</td>
		<td class=\"$clGaris\">&nbsp;</td>
		<td class=\"$clGaris\">&nbsp;</td>
		...
		switch($fmKIB){
			case '01':{
				return "<tr class='$clRow' valign='top' id='$ids' style='display:$display'>
				<td class=\"$clGaris\" align=center></td>
				$tampilCheckbox
				<td class=\"$clGaris\" align=center>".$arr[0]."</td>
				<td class=\"$clGaris\" align=center>".$arr[1]."</td>
				<td class=\"$clGaris\" align=left>".$arr[2]."</td>
				<td class=\"$clGaris\" align=right>".$arr[5]."</td>
				<td class=\"$clGaris\" align=center>".$arr[4]."</td>
				<td class=\"$clGaris\" colspan=4>".$arr[3]."</td>
				<td class=\"$clGaris\"></td>
				<td class=\"$clGaris\"></td>
				<td class=\"$clGaris\" align=right>".$arr[6]."</td>
				$vbast
				$vspk
				<td class=\"$clGaris\">".$arr[8]."/ ".$arr[7]."</td>
				$kolsurvey
				</tr>";
				break;
			}
			case '02':{
				...
				<td class=\"$clGaris\" align=center>".$arr[1]."</td>
				<td class=\"$clGaris\" align=left>".$arr[2]."</td>
				<td class=\"$clGaris\" colspan=2>".$arr[3]."</td>
				<td class=\"$clGaris\">".$arr[5]."</td>
				<td class=\"$clGaris\" align=center>".$arr[4]."</td>
			...
			case '03':{
				...
				<td class=\"$clGaris\" style='width:75' colspan=4>".$arr[3]."</td>
				<td class=\"$clGaris\" ></td>
				<td class=\"$clGaris\" align=center></td>
				<td class=\"$clGaris\"></td>
				<td class=\"$clGaris\" align=center ></td>
			...
			case '04':{
				...
				<td class='$clGaris' >".$arr[2]."</td>
				<td class='$clGaris' colspan=6>".$arr[3]."</td>
				<td class='$clGaris' align=center></td>
				<td class='$clGaris' align=center></td>
				<td class='$clGaris' ></td>
				<td class='$clGaris' ></td>
			...
			case '05':{
				...
				<td class=\"$clGaris\" align=center>".$arr[1]."</td>
				<td class=\"$clGaris\" >".$arr[2]."</td>
				<td class=\"$clGaris\" colspan=5>".$arr[3]."</td>
				<!--<td class=\"$clGaris\" ></td>
				<td class=\"$clGaris\" ></td>
				<td class=\"$clGaris\" ></td>
				<td class=\"$clGaris\" ></td>-->
		...
		$cbxDlmRibu,$clRow, $clGaris, $tampilCheckbox, $cp2=1, $kondTglAkhir='')
	{
		...
		if ($txls=="1"){
			...
			switch ($fmKIB){
				...
				case '07': $colspan = $cetak ? 13: 14; break;
...
function Penatausahaan_GetListDet2($idBI, $fmKIB,
	...
	function insertIntoHistory( $st, $newid, $old, $uid, $ket='' ){ //$old = query result old bi
		//tgl_update,id_bukuinduk,a1,a,b,c,d,e,e1,f,g,h,i,j,noreg,tahun,satuan,harga,asal_usul,kondisi,status_barang,staset,tgl_buku,idall2,tgl_sensus,tahun_sensus,id_lama,idawal,ref_idpemegang,ref_idpenanggung,ref_idpemegang2,harga_atribusi,harga_beli,ref_idatribusi,kejadian,uid
		$new = mysql_fetch_array(mysql_query("select * from buku_induk where id = $newid"));
		switch( $st ){
			case '1':{//baru

				$InsertHistory = mysql_query(
					"insert into t_history (
					tgl_update,id_bukuinduk,a1,a,b,
					c,d,e,e1,f,g,h,i,j,
					noreg,tahun,
					satuan,harga,asal_usul,kondisi,
					status_barang,staset,tgl_buku,idall2,
					tgl_sensus,tahun_sensus,id_lama,idawal,
					ref_idpemegang,ref_idpenanggung,ref_idpemegang2,
					harga_atribusi,harga_beli,ref_idatribusi,
					kejadian,uid, st
					)values(
					now(),'$newid','".$new['a1']."','".$new['a']."','".$new['b']."',
					'".$new['c']."','".$new['d']."','".$new['e']."','".$new['e1']."',
					'".$new['f']."','".$new['g']."','".$new['h']."','".$new['i']."','".$new['j']."',
					'".$new['noreg']."','".$new['tahun']."',
					'".$new['satuan']."','".$new['harga']."','".$new['asal_usul']."','".$new['kondisi']."',
					'".$new['status_barang']."','".$new['staset']."','".$new['tgl_buku']."','".$new['idall2']."',
					'".$new['tgl_sensus']."','".$new['tahun_sensus']."','".$new['id_lama']."','".$new['idawal']."',
					'".$new['ref_idpemegang']."','".$new['ref_idpenanggung']."','".$new['ref_idpemegang2']."',
					'".$new['harga_atribusi']."','".$new['harga_beli']."','".$new['ref_idatribusi']."',
					'".$ket."','".$uid."'".
					",1)"
				);
				break;
			...
			case '2':{ //edit
				if(
					...
					($old['noreg']!= $new['noreg']) ||
					($old['tahun']!= $new['tahun']) ||
					($old['satuan']!= $new['satuan']) ||
					($old['harga']!= $new['harga']) ||
					($old['asal_usul']!= $new['asal_usul']) ||
					($old['kondisi']!= $new['kondisi']) ||
					($old['status_barang']!= $new['status_barang']) ||
					($old['staset']!= $new['staset']) ||
					($old['tgl_buku']!= $new['tgl_buku']) ||
					//($old['idall2']!= $new['idall2']) ||
					($old['tgl_sensus']!= $new['tgl_sensus']) ||
					($old['tahun_sensus']!= $new['tahun_sensus']) ||
					($old['id_lama']!= $new['id_lama']) ||
					($old['idawal']!= $new['idawal']) ||
					($old['ref_idpemegang']!= $new['ref_idpemegang']) ||
					($old['ref_idpenanggung']!= $new['ref_idpenanggung']) ||
					($old['ref_idpemegang2']!= $new['ref_idpemegang2']) ||
					($old['harga_atribusi']!= $new['harga_atribusi']) ||
					($old['harga_beli']!= $new['harga_beli']) ||
					($old['ref_idatribusi']!= $new['ref_idatribusi'])
				){
					$InsertHistory = mysql_query(
						"insert into t_history (
						tgl_update,id_bukuinduk,a1,a,b,
						...
						'".$new['satuan']."','".$new['harga']."','".$new['asal_usul']."','".$new['kondisi']."',
						'".$new['status_barang']."','".$new['staset']."','".$new['tgl_buku']."','".$new['idall2']."',
						'".$new['tgl_sensus']."','".$new['tahun_sensus']."','','$newid',
						'".$new['ref_idpemegang']."','".$new['ref_idpenanggung']."','".$new['ref_idpemegang2']."',
						'".$new['harga_atribusi']."','".$new['harga_beli']."','".$new['ref_idatribusi']."',
						'".$ket."','".$uid."'".
						",2)"
			...
			case '3':{ //delete
				$InsertHistory = mysql_query(
					...
					now(),'".$old['id']."','".$old['a1']."','".$old['a']."','".$old['b']."',
					'".$old['c']."','".$old['d']."','".$old['e']."','".$old['e1']."',
					'".$old['f']."','".$old['g']."','".$old['h']."','".$old['i']."','".$old['j']."',
					'".$old['noreg']."','".$old['tahun']."',
					'".$old['satuan']."','".$old['harga']."','".$old['asal_usul']."','".$old['kondisi']."',
					'".$old['status_barang']."','".$old['staset']."','".$old['tgl_buku']."','".$old['idall2']."',
					'".$old['tgl_sensus']."','".$old['tahun_sensus']."','".$old['id_lama']."','".$old['idawal']."',
					'".$old['ref_idpemegang']."','".$old['ref_idpenanggung']."','".$old['ref_idpemegang2']."',
					'".$old['harga_atribusi']."','".$old['harga_beli']."','".$old['ref_idatribusi']."',
					'".$ket."','".$uid."'".
					",3)"
				);
				break;
	...
	function jurnalPenghapusan($idhps, $uid, $st=1, $jns_trans=7, $jns_trans2 = 6, $refid='' ){
		$err = ''; $cek='';
		$plh = mysql_fetch_array(mysql_query( "select * from penghapusan where id='".$idhps."' "));
		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id='".$plh['id_bukuinduk']."'"));

		$jns_trans2 = $jns_trans2 == ''? 6: $jns_trans2; //penghapusan
		$refid= $refid==''? $idhps: $refid;//$bi['id_lama'];
		$tgl_buku = $plh['tgl_penghapusan'];
		$harga_atribusi=0;

		switch($st){
			case 1: {
				// - penghapusan bi
				if($plh['mutasi']==0){ //penghapusan
					//$jns_trans= $jns_trans == ''? 7: $jns_trans; //rekap penghapusan
					$jns_trans = 7;
					$debet=0; $kredit=$bi['harga'];
					$jml_barang_d=0; $jml_barang_k=$bi['jml_barang'];
					$staset = $plh['staset'];
					$jns_trans3=1; //daftar buku_induk
					$refid2 = $bi['id'];
					$kd = jurnalGetKodeRekap($staset, $bi['f'], $bi['g']);
					$kint=$kd['kint']; $ka=$kd['ka']; $kb=$kd['kb']; $f=$bi['f']; $g=$bi['g'];
					$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );

					jurnalInsertRehab(
						FALSE,$bi['idawal'], $tgl_buku, $staset, $jns_trans, $jns_trans2, $refid,
						$uid,$bi,'', $kint, $ka, $kb , $f, $g
					);
				}else if($plh['mutasi']==1){ // mutasi
					$jns_trans = 5; //rekap mutasi kredit
					$debet=0; $kredit=$bi['harga'];
					$jml_barang_d=0; $jml_barang_k=$bi['jml_barang'];
					$staset = $plh['staset'];
					$jns_trans3=1; //daftar buku_induk
				...
				}else if($plh['mutasi']==2){ // mutasi
					$jns_trans = 9; //rekap reclass kredit
					$debet=0; $kredit=$bi['harga'];
					$jml_barang_d=0; $jml_barang_k=$bi['jml_barang'];
					$staset = $plh['staset'];
					$jns_trans3=1; //daftar buku_induk
				...
				/*
				// - penghapusan pemeliharaan lama
				$aqry = "select * from pemeliharaan where idbi_awal ='".$bi['idawal']."' and tambah_aset=1 and tgl_pemeliharaan<='". $tgl_buku."' "; $cek .= $aqry;
				$qrp = mysql_query($aqry);
				while ($isi = mysql_fetch_array($qrp)){
					//$jns_trans=7; //group reclass/hapus
					$debet=0; $kredit=$isi['biaya_pemeliharaan'];
					$jml_barang_d=0; $jml_barang_k=0;
					//$staset = 0;
					$jns_trans3=3;
					$refid2 = $isi['id'];
					$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
				...
				// - penghapusan pengaman lama
				$qrp = mysql_query("select * from pengamanan where idbi_awal ='".$bi['idawal']."' and tambah_aset=1 and tgl_pengamanan<='". $tgl_buku."' ");
				while ($isi = mysql_fetch_array($qrp)){
					//$jns_trans=7; //$jns_trans2 = 6;
					$debet=0; $kredit=$isi['biaya_pengamanan'];
					$jml_barang_d=0; $jml_barang_k=0;
					//$staset = 0;
					$jns_trans3=4;
					$refid2 = $isi['id'];
				...
				// - penghapusan sebagian lama
				$qrp = mysql_query("select * from penghapusan_sebagian where idbi_awal ='".$bi['idawal']."' and tgl_penghapusan<='". $tgl_buku."' ");
				while ($isi = mysql_fetch_array($qrp)){
					//$jns_trans=3; //$jns_trans2 = 6;
					$debet=0; $kredit=-$isi['harga_hapus'];//?
					$jml_barang_d=0; $jml_barang_k=0;
					//$staset = 0;
					$jns_trans3=5;
					$refid2 = $isi['Id'];
			...
			case 2: {
				$aqry = "update t_jurnal ".
					"set tgl_buku = '$tgl_buku', ".
						//"debet=$debet, ".
						//"tambah_aset=$tambah_aset,".
						"uid='$uid', ".
						"tgl_update=now() ".
					"where refid='$refid' and jns_trans2=$jns_trans2 ";
				mysql_query($aqry); $cek .=$aqry;
				break;
			}
			case 3: { //hapus penghapusan
				$aqry = "delete from t_jurnal ".
					"where refid='$refid' and jns_trans2=$jns_trans2 ";
				mysql_query($aqry); $cek .=$aqry;
				break;
			}
	...
	function jurnalReclass($idbilama, $idbibaru, $uid){
		$err = ''; $cek='';

		$hps = mysql_fetch_array(mysql_query("select * from penghapusan where id_bukuinduk='$idbilama' "));
		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id ='$idbibaru'"));
		$biold= mysql_fetch_array(mysql_query("select * from buku_induk where id ='$idbilama'"));
		$jns_trans=9;//kolom reclass
		$jns_trans2 = 8;//reclass
		$refid=$bi['id'];
		//hapus bi lama -------------------------------------------------------------------------------
		jurnalPenghapusan($hps['id'], $uid, 1, $jns_trans, $jns_trans2, $refid);

		// - reclass bi baru --------------------------------------------------------------------------
		$debet=$bi['harga']; $kredit=0;
		$jml_barang_d=$bi['jml_barang'];  $jml_barang_k=0;
		$harga_atribusi=0;

		$tgl_buku = $bi['tgl_buku'];
		$staset = $bi['staset'];
		$jns_trans3=1;$refid2 = $bi['id'];
		$kd = jurnalGetKodeRekap($staset, $bi['f'], $bi['g']);
		$kint=$kd['kint']; $ka=$kd['ka']; $kb=$kd['kb']; $f=$bi['f']; $g=$bi['g'];
		$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
		jurnalInsertRehab(
			TRUE, $bi['idawal'], $tgl_buku, $staset, $jns_trans, $jns_trans2, $refid,
			$uid,$bi,$status_barang, $kint, $ka, $kb , $f, $g
		);
	...
	function jurnalPerolehan($idbi, $uid){
		$err = ''; $cek='';
		#tgl_buku,staset,jns_trans,debet,kredit,jml_barang_d,jml_barang_k,harga_atribusi,a1,a,b,c,d,e,e1,f,g,h,i,j,asal_usul,status_barang,idbi,idawal,jns_trans2
		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id='$idbi'"));
		//echo $bi['id'].'!='.$bi['id_lama'] .' MUTASI';
		if($bi['id']!=$bi['id_lama'] && $bi['id_lama']!=''){
			//echo 'MUTASI';
			$hps = mysql_fetch_array(mysql_query( "select * from penghapusan where id_bukuinduk=".$bi['id_lama']." "));
			$biold = mysql_fetch_array(mysql_query( "select * from buku_induk where id=".$bi['id_lama']." " ));
			//echo " hps[mutasi] = ".$hps['mutasi'];
			switch($hps['mutasi']){
				case '1':{ //mutasi
					// - mutasi bi baru --------------------------------------------------------------
					$jns_trans=5; $jns_trans2 = 7;
					$debet=$bi['harga']; $kredit=0;
					$jml_barang_d=$bi['jml_barang'];  $jml_barang_k=0;
					$harga_atribusi=0;
					$refid=$bi['id'];
			...
			switch ($bi['asal_usul'] ){
				case '1' : {//pembelian
					//switch($bi['staset']{
					/*if($bi['staset'] == 10 ) { //perolehan extra
						$jns_trans=1; $jns_trans2 = 1;
						$debet=$bi['harga']; $kredit=0;
						$refid=$bi['id'];
						$bi['idawal']=$bi['id'];
						$tgl_buku = $bi['tgl_buku'];
						...
						$staset = 3;
						$jml_barang_d=$bi['jml_barang'];  $jml_barang_k=0;
						$harga_atribusi=$bi['harga_atribusi'];
						$jns_trans3=1;$refid2=$bi['id'];
						$kd = jurnalGetKodeRekap($staset, $bi['f'], $bi['g']);
						$kint=$kd['kint']; $ka=$kd['ka']; $kb=$kd['kb']; $f=$bi['f']; $g=$bi['g'];
						$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
						...
						$jml_barang_d=0;  $jml_barang_k=$bi['jml_barang'];
						$jns_trans3= 1; $refid2=$bi['id'];
						$kd = jurnalGetKodeRekap(3, $bi['f'], $bi['g']);
						$kint=$kd['kint']; $ka=$kd['ka']; $kb=$kd['kb']; $f=$bi['f']; $g=$bi['g'];
						$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
						//jurnal extra debet
						$staset = 10;
				...
				case '3' : { //lainnya
					...
					/*$aqry = "insert into t_jurnal ".
						"(tgl_buku,staset,jns_trans,debet,kredit,jml_barang_d,jml_barang_k,harga_atribusi,a1,a,b,c,d,e,e1,f,g,h,i,j,asal_usul,status_barang,idbi,idawal,jns_trans2,uid,tgl_update,refid) ".
						"values (".
						"'".$bi['tgl_buku']."','".$bi['staset']."',1,'".$bi['harga']."',0,1,0,'".$bi['harga_atribusi']."','".
						$bi['a1']."','".$bi['a']."','".$bi['b']."','".$bi['c']."','".$bi['d']."','".$bi['e']."','".$bi['e1']."','".
						$bi['f']."','".$bi['g']."','".$bi['h']."', '".$bi['i']."','".$bi['j']."','".$bi['asal_usul']."','".$bi['status_barang']."','".$bi['id']."','".$bi['id']."',9,'$uid',now(),{$bi['id']})";
					mysql_query($aqry); $cek .=$aqry;*/
					break;
				}
			...
			if($bi['staset'] == 10 ) { //perolehan extra
				...
				//jurnal extra kredit
				$staset = 10;
				$jns_trans=3; //koolom kapitalisasi
				$debet=0; $kredit=$bi['harga'];
				$jml_barang_d=0;  $jml_barang_k=$bi['jml_barang'];
				$jns_trans3= 1; $refid2=$bi['id'];
				$kd = jurnalGetKodeRekap(3, $bi['f'], $bi['g']);
			...
			if($bi['staset'] == 9 ) { //perolehan aset lain-lain
				$jns_trans=1; $jns_trans2 = 1;
				$debet=$bi['harga']; $kredit=0;
				$refid=$bi['id'];
				$bi['idawal']=$bi['id'];
				...
				$debet=$bi['harga']; $kredit=0;
				$jml_barang_d=$bi['jml_barang'];  $jml_barang_k=0;
				$kd = jurnalGetKodeRekap($staset, $f, $g);
				$kint=$kd['kint']; $ka=$kd['ka']; $kb=$kd['kb'];
				$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
	...
	function jurnalPemeliharaan($idpelihara,$uid, $st=1){
	/** $st= 1 baru, 2 edit, 3=hapus **/
		$cek = ''; $err='';
		$plh = mysql_fetch_array(mysql_query("select * from pemeliharaan where id='$idpelihara' "));
		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id='{$plh['id_bukuinduk']}' "));
		$jns_trans=3;
		$debet=$plh['biaya_pemeliharaan']; $kredit=0;
		$jml_barang_d=0; $jml_barang_k=0;
		$harga_atribusi=0;
		$jns_trans2 = 3;
		$refid=$idpelihara;
		$tgl_buku = $plh['tgl_pemeliharaan'];
		$staset = 0;
		//$tambah_aset = $plh['tambah_aset'];
		switch($st){
			case 1 :{ //baru
				//$jns_trans=5; $jns_trans2 = 7;
					//$debet=$bi['harga']; $kredit=0;
					...
					//$tgl_buku = $bi['tgl_buku'];
					//$staset = $bi['staset'];
					$jns_trans3=3; $refid2=$refid;
					$kd = jurnalGetKodeRekap($bi['staset'], $bi['f'], $bi['g']);
					$kint=$kd['kint']; $ka=$kd['ka']; $kb=$kd['kb']; $f=$bi['f']; $g=$bi['g'];
					$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
				...
				/*$aqry = "insert into t_jurnal ".
					"(tgl_buku,staset,jns_trans,debet,kredit,jml_barang_d,jml_barang_k,harga_atribusi,a1,a,b,c,d,e,e1,f,g,h,i,j,asal_usul,status_barang,idbi,idawal,jns_trans2,uid,tgl_update,refid) ".
					"values (".
					"'$tgl_buku',$staset,$jns_trans,$debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi,'".
					$bi['a1']."','".$bi['a']."','".$bi['b']."','".$bi['c']."','".$bi['d']."','".$bi['e']."','".$bi['e1']."','".
					$bi['f']."','".$bi['g']."','".$bi['h']."', '".$bi['i']."','".$bi['j']."','".$bi['asal_usul']."','".
					$bi['status_barang']."','".$bi['id']."','".$bi['idawal']."',$jns_trans2,'$uid',now(),$refid)";
				mysql_query($aqry); $cek .=$aqry;*/
				break;
			}
			case 2 :{ //edit
	...
	function jurnalPengamanan($id,$uid, $st=1){
	/** $st= 1 baru, 2 edit, 3=hapus **/
		$cek = ''; $err='';
		$plh = mysql_fetch_array(mysql_query("select * from pengamanan where id='$id' "));
		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id='{$plh['id_bukuinduk']}' "));
		$jns_trans=3;
		$debet=$plh['biaya_pengamanan']; $kredit=0;
		$jml_barang_d=0; $jml_barang_k=0;
		$harga_atribusi=0;
		$jns_trans2 = 4;
		$refid=$id;
		$tgl_buku = $plh['tgl_pengamanan'];
		$staset = 0;
		//$tambah_aset = $plh['tambah_aset'];
		switch($st){
			case 1 :{ //baru
				//if()
				$jns_trans3=4; $refid2=$refid;
				$kd = jurnalGetKodeRekap($bi['staset'], $bi['f'], $bi['g']);
				$kint=$kd['kint']; $ka=$kd['ka']; $kb=$kd['kb']; $f=$bi['f']; $g=$bi['g'];
				$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
	...
	function jurnalHapusSebagian($id,$uid, $st=1){
	/** $st= 1 baru, 2 edit, 3=hapus **/
		$cek = ''; $err='';
		$plh = mysql_fetch_array(mysql_query("select * from penghapusan_sebagian where id='$id' "));
		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id='{$plh['id_bukuinduk']}' "));
		$jns_trans=3;
		$debet=0; $kredit=$plh['harga_hapus'];
		$jml_barang_d=0; $jml_barang_k=0;
		$harga_atribusi=0;
		$jns_trans2 = 5;
		$refid=$id;
	...
	function jurnalInsert(
		$tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2,
		...
		$f = $f==''? $bi['f'] : $f;
		$g = $g==''? $bi['g'] : $g;
		$status_barang = $status_barang==''? $bi['status_barang'] : $status_barang;

		$aqry = "insert into t_jurnal ".
			"(tgl_buku, kint, ka, kb, staset,jns_trans,jns_trans2,refid,jns_trans3,refid2,".
			"debet,kredit,jml_barang_d,jml_barang_k,harga_atribusi,".
			"a1,a,b,c,d,e,e1,f,g,h,i,j,asal_usul,status_barang,".
			"idbi,idawal,uid,tgl_update) ".
			"values (".
			"'$tgl_buku', '$kint', '$ka', '$kb', $staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, ".
			"$debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi,'".
			$bi['a1']."','".$bi['a']."','".$bi['b']."','".$bi['c']."','".$bi['d']."','".$bi['e']."','".$bi['e1']."','".
			$f."','".$g."','".$bi['h']."', '".$bi['i']."','".$bi['j']."','".$bi['asal_usul']."','".$status_barang."','".
			$bi['id']."','".$bi['idawal']."','$uid',now())"; $cek .=$aqry;
		$qry = mysql_query($aqry);
		if(!$qry) $err.= 'gagal '.mysql_error();
		return array ('cek'=>$cek, 'err'=>$err);
	}


	function jurnalPemanfaatan($idmanfaat,$uid,$st){
		$cek = ''; $err='';

		$plh = mysql_fetch_array(mysql_query("select * from pemanfaatan where id='$idmanfaat' "));
		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id='{$plh['id_bukuinduk']}' "));
		//$jns_trans=3;


		$harga_atribusi=0;
		$jns_trans2 = 12; //pemanfaatan
		$refid=$idmanfaat;
		$tgl_buku = $plh['tgl_pemanfaatan'];

		switch($st){
			case 1 :{ //baru
				$jns_trans=9; //group reclass
				$harga_atribusi=0;
				//kredit lama ----------------------------------------------
				$debet=0; $kredit=$bi['harga'];
				$jml_barang_d=0; $jml_barang_k=$bi['jml_barang'];
				$staset = $plh['staset'];
				$status_barang = 1; //$bi['status_barang']
				$jns_trans3=1; //daftar buku_induk
				$refid2 = $bi['id'];
				$kd = jurnalGetKodeRekap($staset, $bi['f'], $bi['g']);
				$kint=$kd['kint']; $ka=$kd['ka']; $kb=$kd['kb']; $f=$bi['f']; $g=$bi['g'];
				...
				//debet baru ----------------------------------------------------
				$kint = '01' ; $ka='02'; $kb='03'; $f='07' ; $g='23';
				$debet=$bi['harga'];  $kredit=0;
				$jml_barang_d=$bi['jml_barang'];  $jml_barang_k=0;
				$staset = 7;
				$status_barang = $bi['status_barang'];
				$jns_trans3=1; //daftar hapus sebagian
				$refid2 = $bi['id'];
				$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
				jurnalInsertRehab(
					TRUE, $bi['idawal'], $tgl_buku, $staset, $jns_trans, $jns_trans2, $refid,
			...
			case 2 :{
				mysql_query("update t_jurnal set tgl_buku='$tgl_buku'  where refid='$idmanfaat' and jns_trans2='$jns_trans2' ");
				break;
			}
			case 3 :{
				mysql_query("delete from t_jurnal where refid='$idmanfaat' and jns_trans2='$jns_trans2' ");
				break;
			}
		}
	...
	function jurnalPemindahtangan($idpindah,$uid,$st){
		$cek = ''; $err='';
		$plh = mysql_fetch_array(mysql_query("select * from pemindahtanganan where id='$idpindah' "));
		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id='{$plh['id_bukuinduk']}' "));

		$harga_atribusi=0;
		$jns_trans2 = 10; //
		$refid=$idpindah;
		$tgl_buku = $plh['tgl_pemindahtanganan'];

		switch($st){
			case 1 :{ //pemidahtangan kredit
				$jns_trans=$plh['bentuk_pemindahtanganan']==3? 4 : 9; //group reclass/hapus
				$harga_atribusi=0;
				//kredit lama ----------------------------------------------
				$debet=0; $kredit=$bi['harga'];
				$jml_barang_d=0; $jml_barang_k=$bi['jml_barang'];
				...
				//echo $ins['err'];
				/*// - kredit pemeliharaan lama
				$aqry = "select * from pemeliharaan where idbi_awal ='".$bi['idawal']."' and tambah_aset=1 and tgl_pemeliharaan<='". $tgl_buku."' "; $cek .= $aqry;
				$qrp = mysql_query($aqry);
				while ($isi = mysql_fetch_array($qrp)){
					$debet=0; $kredit=$isi['biaya_pemeliharaan'];
					$jml_barang_d=0; $jml_barang_k=0;
					$jns_trans3=3; //daftar pemeliharaan
					$refid2 = $isi['id'];
					$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
				}
				// - kredit  pengaman lama
				$qrp = mysql_query("select * from pengamanan where idbi_awal ='".$bi['idawal']."' and tambah_aset=1 and tgl_pengamanan<='". $tgl_buku."' ");
				while ($isi = mysql_fetch_array($qrp)){
					$debet=0; $kredit=$isi['biaya_pengamanan'];
					$jml_barang_d=0; $jml_barang_k=0;
					$jns_trans3=4; //daftar pengamanan
					$refid2 = $isi['id'];
					$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
				}
				// - kredit hapus sebagian lama
				$qrp = mysql_query("select * from penghapusan_sebagian where idbi_awal ='".$bi['idawal']."' and tgl_penghapusan<='". $tgl_buku."' ");
				while ($isi = mysql_fetch_array($qrp)){
					$debet=$isi['harga_hapus']; $kredit=0;
					$jml_barang_d=0; $jml_barang_k=0;
					$jns_trans3=5; //daftar hapus sebagian
					$refid2 = $isi['Id'];
					$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
				}
				*/
				if ($plh['bentuk_pemindahtanganan']==1){
					//debet baru ----------------------------------------------------
					$kint = '01' ; $ka='02'; $kb='01'; $f='07' ; $g='21';
					$debet=$bi['harga'];  $kredit=0; $jml_barang_d=$bi['jml_barang'];  $jml_barang_k=0;
					$staset = 5; $status_barang = $bi['status_barang'];
					$jns_trans3=1; $refid2 = $bi['id'];
					$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
					jurnalInsertRehab(
						TRUE, $bi['idawal'], $tgl_buku, $staset, $jns_trans, $jns_trans2, $refid,
					...
					/*// - debet pemeliharan
					$qrp = mysql_query("select * from pemeliharaan where idbi_awal ='".$bi['idawal']."' and tambah_aset=1 and tgl_pemeliharaan<='". $tgl_buku."' ");
					while ($isi = mysql_fetch_array($qrp)){
						//$jns_trans=3; //$jns_trans2 = 6;
						$debet=$isi['biaya_pemeliharaan']; $kredit=0;
						$jml_barang_d=0; $jml_barang_k=0;
						$jns_trans3=3; //daftar hapus sebagian
						$refid2 = $isi['id'];
						$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
					}
					// - debet pengaman baru
					...
					while ($isi = mysql_fetch_array($qrp)){
						//$jns_trans=3; //$jns_trans2 = 6;
						$debet=$isi['biaya_pengamanan']; $kredit=0;
						$jml_barang_d=0; $jml_barang_k=0;
						$jns_trans3=4; //daftar hapus sebagian
						$refid2 = $isi['id'];
						$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
					}
					// - debet penghapusan sebagian baru
					$qrp = mysql_query("select * from penghapusan_sebagian where idbi_awal ='".$bi['idawal']."' and tgl_penghapusan<='".$tgl_buku."' ");
					while ($isi = mysql_fetch_array($qrp)){
						$debet=0; $kredit=$isi['harga_hapus'];
						$jml_barang_d=0; $jml_barang_k=0;
			...
			case 2 :{
				mysql_query("update t_jurnal set tgl_buku='$tgl_buku'  where refid='$idpindah' and jns_trans2='$jns_trans2' ");
				break;
			}
			case 3 :{
				mysql_query("delete from t_jurnal where refid='$idpindah' and jns_trans2='$jns_trans2' ");
				break;
			}
		}
		//$cek = 'ok';
	...
	function jurnalGetKodeRekap($staset, $f, $g){
		if($staset==10){
			$kint = '02' ; $ka='00'; $kb='00';
		}else {
			$kint = '01' ;
		...
		return	array ('kint'=>$kint, 'ka'=>$ka, 'kb'=>$kb );
	}

	function jurnalGantiRugi($idpindah,$uid,$st){
		$cek = ''; $err='';
		$plh = mysql_fetch_array(mysql_query("select * from gantirugi where id='$idpindah' "));
		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id='{$plh['id_bukuinduk']}' "));

		$harga_atribusi=0;
		$jns_trans2 = 11; //
		$refid=$idpindah;
		$tgl_buku = $plh['tgl_gantirugi'];
		...
		switch($st){
			case 1 :{ //baru
				...
				$status_barang = 1; //$bi['status_barang']
				$jns_trans3=1; $refid2 = $bi['id'];
				//$kint=$bi['staset']==10 ? '02': '01';
				//$ka=$bi['f']; 	$kb=$bi['f'];
				$kd = jurnalGetKodeRekap($staset, $bi['f'], $bi['g']);
				$kint=$kd['kint']; $ka=$kd['ka']; $kb=$kd['kb'];
				$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb );
				jurnalInsertRehab(
					FALSE, $bi['idawal'], $tgl_buku, $staset, $jns_trans, $jns_trans2, $refid,
					$uid, $bi, '', $kint, $ka, $kb , $f, $g
				);
				...
				while ($isi = mysql_fetch_array($qrp)){
					...
					$jns_trans3=3; //daftar pemeliharaan
					$refid2 = $isi['id'];
					jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb	);
				}
				// - kredit  pengaman lama
				$qrp = mysql_query("select * from pengamanan where idbi_awal ='".$bi['idawal']."' and tambah_aset=1 and tgl_pengamanan<='". $tgl_buku."' ");
				while ($isi = mysql_fetch_array($qrp)){
				...
				*/
				//debet baru ----------------------------------------------------
				$kint = '01' ; $ka='02'; $kb='02'; $f='07' ; $g='22';
				$debet=$bi['harga'];  $kredit=0; $jml_barang_d=$bi['jml_barang'];  $jml_barang_k=0;
				$staset = 6; $status_barang = $bi['status_barang'];
				$jns_trans3=1; $refid2 = $bi['id'];
				jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
				jurnalInsertRehab(
					TRUE, $bi['idawal'], $tgl_buku, $staset, $jns_trans, $jns_trans2, $refid,
					$uid, $bi, '', $kint, $ka, $kb , $f, $g
				);
	...
	function jurnalInsertRehab(
		$didebet, $idawal, $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,
		$uid,$bi,$status_barang='', $kint, $ka, $kb , $f='', $g=''
	){
		$cek = ''; $err='';
		...
		$harga_atribusi = 0;
		$aqry = "select * from pemeliharaan where idbi_awal ='".$idawal."' and tambah_aset=1 and tgl_pemeliharaan<='". $tgl_buku."' "; $cek .= $aqry;
		$qrp = mysql_query($aqry);
		while ($isi = mysql_fetch_array($qrp)){
			if($didebet){
				$debet=$isi['biaya_pemeliharaan']; $kredit= 0;
		...
		// - kredit  pengaman lama
		$qrp = mysql_query("select * from pengamanan where idbi_awal ='".$idawal."' and tambah_aset=1 and tgl_pengamanan<='". $tgl_buku."' ");
		while ($isi = mysql_fetch_array($qrp)){
			if($didebet){
				$debet=$isi['biaya_pengamanan']; $kredit= 0;
			}else{
		...
		// - kredit hapus sebagian lama
		$qrp = mysql_query("select * from penghapusan_sebagian where idbi_awal ='".$idawal."' and tgl_penghapusan<='". $tgl_buku."' ");
		while ($isi = mysql_fetch_array($qrp)){
			if($didebet){
				$debet=-$isi['harga_hapus']; $kredit= 0;
			}else{
		...
		return	array ('cek'=>$cek, 'err'=>$err, 'newid'=> $newid);
	}

	function insertKoreksi($tgl_koreksi, $oldbi, $idbi,$uid, $jns_trans='' ){ //$oldbi = fetch_array
		$newid = ''; $cek = ''; $err='';

		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id = '$idbi'"));
		//koreksi staset
		if($oldbi['staset'] != $bi['staset']
		//&& ($bi['staset']=='10' || $oldbi['staset']=='10' )
		){
			$jns_trans =  $bi['staset']=='10' || $oldbi['staset']=='10'  ? 16: 17;
			//tgl,jnstrans,idbi,staset,staset_baru,uid,tgl_update
			mysql_query("insert into t_koreksi (tgl,jns_trans,idbi,staset,staset_baru,uid,tgl_update) ".
				"value ('$tgl_koreksi','$jns_trans','$idbi','{$oldbi['staset']}', '{$bi['staset']}','$uid','now()') "
			);
		}
		if($oldbi['harga'] != $bi['harga'] ){
			$jns_trans = 18;
			mysql_query("insert into t_koreksi (tgl,jns_trans,idbi,harga,harga_baru,uid,tgl_update) ".
				"value ('$tgl_koreksi','$jns_trans','$idbi','{$oldbi['harga']}', '{$bi['harga']}','$uid','now()') "
			);
		}
	...
	function jurnalAsetLainLain($oldbi, $idbi,$uid,$tgl_buku,$st=1, $koreksi=FALSE, $refid_=''){
		$cek = ''; $err='';
		//$plh = mysql_fetch_array(mysql_query("select * from pemanfaatan where id='$id' "));
		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id='$idbi' "));

		$harga_atribusi=0;
		//$jns_trans2 = $koreksi ? 17: 14; //koreksi / aset lain-lain
		if($koreksi) {
			$refid=$idbi;
			$jns_trans2 = 17;
		}else{
		...
		switch($st){
			case 1 :{ //baru
				$jns_trans=$koreksi? 8: 9; //group koreksi/ reclass

				$harga_atribusi=0;


				//kredit lama ----------------------------------------------
				$debet=0; $kredit=$oldbi['harga'];
				$jml_barang_d=0; $jml_barang_k=$oldbi['jml_barang'];

				$staset = $oldbi['staset'];
				$status_barang = $oldbi['status_barang'];
				$jns_trans3=1;
				$refid2 = $oldbi['id'];
				if($bi['staset']==9){ //ke aset lainlain
					$kd = jurnalGetKodeRekap($staset, $oldbi['f'], $oldbi['g']);
					$kint=$kd['kint']; $ka=$kd['ka']; $kb=$kd['kb']; $f=$oldbi['f']; $g=$oldbi['g'];
				}else{ //dari
					$kint = '01' ; $ka='02'; $kb='05'; $f='07' ; $g='25';
				}
				$ins = jurnalInsert( $tgl_buku,$staset,$jns_trans,$jns_trans2,$refid,$jns_trans3,$refid2, $debet,$kredit,$jml_barang_d,$jml_barang_k,$harga_atribusi, $uid,$bi,$status_barang, $kint, $ka, $kb, $f, $g );
				$cek .= $ins['cek'];
				$ins= jurnalInsertRehab(
					FALSE, $oldbi['idawal'], $tgl_buku, $staset, $jns_trans, $jns_trans2, $refid,
					$uid, $oldbi, '', $kint, $ka, $kb , $f, $g
				);
				$cek .= $ins['cek'];
	...
	function jurnalKapitalisasi($oldbi, $idbi,$uid,$tgl_buku,$st=1, $koreksi=FALSE, $refid=''){//intra extra
		$cek = ''; $err='';
		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id='$idbi' "));

		$harga_atribusi=0;
		$jns_trans2 = $koreksi? 18: 16; // koreksi / kapitalisasi
		$refid= $refid==''? $idbi : $refid;
		//$tgl_buku = "2014-12-31";//tes

		switch($st){
			case 1 :{ //baru
				$jns_trans= $koreksi? 8: 3; //group koreksi/kapiotalisasi
				$harga_atribusi=0;
				...
				$jns_trans3=1; //
				$refid2 = $bi['id'];
				//if($bi['staset']==10){ //ke aset lainlain
				//	$kint = '01' ; $ka='02'; $kb='05'; $f='07' ; $g='25';
				//}else{ //dari
					$kd = jurnalGetKodeRekap($staset, $bi['f'], $bi['g']);
					$kint=$kd['kint']; $ka=$kd['ka']; $kb=$kd['kb']; $f=$bi['f']; $g=$bi['g'];
	...
	function jurnalKoreksi($oldbi, $idbi, $uid, $tgl_koreksi='',$koreksi = TRUE){//$oldbi = fetch_array
		$cek = ''; $err='';
		$tgl_koreksi = '2014-12-31'; //tes
		$tgl_koreksi = $tgl_koreksi==''? date('Y-m-d') : $tgl_koreksi;
		$bi = mysql_fetch_array(mysql_query("select * from buku_induk where id = '$idbi'"));

		//; //koreksi or transaksi

		///if($koreksi){ //koreksi
			//koreksi staset extra - intra/ intra extra
		...
		//else{
			...
			if($oldbi['staset'] != $bi['staset'] &&
				(
					($bi['staset']=='9' && ($oldbi['staset']=='3' || $oldbi['staset']=='10') ) || 	//ke aset lain
					($oldbi['staset']=='9' && ($bi['staset']=='3' || $bi['staset']=='10'))			// dari aset lain
				)
			){
				jurnalAsetLainLain($oldbi, $idbi,$uid,$tgl_koreksi, 1, $koreksi);
			}

			// kapitalisasi intra extra
			if($oldbi['staset'] != $bi['staset'] &&
				( ($bi['staset']=='10' && $oldbi['staset']=='3' )||  //extra ke intra
					( $oldbi['staset']=='10' && $bi['staset']=='3' ) //intra ke extra
				) ){
				jurnalKapitalisasi($oldbi, $idbi,$uid,$tgl_koreksi, 1, $koreksi);
			}

			if($oldbi['harga'] != $bi['harga'] ){
				$jns_trans= 8; //koreksi pembukuan
				$jns_trans2 = 18; //koreksi harga
				if($bi['harga'] > $oldbi['harga']){ //bertambah
					$debet=$bi['harga']-$oldbi['harga']; $kredit=0;
				}else{ //berkurang
					$debet=0; $kredit=$oldbi['harga']-$bi['harga'];
				}
				$harga_atribusi=0;
				$jml_barang_d=0; $jml_barang_k=0;

				$kor = insertKoreksi($tgl_koreksi, $oldbi, $idbi,$uid );
				$refid=$kor['newid'];// $idbi;
				$tgl_buku = $tgl_koreksi;
			...
			if($oldbi['asal_usul'] != $bi['asal_usul'] ){ //?
	...
	function forbiddenChar($string=''){
		//™|®|©
		$newstring = '';
		$forbidden = array(153,174,169,194);
...
*/
				...
				//cek tahun closing
				if($Baru==1){//baru
					if($errmsg=='' && sudahClosing($tgl_buku, $fmSKPD, $fmUNIT, $fmSUBUNIT, $fmSEKSI) ) $errmsg = 'Tanggal Buku harus lebih besar dari Tanggal Closing !';
				}else{//edit
					$idbi = $_POST['idbi'];
					$old =  mysql_fetch_array( mysql_query (
						"select * from buku_induk where id = '$idbi'"
					));
					if($errmsg=='' && $tgl_buku!=$old['tgl_buku'] && (sudahClosing($tgl_buku, $fmSKPD, $fmUNIT, $fmSUBUNIT, $fmSEKSI) || ($old['id']<>$old['idawal']) || ($old['status_barang']=='2' || $old['staset']=='7')) ){
					...
					if($errmsg=='' && $tgl_buku!=$old['tgl_buku'] && sudahClosing($tgl_buku, $fmSKPD, $fmUNIT, $fmSUBUNIT, $fmSEKSI) ) $errmsg = 'Tanggal Buku harus lebih besar dari Tanggal Closing !';
				}
					...
					switch($Baru){
						case '0':{//edit ------------------------------------------------------------------
								...
								*/
								...
								if($tgl_buku!=$old['tgl_buku'] && (sudahClosing($tgl_buku, $fmSKPD, $fmUNIT, $fmSUBUNIT, $fmSEKSI) || ($old['id']<>$old['idawal']) || ($old['status_barang']=='2' || $old['staset']=='7')) ){
									$update="";
									$update2="";
				...
				if($Act=="Hapus" && count($cidBI) > 0){
					for($i = 0; $i<count($cidBI); $i++)	{
						...
						if($isi = mysql_fetch_array($Qry))	{
							...
							if($errmsg=='' && sudahClosing($isi['tgl_buku'], $isi['c'], $isi['d'], $isi['e'], $isi['e1']) ) $errmsg = 'Barang Sudah Closing !';
							...
							if($errmsg == ''){
								...
								*/
								...
								if($errmsg==''){
									...
									if ( sudahClosing($hps['tgl_penghapusan'], $bilama['c'], $bilama['d'], $bilama['e'], $bilama['e1']) ){
...
//		$alm .= ifempty($alamat,'-');
		$alm .= $alamat;
		$alm .= $alamat_kel != ''? '<br>Kel/Desa. '.$alamat_kel : '';
		$alm .= $nm_kec != ''? '<br>Kec. '.$nm_kec : '';
		if ($nm_kec!=''){
...
function PrintTTD_SensusKK($thnsensus='2013',$pagewidth = '30cm', $xls=FALSE, $cp1='', $cp2='', $cp3='', $cp4='', $cp5='' ) {
    ...
    $NAMASKPD = "";
    $JABATANSKPD = "";
    $vTITIKMANGSA = $Main->CETAK_LOKASI.", " . JuyTgl1(date("Y-m-d"));
	$kdSubUnit0 = genNumber(0, $Main->SUBUNIT_DIGIT );
    if (c == '04') {
	...
	if($xls == FALSE){
		...
		$vNIP1	= "<INPUT TYPE=TEXT VALUE='NIP. $NIPSKPD1' STYLE='background:none;border:none;text-align:center;font-weight:bold' size=50>";
		$vNIP2	= "<INPUT TYPE=TEXT VALUE='NIP. $NIPSKPD2' STYLE='background:none;border:none;text-align:center;font-weight:bold' size=50>";
		$vTITIKMANGSA 	= "<B><INPUT TYPE=TEXT VALUE='".$Main->CETAK_LOKASI.",  ". JuyTgl1(date("Y-m-d"))."' STYLE='background:none;border:none;text-align:center;font-weight:bold' size=50>";
		$vMENGETAHUI 	= "<B><INPUT TYPE=TEXT VALUE='MENGETAHUI' STYLE='background:none;border:none;text-align:center;font-weight:bold' size=50 >";
		...
		$vNIP1	= "<span STYLE='background:none;border:none;text-align:center;font-weight:bold' size=50 >NIP. $NIPSKPD1</span>";
		$vNIP2	= "<span STYLE='background:none;border:none;text-align:center;font-weight:bold' size=50 >NIP. $NIPSKPD2</span>";
		$vTITIKMANGSA 	= "<B><span STYLE='background:none;border:none;text-align:center;font-weight:bold' size=50 >".$Main->CETAK_LOKASI." ,  ". JuyTgl1(date("Y-m-d"))."' </span>";
		$vMENGETAHUI 	= "<B><span STYLE='background:none;border:none;text-align:center;font-weight:bold' size=50 >MENGETAHUI</span>";
	...
	*/
    $sqry = "
		...
		"from ref_barang aa
			...
			".
		" where aa.h='00' $kondisiasettetap
		order by aa.f, aa.g
	";
	// echo "$sqry";
	// $cek.=$sqry;
    ...
    while ($isi = mysql_fetch_array($QryRefBarang)) {
        ...
        //data --------------------------------------------------
		//penghapusan
        $jmlBrgHPS_awal =  0; //$isi['jmlBrgHPS_awal'];
		$jmlHrgHPS_awal =  0; //$isi['jmlHrgHPS_awal'];
		$jmlBrgHPS_akhir =  0; //$isi['jmlBrgHPS_akhir'];
		$jmlHrgHPS_akhir =  0; //$isi['jmlHrgHPS_akhir'];
		$jmlBrgHPS_curr = $jmlBrgHPS_akhir - $jmlBrgHPS_awal;
		$jmlHrgHPS_curr = $jmlHrgHPS_akhir - $jmlHrgHPS_awal;
		//buku_induk
//        $jmlBrgBI_awal = $isi['jmlBrgBI_awal'];
		...
		//hapus pelihara
		$jmlHrgHPS_PLH_awal =  0; //$isi['jmlHrgHPS_PLH_awal'];
		$jmlHrgHPS_PLH_akhir =  0; //$isi['jmlHrgHPS_PLH_akhir'];
		$jmlHrgHPS_PLH_curr = $jmlHrgHPS_PLH_akhir - $jmlHrgHPS_PLH_awal;
		//hapus pengaman
		$jmlHrgHPS_Aman_awal =  0; //$isi['jmlHrgHPS_Aman_awal'];
		$jmlHrgHPS_Aman_akhir =  0; //$isi['jmlHrgHPS_Aman_akhir'];
		$jmlHrgHPS_Aman_curr = $jmlHrgHPS_Aman_akhir - $jmlHrgHPS_Aman_awal;
		//hapus hapus sebagian
		$jmlHrgHPS_HSBG_awal =  0; //$isi['jmlHrgHPS_HSBG_awal'];
		$jmlHrgHPS_HSBG_akhir =  0; //$isi['jmlHrgHPS_HSBG_akhir'];
		$jmlHrgHPS_HSBG_curr = $jmlHrgHPS_HSBG_akhir - $jmlHrgHPS_HSBG_awal;
		...
		//berkurang
		$jmlBrgKurang_curr = 0; //$jmlBrgHPS_curr + $jmlBrgPindah_curr + $jmlBrgGantirugi_curr;
		$jmlHrgKurangPLH_curr = 0; // $jmlHrgHPS_PLH_curr + $jmlHrgPindah_PLH_curr + $jmlHrgGantirugi_PLH_curr;
		$jmlHrgKurangAman_curr = 0; // $jmlHrgHPS_Aman_curr + $jmlHrgPindah_Aman_curr + $jmlHrgGantirugi_Aman_curr;
		$jmlHrgKurangBi_curr = 0; // $jmlHrgHPS_curr + $jmlHrgPindah_curr + $jmlHrgGantirugi_curr;
		$jmlHrgKurangHSBG_curr = $jmlHrgHSBG_curr + $jmlHrgMut_HSBG_curr;
...
function Mutasi_RekapByBrg_GetList3_Keu($fmKEPEMILIKAN, $fmSKPD, $fmUNIT, $fmSUBUNIT,
	$noawal, $limitHal, $kolomwidth, $dlmRibuan=TRUE, $cetak=FALSE,
	$Style=1, $fmSEKSI='00', $fmKONDBRG='7',$jns_aset=0,$jmlBarangAwal=0,$jmlHargaAwal=0,$tampiltot=0) {
    // ------------------------------
    // $Style=1 = total penambahan, 2= pemelihara, pemgaman, peroleh, 3 = saldo akhir sampai dgn tglakhir
...
$ListHeader =
	"<tr>
		...
		<th class='th02' width=''>HASIL AUDIT</th>
...
$aqry = "select * from bpk  group by no,c,d,e,e1 $Limit";// group by c,d,e";
$qry = mysql_query($aqry);
...
while($isi=mysql_fetch_array($qry)){
	$no++;
	$aqry = "select * from ref_skpd where c='".$isi['c']."' and d='".$isi['d']."' and e='".$isi['e']."' and e1='".$isi['e1']."' ";// echo $aqry;
	$get = mysql_fetch_array(mysql_query($aqry));
	...
	list($ListData, $jmlData, $data, $cek) =
		Banding($fmKEPEMILIKAN, $isi['c'], $isi['d'], $isi['e'],$isi['e1'],
...
"<tr class='$clRow'>
	...
	list($ListData, $jmlData, $data, $cek) =
		Banding($fmKEPEMILIKAN, '00', '00', '00','000',
			0,//$Main->PagePerHal * (($HalDefault*1) - 1),
			400, //0
			$kolomwidth, !empty($cbxDlmRibu), $cetak, 4
		);
...
function Banding($fmKEPEMILIKAN, $fmSKPD, $fmUNIT, $fmSUBUNIT,$fmSEKSI, $noawal, $limitHal, $kolomwidth, $dlmRibuan=TRUE, $cetak=FALSE, $Style=1) {
    // ------------------------------
    // $Style=1 = total penambahan, 2= pemelihara, pemgaman, peroleh, 3 = saldo akhir sampai dgn tglakhir
		    ...
		    //    case 4: {
					if($fmSKPD != '00'){
						$aqry = " select * from bpk ".
							" where c='".$fmSKPD."' and d='".$fmUNIT."' and e='".$fmSUBUNIT."' and e1='".$fmSEKSI."' ".
...
function jamToPetik($jmljam){
	...
	$tmp = ($tmp-$menit)*60;
	$detik = $tmp;//floor($tmp);
	$vjam = ($jam<=99) ?genNumber($jam,2): $jam;
	$topetik = $vjam.':'.genNumber($menit,2).':'.genNumber($detik,2);
	return $topetik;
}

function Penatausahaan_FormEntry_reclass($currID=0, $btCariBrg=TRUE , $entryMutasi=FALSE){
		global $Main, $btnTampilKib, $titleAct, $DisAbled;
		global $Pg, $SPg, $fmIDBARANG, $fmNMBARANG, $fmTAHUNPEROLEHAN, $fmREGISTER, $fmSATUAN;
		...
		global $ref_idpenanggung, $nama2, $nip2, $jbt2;
		global $ref_idpemegang, $nama1, $nip1, $jbt1;
		global $ref_idpemegang2, $nama3, $nip3, $jbt3;
		global $nm_gedung, $nm_ruang, $ref_idruang;
		...
		if ($Baru==0){	//edit
			...
			formEntryBase2('Tanggal Cek Barang',':',
				TglJamInd($tgl_sensus)." <input type='hidden' id='tgl_sensus' name='tgl_sensus' value='$tgl_sensus'>"
				,'','','','valign="" height="24"'
			).
			formEntryBase2('Tahun Sensus',':',
				$tahun_sensus." <input type='hidden' id='tahun_sensus' name='tahun_sensus' value='$tahun_sensus'>"
				,'','','','valign="top" height="24"'
			);
		...
		//if ($btCariBrg){
			$entryBrg =
				formEntryBase2('Nama Barang',':',
				cariInfo2("adminForm","pages/01/caribarang1.php","pages/01/caribarang_reclass.php",
				"fmIDBARANG","fmNMBARANG","readonly","$DisAbled",
				" onselect=\"adminForm.action='?Pg=$Pg&SPg=$SPg';adminForm.submit();\" ",$KIB),
		...
		"<tr><td>Pengguna/Kuasa Pengguna Barang</td><td>:</td><td>".
				...
				"NIP  &nbsp;<input type='text' id='nip2' readonly=true value='".$penanggung['nip']."' style='width:150' > ".
				"<input type='button' value='Pilih' onclick=\"pilihPenanggung()\">".
				"<input type='button' value='Reset' onclick=\"document.getElementById('nama2').value='';document.getElementById('nip2').value='';document.getElementById('jbt2').value='';document.getElementById('ref_idpenanggung').value='';\">".
		"</td></tr>".
		"<tr><td></td><td></td><td>JABATAN  &nbsp;<input type='text' id='jbt2' readonly=true value='".$penanggung['jabatan']."' style='width:380'> </td></tr>".
		"<tr><td>Pengurus Barang/Pembantu</td><td>:</td><td>".
			"<script>
			...
			"NIP  &nbsp;<input type='text' id='nip1' readonly=true value='".$pemegang['nip']."' style='width:150' > ".
			"<input type='button' value='Pilih' onclick=\"pilihPemegang()\">".
			"<input type='button' value='Reset' onclick=\"document.getElementById('nama1').value='';document.getElementById('nip1').value='';document.getElementById('jbt1').value='';document.getElementById('ref_idpemegang').value='';\">".
		"</td></tr>".
		...
		"<tr><td>Penanggung Jawab Barang</td><td>:</td><td>".
			...
			"NIP  &nbsp;<input type='text' id='nip3' readonly=true value='".$pemegang2['nip']."' style='width:150' > ".
			"<input type='button' value='Pilih' onclick=\"pilihPemegang2()\">".
			"<input type='button' value='Reset' onclick=\"document.getElementById('nama3').value='';document.getElementById('nip3').value='';document.getElementById('jbt3').value='';document.getElementById('ref_idpemegang2').value='';\">".
		"</td></tr>".
		"<tr><td></td><td></td><td>JABATAN  &nbsp;<input type='text' id='jbt3' readonly=true value='".$pemegang2['jabatan']."' style='width:380'> </td></tr>".
		...
		"<tr><td>Gedung/Ruang</td><td>:</td><td>".
			...
			" <input type='text' id='nm_ruang' value='".$ruang['nm_ruang']."' readonly='true' style='width:205'>".
			" <input type='button' value='Pilih' onclick=\"pilihRuang()\" >".
			" <input type='button' value='Reset' onclick=\"document.getElementById('nm_gedung').value='';document.getElementById('nm_ruang').value='';document.getElementById('ref_idruang').value='';\" >".
			" <input type='hidden' id='ref_idruang' name='ref_idruang' value='".$ref_idruang."'>".
		...
		if ($Baru==0){
			...
			if ($disModul06 == ''){
				...
				"<div id='divPemanfaatList'>".
				//$Pemanfaat->GetList($Kondisi, '', '', 1, 'koptable2', FALSE, 0, ($ridModul06!='')).
				$Pemanfaat->GetList($Kondisi, '', '', 1, 'koptable2', FALSE, 0, TRUE).
				"</div></div>";
				$PemanfaatBar = createBarPanel('barPemanfaat', 'Pemanfaatan', 25, $PemanfaatBar);
			}
		}
...
function getKodeRekap($staset,$f){

	switch ($staset){
		case '3' : $kint = '01'; $ka='01'; $kb=$f; break;
		case '8' : $kint = '01'; $ka='02'; $kb='04'; break;
		case '10' :	$kint = '02'; $ka='00'; $kb='00'; break;
	}

	return array('kint'=>$kint, 'ka'=> $ka, 'kb'=>$kb);
}

function sudahClosing($tgl, $c='00', $d='00', $e='00', $e1='000'){
	global $Main;
	$tglclosing = getTglClosing($c,$d,$e,$e1);
	$hsl =  compareTanggal($tgl, $tglclosing)==0 || compareTanggal($tgl,$tglclosing)==1 ;
	return $hsl;
}

function getTglClosing($c='00', $d='00', $e='00', $e1='000'){
	global $Main, $HTTP_COOKIE_VARS;
	$cek ='';
	$coGroup = $HTTP_COOKIE_VARS['coGroup'];
	$coLevel = $HTTP_COOKIE_VARS['coLevel'];
	if($Main->VERSI_NAME=='KOTA_BANDUNG' && $coGroup =='00.00.00.000' && $coLevel==1  ){
		$get['tgl'] =0;
	}else{
		//if($Main->CLOSING_SKPD){
			...
			if($get['tgl']== NULL){
				$aqry = "select * from t_closing where c='$c' and d='$d' and e='$e' and e1='000' order by Id desc limit 0,1 "; $cek .= $aqry;
				$get = mysql_fetch_array(mysql_query( $aqry ));
			}
			if($get['tgl']== NULL){
				$aqry = "select * from t_closing where c='$c' and d='$d' and e='00' and e1='000' order by Id desc limit 0,1 " ; $cek .= $aqry;
				$get = mysql_fetch_array(mysql_query($aqry));
			}
			if($get['tgl']== NULL){
				$aqry = "select * from t_closing where c='$c' and d='00' and e='00' and e1='000' order by Id desc limit 0,1 "; $cek .= $aqry;
				$get = mysql_fetch_array(mysql_query( $aqry ));
			}
			if($get['tgl']==NULL){
				$aqry = "select * from t_closing where c='00' and d='00' and e='00' and e1='000'  order by Id desc limit 0,1 " ; $cek .= $aqry;
				$get = mysql_fetch_array(mysql_query( $aqry));
			}
...
function getNilaiBuku_($idbi, $tgl, $withSusut=FALSE){

	$tot=0;
	$get = mysql_query(mysql_fetch_array(
		" select jml_harga as tot, idawal from buku_induk Where id='$idbi' "
	));
	$tot += $get['tot'];
	$idbiawal = $get['idawal'];
	...
	//tot_perlihara
	$get = mysql_query(mysql_fetch_array(
		" select Sum(biaya_pemeliharaan) as tot from pemeliharaan ".
		" Where idbi_awal=$idbiawal and tambah_aset=1 and tgl_pemeliharaan<='$tgl' "
	));
	$tot += $get['tot'];

	//tot_pengaman
	$get = mysql_query(mysql_fetch_array(
		" select Sum(biaya_pengamanan) as tot from pengamanan ".
		" Where idbi_awal=$idbiawal and tambah_aset=1 and tgl_pengamanan<='$tgl' "
	));
	$tot += $get['tot'];

	//tot penghapusan_sebagian
	$get = mysql_query(mysql_fetch_array(
		" select Sum(harga_hapus) as tot from penghapusan_sebagian ".
		" Where idbi_awal=$idbiawal and tgl_penghapusan<='$tgl' "
	));
	$tot -= $get['tot'];

	//tot koreksi
	$get = mysql_query(mysql_fetch_array(
		" select Sum(harga_baru - harga) as tot from t_koreksi ".
		" Where idbi_awal=$idbiawal and tgl<='$tgl' "
	));
	$tot += $get['tot'];

	//tot penilaian
	$get = mysql_query(mysql_fetch_array(
		" select Sum(nilai_barang-nilai_barang_asal) as tot from penilaian ".
		" Where idbi_awal=$idbiawal and tgl_penilaian<='$tgl' "
	));
	$tot += $get['tot'];
	...
	if($withSusut){
		//tot susut
		$get = mysql_query(mysql_fetch_array(
			" select Sum(harga) as tot from penyusutan ".
			" Where idbi=$idbi and tgl<='$tgl' "
		));
		$tot -= $get['tot'];
	}
