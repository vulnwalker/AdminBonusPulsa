...
var ref_pegawai = new DaftarObj2({
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			$.ajax({
				type:'POST', 
				data:$('#ref_pegawai_form').serialize(),
...
PemdaEdit: function(){	
		...
		if (err =='' ){		
			...
			$.ajax({
				type:'POST', 
				data:$('#ref_pegawai_form').serialize()+"&idPemda="+$("#cmbPemda").val(),
			  	url: this.url+'&tipe=formBaruPemda',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');			
					document.getElementById(cover).innerHTML = resp.content;			
...
AplikasiEdit: function(){	
		...
		if (err =='' ){		
			...
			$.ajax({
				type:'POST', 
				data:$('#ref_pegawai_form').serialize()+"&idAplikasi="+$("#cmbAplikasi").val(),
			  	url: this.url+'&tipe=formBaruAplikasi',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');			
					document.getElementById(cover).innerHTML = resp.content;			
	...
	Edit:function(){
		...
		if(errmsg ==''){ 
			...
			$.ajax({
				type:'POST', 
				data:$('#ref_pegawaiForm').serialize(),
