	...
	Edit:function(id){
		...
		if(errmsg ==''){ 
			...
			$.ajax({
				type:'POST', 
				data:{id : id},
				url: 'http://123.231.253.228/atisisbada_v2/curl/moveTemplateToTemp.php',
				success: function(data) {	
						$.ajax({
							type:'POST', 
							data:{template_cb : id},
		...
		var me = this;
			if($("#nama_template").val() == '' || $('#tanggal').val() == '' || $("#nomor").val() == '' || document.getElementById('cmbUnitForm').value == '' || document.getElementById('cmbUnitForm').value == '00'  ){
				...
				var postTanggal = $('#tanggal').val().split("-");
						 $.ajax({
								type : 'POST',
								url: 'http://123.231.253.228/atisisbada_v2/curl/kembalikanKeTemplate.php',
								data:	{ 
		...
		var me = this;
			if($("#nama_template").val() == '' || $('#tanggal').val() == '' || $("#nomor").val() == '' || document.getElementById('cmbUnitForm').value == '' || document.getElementById('cmbUnitForm').value == '00'  ){
				...
				var postTanggal = $('#tanggal').val().split("-");
						 $.ajax({
								type : 'POST',
								url: 'http://123.231.253.228/atisisbada_v2/curl/updateTempRincianTemplate.php',
								data:	{ 
										 nama : $("#nama_template").val(),
										 tanggal : postTanggal[2] + "-" + postTanggal[1] + "-" + postTanggal[0],
										 nomor : $("#nomor").val(),
