var ref_aplikasi = new DaftarObj2({
	prefix : 'ref_aplikasi',
	url : 'pages.php?Pg=ref_aplikasi', 
	formName : 'ref_aplikasiForm',
	ref_aplikasi_form : '0',
	loading: function(){
		//alert('loading');
		this.topBarRender();
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			if(me.ref_aplikasi_form==0){//baru dari ref_aplikasi
	...
	checkref_aplikasi:function(){
		...
		if(errmsg ==''){ 
			...
			$.ajax({
				type:'POST', 
				data:$('#'+this.formName).serialize(),
				url: this.url+'&tipe=checkref_aplikasi',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');	
					if (resp.err ==''){		
						document.getElementById(cover).innerHTML = resp.content;
	...
	showKoreksiref_aplikasi:function(id){
		var me = this;
			...
			$.ajax({
				type:'POST', 
				data:{id:id},
				url: this.url+'&tipe=showKoreksiref_aplikasi',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');	
					if (resp.err ==''){		
						document.getElementById(cover).innerHTML = resp.content;
