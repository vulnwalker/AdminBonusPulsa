...
var pengguna_aplikasi = new DaftarObj2({
	prefix : 'pengguna_aplikasi',
	url : 'pages.php?Pg=pengguna_aplikasi', 
	//formName : 'pengguna_aplikasi_form',
	formName : 'pengguna_aplikasiForm',
	//pengguna_aplikasi_form
	loading: function(){
		//alert('loading');
		this.topBarRender();
		this.filterRender();
	...
	Baru: function(){	
		...
		if (err =='' ){		
			...
			$.ajax({
				type:'POST', 
				data:$('#pengguna_aplikasi_form').serialize(),
...
PemdaEdit: function(){	
		...
		if (err =='' ){		
			...
			$.ajax({
				type:'POST', 
				data:$('#pengguna_aplikasi_form').serialize()+"&idPemda="+$("#cmbPemda").val(),
			  	url: this.url+'&tipe=formBaruPemda',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');			
					document.getElementById(cover).innerHTML = resp.content;			
...
AplikasiEdit: function(){	
		...
		if (err =='' ){		
			...
			$.ajax({
				type:'POST', 
				data:$('#pengguna_aplikasi_form').serialize()+"&idAplikasi="+$("#cmbAplikasi").val(),
			  	url: this.url+'&tipe=formBaruAplikasi',
			  	success: function(data) {		
					var resp = eval('(' + data + ')');			
					document.getElementById(cover).innerHTML = resp.content;			
	...
	Edit:function(){
		...
		if(errmsg ==''){ 
			...
			$.ajax({
				type:'POST', 
				data:$('#pengguna_aplikasiForm').serialize(),
