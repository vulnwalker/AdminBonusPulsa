...
var penukaran = new DaftarObj2({
	prefix : 'penukaran',
	url : 'pages.php?Pg=penukaran',
	//formName : 'penukaran_form',
	formName : 'penukaranForm',
	//penukaran_form
	loading: function(){
		//alert('loading');
		this.topBarRender();
...
PemdaEdit: function(){
		...
		if (err =='' ){
			...
			$.ajax({
				type:'POST',
				data:$('#penukaran_form').serialize()+"&idPemda="+$("#cmbPemda").val(),
			  	url: this.url+'&tipe=formBaruPemda',
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					document.getElementById(cover).innerHTML = resp.content;
...
AplikasiEdit: function(){
		...
		if (err =='' ){
			...
			$.ajax({
				type:'POST',
				data:$('#penukaran_form').serialize()+"&idAplikasi="+$("#cmbAplikasi").val(),
			  	url: this.url+'&tipe=formBaruAplikasi',
			  	success: function(data) {
					var resp = eval('(' + data + ')');
					document.getElementById(cover).innerHTML = resp.content;
